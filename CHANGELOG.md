...
### Added
  - 关闭开屏动画时新增主题自适应的「石墨蚀刻光场」启动占位：以宿主编辑器背景、侧栏、浮层与前景色变量生成银黑/雾白灰阶渐变，叠加宽幅棱面、椭圆折射层和极淡拉丝纹理；中心光环低频呼吸并轻微偏转，斜向交界的灰银反光周期性掠过，明确表示仍在加载，同时不复刻正式 Splash 的 Logo 演出；不含 logo、文字或传统转圈图标，`prefers-reduced-motion` 下恢复静态画面。设置读取完成且明确关闭开屏动画后才显示，并持续至主界面初始化结束；配置尚未解析或明确开启期间均不挂载。

### Fixed
  - 修复开启开屏动画时仍会先显示关闭态「石墨蚀刻光场」的问题：启动偏好尚未解析时不再预挂载任一偏好专属画面；只有配置明确关闭时才显示关闭态占位，明确开启时从 Splash 挂载到结束均不会渲染该内容。
