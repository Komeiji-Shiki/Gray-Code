...
### Fixed
  ...
  - 流式/挂起：`streamBufferParser` 修复 SSE `data:` 前缀被 chunk 边界切断时事件静默丢失（chunk 末尾非完整行并入 remaining 等待拼接）；`ToolIterationLoopService` 早启动工具等待循环增加超时兜底（与取消路径对齐，超时未落定工具以失败占位结算，避免悬空 tool_use 触发 API 400 与请求无限挂起）；`processRunner` 超时杀进程补 SIGKILL 升级（对齐 `killTerminalProcess` 的 KILL_WAIT_CLOSE_TIMEOUT_MS，SIGTERM 免疫进程不再让 execute_command 永久挂起）。
