...
### Fixed
  - 前端：修复 1.5.0 主界面新对话首条消息闪现后消失、会话无法创建的问题——首次空白标签页改在任何初始化 IPC 前同步建立，异步对话列表改为保留加载期间刚创建的持久化会话；渠道配置预加载延后到关键聊天初始化完成并与 InputArea 共享缓存，避免重复请求占用消息队列；无标签页承接的迟到流补全等待状态回收。
