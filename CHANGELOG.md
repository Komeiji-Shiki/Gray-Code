...
### Fixed
  ...
  - 修复工具执行存档部分快照（CP-PARTIAL-1）的恢复误删（CP-PARTIAL-2，审查发现）：部分快照的 fileHashes 只含受影响文件，恢复引擎却按「目标缺失 + base 链存在 = 快照时被工具删除」默认删除——恢复到工具批次存档会把工作区其余文件误删（无需用户确认）。现存档 manifest/记录携带 `partial` 标记（`CheckpointBackupExecutor` 写入、`enrichRecord` 回填、`RestoreTargetState` 透传），`computeRestorePlan` 对 partial 目标禁用 deletedInSnapshot 判定：缺失文件一律归 untracked 默认保留，白名单缺省时也不得进入 toDelete（引擎层最强制保证——恢复部分快照绝不删除任何文件）；同时部分快照不再计算 `changes.deleted`（「上一存档有而部分快照没有」只是不在扫描范围内，并非被删除，此前会污染恢复链与后续存档的增量比较）。新增恢复引擎回归测试（partial 目标不误删 / 全量目标旧语义保持 / 恢复执行不删除未记录文件）。
  - 修复部分快照提取与判定的四处缺陷（CP-PARTIAL-1 复查）：① `search_in_files` replace 模式此前取 `args.path` 进入部分快照，但 `args.path` 通常是目录（对整个子树替换），部分快照对非空目录不递归——被替换的文件不进入快照，恢复时既无法还原内容又触发误删判定；现一律返回 null 回退全量（影响面 = pattern × 目录子树，静态不可知，正确性优先）。② 部分快照分支在 stat 之前以 `isDirectory=false` 判定忽略规则，目录型规则（尾斜杠）对受影响路径中的目录不生效；现先 stat 再按真实类型判定（与全量分支 collectEntries 顺序一致）。③ `isPathWithin` 新增 platform 参数可注入（与 `isSameStoragePath` 同策略，Linux CI 可测 win32 大小写折叠）；`workspaceUriToFsPath` 剥离未编码 fragment/query 并拒绝非法编码序列（回退全量）。④ execution.ts 批量路径 after 存档排除被参数错误/策略拒绝/中止未执行的工具（与 orchestrator M3 同一语义），不再多建空批次存档。
...
### Security
  ...
  - 会话写锁挂起超时不再放行并发写（P1）：`ConversationManager.withConversationWriteLock`、分段历史写队列、元数据写链（storage.ts）与用量索引写队列（UsageIndexStore）此前在挂起超时后让队列链继续前进，超时后的底层文件操作仍在后台运行——新写入任务并发启动会互相覆盖/删除同一会话的段文件、索引与元数据（历史损坏）；现队列链尾挂在底层任务（underlying）上：超时只让调用方 fail-fast，链不前进，后续写必须等旧任务真正结束；挂起超时同时改为从任务真正启动时开始计时（排队等待不计入，慢而健康的写入不再被误报）。新增 `writeChainHangTimeout.test.ts`（链不前进 + fail-fast + 正常串行）并更新 `UsageIndexStore.test.ts` 挂起语义用例。
  - 流式响应缓冲加入大小上限（P1）：流式解析无法识别上游格式（非 SSE/JSON）时 `parseStreamBuffer` 把整段缓冲保留为 remaining，代理与原生 fetch 两条分支的缓冲逐轮累积且无上限，异常上游持续吐垃圾数据会造成 Extension Host 内存耗尽；现仅在「解析无进展」时按 64MB 上限终止（PARSE_ERROR，不可重试），合法巨型单事件（多模态 base64 附件等）不受影响。新增 `streamSecurity.test.ts` 超限终止 + 合法巨型事件不误杀两用例。
  - 检查点双文件提交配对一致性（P2）：`files.json` 与 `manifest.json` 两次独立 rename 无法整体原子，崩溃发生在两次提交之间时磁盘会出现「新 files.json + 旧 manifest.json」的混合配对且读取侧无法识别；现每次提交生成随机 `filesRevision` 同时写入两个文件（读取校验配对、混合配对被拒绝），写新 files.json 前旧配对被暂存为 `files.json.prev`（崩溃后读取路径自动恢复 manifest 对应的完整配对，不误报数据丢失）；`files.json.prev` 同步纳入目录遍历/大小统计/恢复路径的元数据跳过清单。新增 CheckpointManifestRepository ATOMIC-PAIR 测试（配对绑定、两崩溃窗口恢复、无备份孤儿拒绝、损坏 files.json 不裸抛、filesRevision 类型守卫）。
  - 子代理无可用工具时不再向模型声明全部工具（P2）：空工具集此前被转成 `undefined`，ChannelManager 把 undefined 当作「未指定覆盖」回退成渠道全量工具声明——模型反复调用不可用工具形成失败循环；现空数组原样透传，各 formatter 只在工具列表非空时注入声明。
  - 流式请求补上渠道配置验证（P2）：`generateStream` 此前跳过 `formatter.validateConfig`（非流式路径有），无效 API Key / URL / 模型配置不提前拦截而表现为网络/鉴权/解析错误并空耗重试；现与非流式路径一致在发起请求前校验（VALIDATION_ERROR 提前失败）。
  - 纯文本上游错误正文不再丢失（P2）：`executeStreamRequest` 原生分支、`proxyFetch.proxyStreamFetch` 无代理分支与非流式 `executeRequest` 此前先 `response.json()`（消费响应体）失败后再读 `response.text()` 只能拿到空串——网关 502 HTML / 纯文本错误体丢失；现统一改为先读 `text()` 再尝试 `JSON.parse`，真实错误正文进入错误信息。新增 `proxyFetchErrorBody.test.ts` 与 `streamSecurity.test.ts` 错误体用例。
  - 多模态流不再被误判为空响应（P2）：流式内容检测 `streamChunkHasContent` 只认 text/functionCall，Gemini `inlineData`/`fileData` 附件流在连接中断时被误判为空响应触发整条流重播（附件重复、重复计费）；现与非流式 `isResponseContentEmpty` 同一口径纳入多模态字段；前端 `messageActions` / `toolActions` / `streamChunkHandlers` 的空消息清理判定同步补齐。新增 `streamSecurity.test.ts` 多模态用例。
  - 子代理请求级上下文裁剪（P2）：子代理未接主链路 ContextTrimService，history 只增不减，长任务直接撞模型上下文上限失败；现发送前按渠道 `maxContextTokens`（缺省 128000）× 0.8 做请求级裁剪——超预算时从最旧开始整轮丢弃（functionCall/functionResponse 配对整体移除不产生孤儿，首条任务消息与末尾两轮保留），仍超预算时对超大字符串截断并标记（结构与 provider JSON 兼容），估算含 1.5× 安全系数并与多模态 part 折算对齐；裁剪不修改 run 内原 history，`lastSentHistory` 同步记录裁剪结果保证续跑前缀一致。新增 `subagentContextTrim.test.ts`（零开销原引用、整轮丢弃保配对、首条/末尾保留、超大文本/工具结果截断且不改原史、预算折算）。
