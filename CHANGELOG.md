...
### Changed
  - 前端非首屏页面、消息列表、17 个设置页签及 33 个工具详情面板改为真实动态导入，注册元数据继续同步可用；生产入口由约 3.46 MB（gzip 999 KB）降至约 1.52 MB（gzip 458 KB），原 682 KB 单体 CSS 拆为约 6 KB 入口样式与按需组件样式。跨 chunk 的消息重试与编辑对话框导入改为直达定义模块，消除 Rollup 执行顺序警告。
