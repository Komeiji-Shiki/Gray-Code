...
### Fixed
  - 修复 DeepSeek 等第三方兼容端点在「发送思考签名」关闭时仍收到 `reasoning` 输入项报 400：`sendHistoryThoughts` 默认 `true` 且 openai-responses 设置 UI 无对应开关（关不掉），此前仅靠它控制普通 reasoning 文本回传，无法关停；现 `allowReasoningContent` 改为 `sendHistoryThoughts && sendHistoryThoughtSignatures` 双开关把关——签名开关关闭时不再构造任何 reasoning item（可见思考摘要降级为普通 assistant 文本保留），开启后两个开关分别控制 content/summary 与 encrypted_content 形态回传。新增回归测试（签名开关关闭 + `sendHistoryThoughts: true` 默认值）。
  - 修复上一条（双开关绑定）引入的 DeepSeek thinking mode 回归：`allowReasoningContent` 被绑定到 `sendHistoryThoughtSignatures`（默认关闭）后，DeepSeek 兼容端点的 plain `reasoning_text` 也会被降级成普通 `output_text`，而 DeepSeek 无状态 Responses 接口要求带 tools 的后续请求把上一轮 reasoning 按 `reasoning` 输入项完整回传（`content[].reasoning_text`），否则报 `HTTP 400: The reasoning_text in the thinking mode must be passed back to the API`（子代理在 thinking 模式下持续触发）。现恢复 `allowReasoningContent = sendHistoryThoughts`（`reasoning_text`/summary 回传由内容开关独立控制），并把上一条想修的「无效 reasoning 输入项」收敛为精准守卫：仅当 part 既无 `reasoning_text` content 又未开启签名回传、只剩 summary 时，才降级为可见文本、不构造 summary-only reasoning item（官方 API 要求 `encrypted_content` 或 `content` 才能恢复推理上下文，第三方端点也可能报「输入项类型 'reasoning' 当前暂不支持」）。DeepSeek content-only 形态不受守卫影响，始终按 `reasoning_text` 完整回传。更新回归测试断言（content-only 在签名关闭时必须回传 reasoning item），新增 summary-only 降级测试。
  ...
  - 修复手动总结在「当前两轮、`keepRecentRounds=2`」时被误判为无需总结：总量超过保留预算时，现把活跃历史按单轮长回合规划，在完整消息序列上寻找安全的 model 切点，不再固定保留内容很少的最后一轮；同时子代理自动上下文压缩改为复用主模型总结服务，使用相同的总结设置、独立总结模型、提示词、流式解析和低质量校验，失败时回退请求级裁剪，续跑仍复用压缩后的 `lastSentHistory`。
  - 修复子代理发送请求前历史只增不减的问题：按完整工具回合边界裁剪旧上下文，保留首条任务与工具调用/响应配对，计入系统提示词和工具声明预算，并在压缩后同步实际发送历史，避免长任务撞上 provider 上下文上限或续跑重新携带已裁剪内容。
