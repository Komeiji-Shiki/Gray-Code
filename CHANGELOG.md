...
### Changed
  - 工具执行存档改为按「模型传入参数涉及的文件」构建部分快照（不再全量扫描工作区，CP-PARTIAL-1）：新增 `extractAffectedPaths` / `workspaceUriToFsPath`（`checkpoint/affectedPaths.ts`），对 write_file / apply_diff / insert_code / delete_code / delete_file / search_in_files(replace) / create_directory 白名单工具提取 `args.path`（相对路径 resolve 到工作区根、绝对路径原样、路径穿越防御回退全量）；`buildWorkspaceSnapshot` 新增 `affectedPaths` 分支——只对列出的路径 stat + 哈希（忽略规则/大小上限/previous 复用/空目录/多根归属/ENOENT 不可读语义与全量分支一致），不再递归扫描整个工作区；ToolIterationLoopService（批次 before/after 与确认路径）、execution.ts（单工具/批量路径）、orchestrator（确认路径补 after）在创建检查点时收集受影响路径透传，批内任一工具无法确定（execute_command 等副作用不可知）→ 整批回退全量保证快照完整性；手动存档 / user/model 消息存档保持全量不变。新增 `affectedPaths.test.ts` 18 用例（提取单测 / 部分快照分支 / 链路透传与回退）。
