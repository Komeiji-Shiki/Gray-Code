...
### Added
  - 附件预览升级为图片查看器：同一条消息的多张图片打包成图片组一次注册，可在查看器内左右切换、缩放（滚轮/按钮，以光标为锚点）、平移与旋转，并显示序号与缩放百分比；超出媒体通道预算时按「离当前图片最远优先」裁剪组，关闭时组内资源一次回收（VS Code 扩展与单图场景保持原有单图预览）。

### Changed
  - 记忆扫描按文件大小分配并复用缓冲区，4 条记忆连续读取 50 次的合成对照中，累计缓冲区申请量从 1,600.13 MiB 降至 0.91 MiB；定宽记录短读、短写统一按实际完成字节数处理。
  - 搜索消费后立即释放预取全文，避免已扫描文件一直驻留到结束；同机 256 × 128 KiB 合成夹具的采样峰值堆内存从 42.34 MiB 降至 5.40 MiB，原有匹配顺序、预算与 8 路预取保持不变。
  - 运行记录按序到达时直接追加，不再每次复制、去重和排序整份历史；补读和重复事件仍保持原有合并规则。
  - 完善暗色平面界面：文件树增加选中提示和键盘焦点，历史加载入口区分空闲与加载状态，附件使用直角边框并提供完整文件名提示。
  - 永久记忆容量放宽：LOG 固定宽度记录 `LOG_REC` 1024 → 4096 字节，`entryChars` 配置上限从 1000 提升到 4072（`memory_config` 可设更大条目，1280 等旧上限拒绝的值现在可用）；树摘要记录 `TREE_REC` 288 → 1024 字节，单行摘要上限从 287 提升到 1023（`compress` 预算取 `min(entryChars, 1023)`）。旧数据自动无损迁移：LOG 按宽度阶梯探测（320 / 1024 → 4096，歧义尺寸靠内容判别，`id` 连续 + ISO 日期全量校验，tmp+rename 原子替换、幂等且 fail-open），TREE 目录的 288 字节记录在首次访问时无损迁移；迁移失败或错位的旧宽度内容在读取时被判为无效摘要（视为空槽触发重建），不会把拼接乱码当摘要展示。设置页输入框的 min/max 改由 `getMemoryConfig` 下发 `bounds`（单一来源为 `MEMORY_CONFIG_BOUNDS`），不再前端硬编码上限。
  - 永久记忆压缩提示更节制：`memory_note` 只在待压缩状态（待压缩块数 + 最老块）变化时返回 `pendingCompression`，同一状态不重复推送同一段含记忆正文的提示；`memory_wake`（会话入口）与 `memory_compress` 仍总是返回下一步提示，但会记录签名，使紧随其后的 `memory_note` 不重复提示。系统提示词、工具描述与 `memory_compress` 摘要上限说明同步更新（含三语设置页默认文案）。
  - `find_files` / `search_in_files` 在大工作区下显著提速：目录遍历只解析一次根目录、对默认排除模式与常见 `**/*.ext` 类模式改用字面量快速匹配，个别子目录消失（ENOENT）只跳过该子树而不再让整个查找失败；搜索改为受控并发预读文件、并在逐行匹配前对零命中文件做全文快速拒绝，结果的顺序、上下文、预算截断与跳过原因语义保持不变。

### Fixed
  - 修复工具准备期间权限被撤销后仍执行的问题，Bot 异步路由读取后使用当前账号授权；群聊子代理只继承根频道工作区和群聊记忆范围，避免回退到个人记忆。
  - 修复 MCP 读取配置期间断开仍启动连接、ACP 读取会话期间关闭仍启动操作，以及恢复失败后复用未恢复连接的问题；统一关闭信号和连接登记，减少重复状态。
  - 修复异步文件和附件读入错误草稿、配置与提问结果迟到覆盖新会话、新建会话绑定到另一个空白标签的问题；删除解析失败后猜同名文件的回退，合并发送清理与配置恢复路径。
  - 修复记忆初始化覆盖并发创建的文件、迁移和删除重写遇到部分读写时丢失记录的问题，临时文件创建失败时及时关闭原文件句柄；移除配置和 diff 替换失败后先删除旧文件的危险兜底，配置读取错误正常报告。
  - 修复输入框整数行高缓存与盒模型计算导致第二行被裁剪，按当前样式完整计算正文、内边距和边框高度。
  - 修复历史读取自愈可能覆盖正在执行任务的新结果：SQLite 写回使用现有事务检查历史版本和活跃任务，冲突时读取最新历史；活跃状态查询失败不再补写拒绝响应。
  - 修复异步工具准备期间取消后仍执行工具，以及关闭运行器时尚在准备的任务继续启动；等待会话空闲时实际执行退避，自动标题通过元数据版本校验保护手动改名，暂时失败的历史标题补齐可在下次启动重试。
  - 修复 Responses 流式 `item_id` 覆盖 `call_id`、并行参数关联错误和最终工具项遗漏；多段思考摘要按段合并，中断时保留已经显示的前段内容。
  - 修复通用历史过滤误删 Responses 密文、Anthropic 独立签名与加密思考、Gemini Interactions 仅签名思考；Anthropic 按原顺序恢复思考块，关闭 Responses 明文回传时不再补空内容。
  - 修复长历史上翻时跳过本地消息或丢失阅读锚点，支持定位到中段后继续向下跨页阅读；连续定位保留最新目标，已保存密钥的迟到显示结果不再覆盖正在编辑的输入。
  - 修复目录读取期间收起、移动、删除后的迟到回写，以及浏览器关闭或初始化失败后残留原生页面；面板卸载停止补读，过期附件预览按组释放资源。
  - 修复独立宿主新建对话标题一直停在「新对话」：桌面/Web 先在 `ui.mode.new` / `conversation.createConversation` 以占位标题落库，首条消息不再经过「以消息创建并命名对话」的旧流程，标题从此不再变化；现在首个回合写入历史前把仍是占位（或空）标题的空对话替换为消息摘要（压缩空白、限长 48 字，与旧 RPC 命名规则一致）并广播元数据变更，启动时一次性补齐历史遗留的占位标题（按首条真实用户消息命名，跳过工具响应/总结/后台回执）。
  - 修复同一工具调用出现重复结果导致对话中断（`API_ERROR: Unpaired tool history: duplicate_function_response_id`）：读取路径会把"结果尚未落盘"的活跃任务调用误判为已放弃并写入「用户拒绝」占位，真实结果随后的落盘就成了同一 `functionCall` ID 的第二条 `functionResponse`——下一次请求被完整性校验拦下，重试也不会好。现在①读取路径检出重复响应并就地自愈（同一 ID 只保留真实结果、清除被取代的拒绝占位与残留 rejected 标记、修正消息 parentId 链）；②悬空调用补齐跳过仍活跃任务写下的在途调用，不再给迟到结果制造重复；③发送请求前在副本上同样清理，存量脏会话可以直接继续对话。
  - 修复 AI 把使用时间统计读成一段连续工作（例如「今天从 01:31 忙到现在」，实际中途睡过觉）：`get_activity_stats` 现在按天返回会话时段明细（最近 7 天，形如 `10:00-10:02 (2m)`）与最近一天各时段之间的空档，工具描述说明「15 分钟无活动即断开会话、空档代表用户离开编辑器」，不再让模型只凭首次/末次活跃推断连续工作时长；`currentSession.startedAt` 补日期以区分跨午夜会话，工具卡片同步显示今日工作时段（单日时段超过 24 段时给出省略条数）。
  - 修复长对话楼层号错误：原先只对当前已加载的消息编号，分页窗口上翻历史时同一条消息在不同窗口显示不同楼层，且工具响应被当作独立消息占了楼层；现在由后端一次全局扫描给出绝对楼层索引，前端按索引对齐，工具响应不占楼。
...
Windows x64 独立桌面版。GitHub Release 按常规版本发布；版本号沿用 2.0 预览阶段的命名。

### Added
  - 便携版可同步显式放入 `portable-data/memory-storage` 的导入记忆资料库，跨电脑合并修订、删除与召回开关；未经复核的资料默认不启用自动召回。
  - 桌面文件树、项目和对话菜单可在系统资源管理器中打开所属工作区或定位文件。
  - 普通对话、项目对话、项目分组、置顶项与草稿支持拖动调整显示顺序，并在重启后保留；拖动不改变对话所属项目。

  - 电脑和内置浏览器支持按截图像素坐标操作，动作后返回新观察；浏览器保留同一页面的后台绘制与登录会话。
  - 新增 ACP 编码代理配置与 coding_agent 工具，支持创建、追加提示、恢复、派生、模式/配置、具体权限选项、取消和关闭；使用稳定 SDK 1.5.0。
  - 运行记录显示实际请求图片数、工具数、流式合并次数和活动资源；远程设备声明截图坐标版本与支持动作。
  - Git 工作树列表可以在所选目录中新建编码任务，原有会话继续使用原工作区。

### Changed
  - 后续 GrayCode 代码迁移至 AGPL-3.0-only，保留历史 MIT 及第三方声明，并为指定 Cubism 组件提供限定组合许可；新增贡献来源约定。
  - 桌面打包生成并携带与程序提交一致的源码归档、SHA-256 和完整许可；桌面/Web 应用信息与 Bot 指令提供对应版本源码入口。
  - 新建普通对话自动创建专用工作区，不再继承此前代码模式所选的项目目录。
  - 动态上下文未变化时省略仅包含提示说明的空条目；普通桌面对话不再额外插入通用的认证后任务身份消息。
  - 统一桌面开关的方形外框与圆形滑块，并修正开启时悬停边框被背景遮挡的问题。
  - 移除“最近 N 张图片”的限制和设置，新增图片时完整保留已有历史图片与请求前缀。
  - 文件树按视口渲染并合并父目录刷新，活跃历史采用增量读取，精确向量检索移至独立工作线程；同批独立读取最多四个并行，流式文本合并推送。
  - 模型请求快照无损共享相同消息与工具对象，存储格式升级至 7；旧数据正常读取，旧客户端不能读取新版引用格式，降级需使用匹配的数据备份。
  - 官方 MCP SDK 2.0.0 统一新旧协议、分页发现、目录通知与连接生命周期，删除重复的私有协议实现；需要输入和未知执行状态明确保留。
  - 常用设置、文件、运行、电脑和浏览器 RPC 使用共享类型与入口字段校验；正式桌面打包使用完整构建并携带 ACP/MCP 原始许可证。
  - 重写独立平台 README、架构和贡献指南，新增包含合成界面截图的仓库 Wiki。
  - 未单独适配的工具与 MCP 使用统一结构化结果面板，支持字段、链接、图片、状态和分页列表；长文本与原始数据按需展开，窄屏嵌套字段自动调整排版。
  - 项目搜索按需遍历文件，复用草稿快照和查询表达式，仅为命中的内容计算行号与校验值；取消搜索在最后一次文件读取期间也能生效。
  - Windows 便携版自动在程序旁保存加密用户配置，移动程序时携带配置与密钥，聊天和工作区数据继续保存在本机；打包时保护已有便携配置。
  - 网页观察默认使用精简文本节点并限制单次返回数量，可按元素引用聚焦区域或请求完整字段；控制台和网络日志支持游标增量读取。
  - 电脑观察精简重复状态与内部标识，保留元素、进程和坐标信息，支持按需读取完整字段；电脑与网页截图默认最长边 1280 像素，可提高到 2560 像素。

### Fixed
  - 流式 API 报错或取消时保存已经显示的正文和思考；关闭错误提示不再删除回复，重试保护已保存的部分历史，后续请求过滤未完成的思考签名与工具调用。
  - 聊天错误、统计加载错误和工作台错误提示支持一键复制，并显示复制成功或失败状态。
  - 用量统计分页批量读取会话信息与版本，修复超过 128 个会话时缓存反复淘汰导致的历史重读；刷新保留已有结果，切换时间范围时清除旧范围数据。
  - 桌面沙箱预加载将纯 RPC 契约打入自身，避免外部 require 导致工作台桥接未建立；构建时检查预加载外部依赖。
  - 浏览器截图等待新绘制帧，避免滚动请求完成后返回操作前的画面和坐标。
  - 修复 OpenAI Chat Completions 函数调用结果中的图片丢失，同批工具响应保持完整配对。
  - 工具参数校验器复用并回收失效目录缓存，事件订阅者异常不再阻止其他订阅者收到事件。
  - 受管进程退出有界等待，完成命令释放活动句柄并保留结果；事件流对过慢接收端结束已接受的完整帧后重置。
  - 旧设置读取和过期目录展开结果不再覆盖新工作区；连续权限请求保留独立审批 ID，迟到的旧回复不影响新确认。
  - 修复翻译缓存命中后部分工具标签不随界面语言切换刷新的问题。
  - 预设单独导入支持旧版模式映射及完整设置导出中的提示词配置；无效 JSON 内容明确报错，避免静默生成默认预设。
  - 电脑与网页操控统一遵守自动执行页的勾选状态，旧配置缺少新增工具键时也正确继承；保留取消勾选、Agent 单独审批规则与账号权限检查。
  - 独立桌面完整导入旧设置后立即刷新提示词预设列表；导入的自定义预设在统一草稿、保存和重启后保持可用，当前预设继续沿用导入文件中的原值，已有对话不被隐式改写。

## [2.0.0-pre.2] - 2026-09-15

这是 GrayCode 独立桌面的 2.0 增量预览版，提供 Windows x64 安装器与便携包。1.x VS Code 扩展仍使用既有发布渠道；本预览版不发布 VSIX。Linux/macOS 的发行与验证尚未完成。

### Added
  - OneBot 新增待发送消息入口，与 Discord 共用队列面板；可查看连接中断后的消息、原因与发送进度，明确确认后重试未确认送达的回复。读取失败可刷新恢复，重试确认不会改动设置草稿。
  - 新增 Windows 安装、应用内更新、独立修复和数据回退流程；安装器使用固定的 Velopack 版本生成更新清单，并校验桌面包与源码版本一致。
  - 新增程序数据一致性备份、资源清单校验、选择性恢复与恢复预览；设置、历史、记忆、工作区绑定和运行数据可按类别处理，恢复失败保留原数据。
  - 新增事件驱动自动任务、长期目标持续执行和定时任务管理，支持持久化运行状态、用量统计、失败恢复与通知策略。
  - 新增分层长期记忆、来源管理、后台整理、删除与恢复；记忆查询、修改和归档通过统一存储与权限路径执行。
  - 新增 Windows 电脑控制、屏幕输入与人工接管，以及具备安全配对、账号权限和工作区边界的远端执行节点。
  - 新增桌宠、Live2D、屏幕感知与陪伴配置；普通对话、角色剧情和个人记忆使用独立配置边界，并支持统一免打扰时段。
  - 新增 DAP 调试、项目搜索与大纲、跨文件批量撤销、Git 暂存分支和工作树操作，以及多种内置或外部语言服务。
  - 语言服务与 Windows 文件关联集中到独立的“开发工具”设置，设置搜索补齐 Bot、账号授权、工作区和远程连接入口，并将旧提示词关键词定位到当前预设条目。
  - 独立编辑器新增 Python、Vue、Svelte、HTML、CSS、JSON/JSONC、YAML 和 Shell 内置语言服务，Vue 与 TypeScript 协作提供脚本补全、类型诊断和代码操作。语言识别、未保存内容、服务生命周期和诊断统一处理，并提供 Go、Rust、C/C++、Java、C#、PHP、Lua、PowerShell、SQL 与 Markdown 的系统服务检测和配置入口。
  - 开发设置按语言展示内置、待安装和停用状态，支持重新检测、安装说明、逐项编辑启动程序与参数，并可从设置搜索直接定位。检测保留未保存的表单，自定义配置继续使用统一保存和撤销。
  - 左侧项目支持右键重命名和移除，可选择同时清理关联旧对话，默认保留历史及磁盘文件。旧对话生成的项目同样可管理，重新添加目录可以恢复项目显示；Discord / QQ 会话及其置顶项独立放入“机器人会话”。
  - “账号与授权”新增未绑定用户默认权限、指定用户独立授权和黑名单，MCP 工具可逐项配置。工作区默认限定当前机器人会话的专用目录，操作权限、工作区和 MCP 采用可搜索、多项勾选的选择器；默认访客不会共享运行身份，拉黑和撤销在后续执行及回复发送时重新核对。

### Fixed
  - 输入区图片预览支持 Tab 和 Enter 操作；桌面预览窗口接管焦点、限制 Tab 范围并支持 Esc 关闭，关闭后返回聊天页面中原来的附件按钮。
  - 桌面 IPC 信任校验改用 Electron 的稳定框架标识识别应用主页面，继续拒绝未登记窗口、子框架、已分离框架和非应用页面，避免同一底层框架的不同包装对象被误判。
  - MCP 工具取消与超时会向 stdio 和当前 HTTP 握手协议发送对应请求的取消通知，初始化及未发出的请求除外。修正 HTTP 握手版本，并拒绝尚未实现的协议版本，避免错误显示连接成功。
  - MCP 列表读取失败显示原因并可重试，迟到的查询不会覆盖新列表；未完成的服务器表单在设置分类切换后保留，撤销全部仍可清除。补齐表单无障碍名称，保存期间防止继续修改后丢失输入。
  - OneBot 连接状态实时显示断线、重连和机器人身份；Discord 与 OneBot 共用状态通知，迟到的查询不再覆盖新状态。新增用户绑定标记未保存并聚焦用户 ID，连接前检查本地草稿，连接配置适配手机宽度。
  - OneBot 回复保留触发消息的来源关系，并以真实发送回执更新待发送状态；机器人正文、来源显示和可读时间在桌面分页与模型历史中保持一致。
  - 首次使用的输入区按实际渠道和模型状态提示下一步，保留输入草稿并进入现有配置页；渠道加载和创建失败会显示原因并允许重试，创建期间避免重复提交，迟到的加载结果不再覆盖新结果。
  - Windows 打包按依赖方的实际安装位置收集生产依赖，保留同名包的嵌套版本，修复新增语言服务缺少依赖而无法打包的问题。
  - Windows 打包在生成前检查完整页面入口、原生电脑控制宿主和发行依赖，避免产生缺少聊天页面、嵌套依赖或原生组件的安装包。
  - 语言服务退出等待进程和管道关闭，Windows 直接结束已确认的服务进程树；辅助语言服务随主服务一起释放。HTML、CSS 和 JSON 服务显式启用格式化，避免显示服务已启动却无法格式化文件。
  - 手动检查点在内容未变化时显示明确状态并跳过冗余节点；恢复入口继续保护未保存文件、排除项和增量链一致性。
  - Bot 消息正文不再重复包含平台消息 ID 和作者 ID，时间转换为带时区的可读格式；旧记录在模型历史和桌面分页显示中使用同一转换，原始来源仍保留在内部记录。
  - 独立桌面与 Bot 的 MCP 工具执行统一遵守已有自动执行配置，修复设置页默认允许自动执行、实际调用却重复要求确认的问题；逐工具确认规则与账号权限继续生效。
  - 独立平台的自动总结方式可在每个模型渠道中分别选择普通总结或笔记换窗口，手动总结保留全局设置；修复自动入口只有旧模式选项的问题。渠道切换协议、导入导出及重启保留选择，运行中的回合保持原方式，手动换到笔记窗口后仍可恢复历史。

## [2.0.0-pre] - 2026-09-10

这是 GrayCode 独立桌面的首个 2.0 预览版，提供 Windows x64 便携包。1.x VS Code 扩展仍使用既有发布渠道；本预览版不发布 VSIX。Linux/macOS 的发行与验证尚未完成。

### Added
  - 新增共享团队任务与依赖：原子领取就绪任务、负责人和执行轮次校验、持久事件游标与等待唤醒；等待中的子代理释放并发席位，同毫秒消息按持久序号交付。
  - 常规上下文管理支持完整请求前缀总结和持久笔记换窗口；机器人另保留按时间总结。原历史保留，可通过当前会话工具分段恢复。
  - Discord 补齐背景消息记录、连续消息合并、嵌套引用及图片；TXT/MD 原文件本地保存后通过工具分段读取。流式回复保留工具前正文，展示统计不回注模型。
  - 新增可选 Web 远程工作台、Discord 与 OneBot 接入、账号权限管理、语言服务、原生终端与文件审查。
  - 独立桌面接入原 MCP 客户端与工具适配器，支持统一草稿中的服务器表单和 JSON 编辑、连接与发现、任务工具调用及取消，环境变量与请求头加密存储。工具返回的二进制内容通过附件保存，避免丢失或作为大段 Base64 文本重复发送。
  - 统一使用预设条目与原位保留的动态上下文，自动转换传统模板并保存迁移前配置。提取可供独立宿主复用的提示词装配、文件树和公共上下文段落；桌面任务开始时解析所选预设，保存上下文快照并在工具迭代中复用。
  - 新增独立 Electron 桌面宿主、Monaco 工作台、原生终端、Git 操作、内置 HTML 预览及系统密钥保护。聊天与设置复用原前端，通过宿主接口连接共享应用服务；保留顶部对话标签页和完整输入区，支持左侧对话、全宽显示、蓝色主题与系统字体选择。
  - 新增跨设置分类的统一草稿与修订校验、Discord Gateway 接入和身份绑定面板，以及原界面与独立任务流的适配。模型连接复用现有五类协议格式化器；Discord 运行测试使用隔离的测试端口，未连接真实账号。
  - 独立角色对话接入角色卡、世界书、随卡正则及沙盒 HTML/CSS 显示。
  - 独立核心新增任务运行循环、持久化状态与顺序事件、请求去重、账号权限检查和逐工具审批。普通成员不能执行私有工具或取消主人的任务，权限拒绝保留工具调用/结果配对与稳定声明；重启后将未完成操作标为中断，不自动重放副作用。
  - 新增 `ask_user` 异步询问工具核心，支持建议选项与自由输入、等待期间继续独立工作、超时提示和工具批次完成后的反馈注入。该工具不承担操作审批，用户沉默不会扩大权限。
  - 启动独立平台重构：新增 `packages/contracts`、`packages/core` 与 Node CLI，建立不依赖 VS Code/Electron 的 SQLite 存储 worker。历史正文使用 MessagePack、Zstandard、内容去重和二进制附件引用，支持分页、事务、修订校验、共享历史分叉与快照、完整性检查及垃圾回收。
  - 新增旧 JSON/分段 NDJSON 历史的只读导入器，支持批量恢复、源内容核对和未完成数据隔离，并复制可识别的分支、检查点、用量和附件；未处理的来源明确报告，旧数据保持只读。
  - 平台工作区检查点新增删除与保留期清理：单个删除、按 `maxCheckpoints` 保留最近自动清理，仍被当前历史引用的检查点拒绝删除（分支文件恢复依赖），强制删除显式确认；创建成功后自动触发一次清理，并接通桌面与远端两套调用入口。
  - 独立宿主新增 MCP 资源与提示模板只读选择入口：按服务器列出可用资源与模板、读取资源内容、按参数获取模板消息，均复用已连接的管理器，不额外启动进程，未连接返回具体状态与错误；取消经中止信号透传。
  - 独立宿主新增上下文只读统计展示：已用 Token（供应商精确计数优先、总结估算次之，本地估算只标为估算）、上下文上限与使用率、可见与已总结消息范围、已保存总结明细；不推进裁剪状态、不发起总结。
  - 旧历史导入补上附属阶段接线：分支图、用量、Diff 与附件随历史导入一并识别（用量等无需转换器即落盘，分支与检查点由独立宿主注入转换器），导入报告新增已导入附属统计；旧用量文件不再误报为未处理项。
  - 独立宿主注入旧检查点与旧分支图转换器：备份目录字节转为内容记录与自包含清单（多根、增量链回溯、局部范围、排除规则保留，源目录只读），分支拓扑与完整正文按新版分离口径重建并可用只读分支视图重现活跃路径。
  - 独立桌面新增 MCP 资源与提示模板浏览面板：已连接服务器下展开资源列表并查看内容、模板填参数预览消息，失败显示原文；扩展宿主缺对应处理器时明确提示仅桌面版可用。
  - 对话输入区用量旁新增上下文详情入口：已用 Token、上限、使用率、可见与已总结范围、已保存总结明细只读展示，不触发总结。
  - 新增平台存储与现有会话管理器的集成适配、18 个平台测试及可复现合成基准。开发与 CI 环境调整为 Node 22，平台源文件和构建产物从旧 VSIX 包中排除。

### Fixed
  - 平台工作区检查点恢复入口统一拦截未保存的编辑器内容：恢复前做工作区级脏检查，不再只依赖事务内逐文件拦截，清单外脏文件也会阻止恢复；分支切换文件恢复经同一入口自动受益，脏时抛错且历史与磁盘保持不变。
  - 独立宿主在等待审批时保留运行事件订阅，避免其他入口处理审批后界面仍显示等待；保留逐条工具结果的真实消息 ID 与索引，正确插入异步回答。
  - 纯设置服务直接引用存档排除配置和记忆工具名称，消除通过聚合入口间接引入 VS Code 的依赖；补全已有 TPS 显示设置的类型声明。
  - 调整会话查询服务的构造顺序，避免在原生 ES2022 类字段语义下读取尚未赋值的存储参数；会话与分支仓储直接引用独立写入队列，移除 Diff 存储中未使用的 VS Code 导入。

## [1.5.6] - 2026-09-05

### Added
  - 前端新增 GrayCode 视觉系统：以 VS Code 主题变量为源建立统一的间距、字号、圆角、语义颜色、动效与浮层令牌，提供低优先级 `.gc-*` 基础样式和可访问的 `CustomSwitch`，公共按钮、复选框、下拉框、弹窗、提示框及高频聊天/设置界面开始统一使用该规范。
  - 前端新增公共 `Modal` 组件，承接确认/删除/重试/输入/编辑/公告/更新/模型选择/渠道创建/存档删除确认/存档清单/提示词导入等 18 个弹窗的遮罩、焦点陷阱、焦点归还、Escape 与滚动锁；迁移后弹窗按钮统一为 `.gc-button` 公共样式，Escape 与遮罩点击关闭行为有独立开关。
...
### Changed
  - 调整暗色平面对话布局：收紧用户消息与工具卡片、模型名周围的间距，用户时间显示在楼层号右侧，输入区降低初始高度并保留自动增高。
  - 优化输入区、消息与工具卡片的布局：输入区共用外框与焦点反馈，渠道和模型选择栏按面板宽度分配空间，窄面板下模型选择独占一行；用户消息采用轻量卡片，统计信息支持换行，工具卡片通过状态侧边线区分运行、待确认与失败，并改善标题、路径描述和操作按钮的对齐。
  - Markdown 渲染缓存按键与渲染结果的合计占用设置上限，避免长文本和长对话持续积累缓存。
  - 用量统计在索引缺失或过期时直接进入队列内重建，成功路径只读取一次完整历史；仍在索引写队列内读取最新消息，保留子代理用量与失败时的历史回退。
  - 用户在前台命令或子代理仍运行时发送新消息，实际被转入后台的工作数量现在随该条真实用户消息持久化；后端以固定、不可由 Webview 注入自由文本的模板在每次 API 历史构造时把提醒放到用户原文之前，明确说明旧工作未取消且完成或失败会以 `[Background task completed]` 自动回报。提醒不显示在聊天气泡中，但会稳定参与工具续跑、重试、后续回合、分支往返与 token 预算，避免一次性提示在缓存或回合切换后丢失。
  - 永久记忆压缩改为不阻塞当前交付：成功的 `memory_note` / `memory_wake` 返回 `pendingCompression` 时明确标记为可延后维护，模型先完成用户任务；仅当 `memory_wake` 因所需树摘要缺失而失败时，压缩才是继续唤醒前的必要步骤。系统提示、工具声明、中英文工具说明及中英日设置页默认文案同步采用这一语义，并收紧长期记忆记录范围，默认排除临时进度、工作日志、可重建内容、凭据、秘密和未经用户要求的敏感个人信息；压缩提示新增 `required` 元数据以区分维护与阻塞场景。
  - 优化内置 Code 模式默认提示词：明确要求把互不依赖的同类或不同工具调用放进同一响应，也允许一次多文件改动发出多个不重叠的 `apply_diff` 调用；依赖结果、共享可变状态或存在风险的调用仍保持顺序执行。提示词同时补齐基于仓库证据决策、保护已有工作区改动、按风险验证、自主完成任务、仅顺手修复清晰且局部低风险的小缺陷，以及直接精确写作等边界。Code 与动态上下文默认模板移到 `shared/defaultPromptTemplates.ts` 供后端运行时和前端“恢复默认”共用，历史导出 `DEFAULT_SYSTEM_PROMPT_TEMPLATE` 改为同一模板别名，并修复后端新建默认配置遗漏 `{{$SKILLS}}`、前后端默认值漂移的问题。`SummarizeService.ts` 的详细总结指令继续保留用户决定、被否决方案及原因、困难与解决方式、当前状态和下一步，并对用户原话与模型推理采用差异化压缩；新增前后端回归测试固定这些行为。
  - 前端非首屏页面、消息列表、17 个设置页签及 33 个工具详情面板改为真实动态导入，注册元数据继续同步可用；生产入口由约 3.46 MB（gzip 999 KB）降至约 1.52 MB（gzip 458 KB），原约 682 KB 单体 CSS 拆为约 302 KB 首屏入口样式与 44 份按需组件样式。跨 chunk 的消息重试与编辑对话框导入改为直达定义模块，消除 Rollup 执行顺序警告。
  - 输入框撤销/重做改为独立历史状态机：连续输入或删除在 750ms 内合并为单步，粘贴、剪切、结构修改与 IME 合成保持原子事务；接入 `beforeinput` 菜单撤销/重做，chip 按钮删除可撤销，重复终态事件不再产生空步骤，并避免每个快照深拷贝大段上下文正文。
  ...
  - 16 个旧弹窗统一迁移到公共 `Modal`（遮罩/焦点陷阱/Escape/滚动锁/焦点归还单点实现），并新增公共 `.gc-*` 视觉原语：图标按钮、链接按钮、选择卡片、字段、徽标与消息反馈在弹窗、模型选择、上下文标签、附件预览及部分设置控件中统一应用；旧 `rgba(var(--vscode-*))` 失效色改为 `color-mix()`。
  - 点击语义全面转向原生控件：上下文标签、附件预览、模型选择卡、工具结果文件名/路径、子代理预设卡改为原生 `<button>` 并补齐可访问名称；模型管理器拆出独立「选择/删除」按钮；隐藏操作按钮增加 `focus-within`，键盘聚焦时可见。
  - 复制与打开失败反馈：工具面板与消息区的 9 处复制调用统一改走 `copyToClipboard`（含 textarea + execCommand 回退），失败与打开失败经 VS Code 非阻塞通知反馈；`ModelManager` 的 7 处 `alert()` 全部替换为通知。
  - 公共 `CustomSelect` 的选项面板改为 `Teleport` 挂到 `body` 并按触发器视口矩形定位：面板原先以 `position: absolute` 贴在触发器下方，会被 `Modal` 的 `modal-body`（`overflow-y: auto`）裁切，新建渠道对话框里接口类型下拉展开后几乎选不到任何项。现在展开方向由视口剩余空间自动判定（下方放不下且上方更宽裕时朝上展开，`drop-up` 参数保留为强制指定），选项列表最大高度按可用空间压缩，触发器被滚出视口时自动收起；宿主容器滚动（捕获阶段）与窗口缩放实时跟随重定位，内容自适应宽度模式下还会把右侧溢出拉回视口内。面板内搜索框同样接管方向键/Enter/Esc 键盘导航；`Esc` 改为停止冒泡，不再连带关闭承载它的对话框。长文案场景新增 `dropdown-fit-content` 开关（面板按内容取宽、选项文案换行），`ModeSelectorBar` 原先靠 `:deep(.select-dropdown)` 撑宽的写法迁入该开关；浮层层级改为 `--gc-layer-modal + 10`，确保挂在 `body` 上时仍压在对话框之上、通知层之下。新增 9 例定位与事件边界回归测试。

### Fixed
  - 修复 MCP 请求与响应的协议边界、工具列表分页以及多类型工具内容转发问题。
  - 修复多渠道请求选项和结束响应处理的兼容性问题，并统一工具调用参数的协议校验。
  - 修复附件异步上传期间切换会话或标签页导致的附件归属混乱。
  - 修复发送请求在切换标签后迟到失败时误清当前标签流式状态的问题：错误、占位消息、加载状态及渠道/模型覆盖只在原会话收尾，已关闭标签或同会话的新流不会被旧请求覆盖。正文和附件也恢复到原标签快照；后台已接受的发送按成功处理，避免恢复重复草稿。等待工具拒绝期间切换标签时，不再向新标签发送旧正文。
  - 修复用量索引重建与主消息增量追加交错造成的重复计数：增量追加按主消息稳定 ID 幂等处理，同批重复 ID 也只计一次；无 ID 的旧消息仍逐条保留。
  - 修复活动统计读取将临时 I/O 或权限错误误当内容损坏并删除有效文件的问题：只有成功读取后的 JSON/结构损坏才沿用清理行为，`EIO` / `EACCES` / `EPERM` 等读取错误保留原文件且不缓存空结果，后续可重试。
  - 修复运行中迁移与会话、活动统计等旧路径写入实例交错导致的数据丢失：界面先持久化迁移或恢复默认路径的意图，当前运行保持原路径，重载后由 bootstrap 在数据服务创建之前执行迁移；失败保留迁移意图并报告初始化错误。中英日提示同步说明重载前结束任务、关闭其他 GrayCode 窗口，避免跨窗口并发写入。
  - 修复迁移遗漏数据目录根部 `branches.config.json`、导致分支保留期恢复默认值的问题：根部配置纳入复制、恢复、清理、统计及目标目录冲突检查，覆盖独立与嵌套路径往返迁移。
  ...
  - 修复重试消息对话框（RetryDialog）在迁移到公共 `Modal` 与 `.gc-*` 视觉原语后观感退化的问题：检查点提示改回编辑器信息背景色块（无边框），底部按钮恢复旧样式——取消按钮透明底 + `--vscode-panel-border` 边框、回档按钮信息蓝底白字、重试按钮主色底；保留 Modal 骨架与按钮原生语义。
  - 修复设置导入后界面不刷新、必须重启插件才能看到刚导入渠道配置的问题：导出与导入本身一直接收渠道配置（`SettingsExporter.collectExportData` 走 `configManager.listConfigs()`，导入写回同一个 `ConfigManager` 实例），但 webview 侧的渠道/MCP/设置视图全部是「挂载时拉一次 + 模块级缓存」的快照——`channels.configChanged` 推送此前只在设置页单次编辑（create/update/delete/models）时发出，`settings.import` 完成后一句通知都没有；而渠道设置页挂载时命中 `channelConfigCache` 就直接复用旧列表，因此连切走再切回页签都刷新不出来，只有重启插件（重置模块缓存）才生效。现新建 `webview/utils/configChangeNotifier.ts` 收敛刷新通知（从 `ConfigHandlers` 抽出 `notifyChannelsChanged`，不复制一份实现），导入成功后按实际变更的域广播：渠道走 `channels.configChanged`（不带 configId 表示外部批量变更），MCP 走新增的 `mcp.configChanged`，VSCode 设置走新增的 `settings.imported`；无变更与用户取消/报错时不广播（渠道重拉是 listConfigs + N×getConfig，不做无谓全量请求）。前端 `ChannelSettings` 订阅该命令并只处理批量变更（自身单次编辑已就地刷新，避开重复请求），重拉后复用抽出的 `syncSelectedConfigId` 修正选中态并刷新 chatStore 渠道快照；`McpSettings` 与 `SettingsPanel` 同步订阅对应命令。新增后端 8 例（按域选择广播、取消/报错/无变更不广播、响应先于推送）与前端 7 例（批量变更重拉与缓存更新、带 configId 不重拉、卸载取消订阅）。
  - 修复全局基础重置反过来压制 `.gc-*` 视觉原语、导致直接套用原语的控件再次退化的问题：`main.ts` 的样式顺序为 `tokens.css` → `primitives.css` → `style.css`，而 `primitives.css` 刻意用 `:where()` 把原语写成零特异性（便于组件 scoped 样式细化），于是最后加载的 `style.css` 中 `button { border: none; background: none; color: inherit }` 与通配符 `* { padding: 0 }` 把原语的边框、底色、文字色和内边距全部抹掉——表现为新建渠道对话框底部「取消/创建」退化成纯文字（无按钮感），以及配置名称输入框的 placeholder 贴到 `:focus-visible` 描边上。现把 `style.css` 里的基础重置（`*`、`button`、`button:disabled`、`input/textarea`）降级进 `@layer gc-reset`：未分层样式恒高于任何分层样式，重置继续兜住裸元素，但不再压制带 class 的原语；`body`、`#app`、滚动条、`::selection`、`:focus-visible` 保持未分层，不改变既有焦点描边与滚动条观感，组件 scoped 样式与原语之间的竞争关系也维持原样。已核对全局裸元素规则仅存在于 `style.css`，无组件书写未加 scope 的 `button`/`input` 选择器，因此影响面限定在原语本身恢复设计外观。
  - 修复模型管理器条目右侧删除叉号贴顶的问题：`.model-item` 使用 `align-items: stretch`，而删除按钮自带固定 `height: 20px`——弹性项在有确定交叉轴尺寸时 `stretch` 会退化为顶端对齐，因此在模型 ID 与 `Created: ...` 描述撑成两行的条目里叉号视觉上偏上。现显式声明 `align-self: center` 并补右侧留白，使叉号垂直居中且不与条目圆角边贴死。

## [1.5.5] - 2026-08-21

### Added
  - 新增 DeepSeek Vision 专用开关（位于 OpenAI 渠道的 DeepSeek user_id 开关旁，也可用于 OpenAI Responses）：开启后仅对 Vision 模型启用 PDF 逐页栅格化、大图分块与官方图片格式规范化；请求前校验 800×800 总像素分块、4096 长边、600 图片、32 MiB 单图和 48 MiB 请求体限制。PDF 渲染使用可选 `pdfjs-dist` + `@napi-rs/canvas`，图片处理使用可选 `sharp`，三语设置说明与依赖管理同步。

  - 渠道设置的“上下文阈值”新增悬停说明：按当前渠道/模型窗口、最大输出预留实时展示有效输入预算与百分比触发 token；绝对 token 模式明确说明直接比较规则；结合全局 `keepRecentTokens` / `keepRecentRounds` 显示首次大约总结的比例或保留量，并说明工具配对、安全边界及 provider 用量优先的估算口径。绝对阈值高于有效输入预算时仅提示风险，不改变既有阈值行为。通用 Tooltip 同步支持多行限宽内容。
  - OpenAI Responses HTTP 执行链新增脱敏请求诊断日志：仅对 `/responses` 请求记录 endpoint host、模型、流式标记、请求体字段、input 项类型/角色/字段形状、reasoning content 路径、`include` 与工具数量，input 过长时保留首尾并报告省略数量；流式和非流式拒绝进一步记录 HTTP 状态及上游 `code/param/type`，兼容多层对象和 JSON 字符串包装。日志不包含 instructions、消息正文、推理文本、`encrypted_content`、工具参数、请求头、API Key 或 URL 查询参数，可直接定位 `input[n].content` 等 schema 错误而不暴露会话内容。
  - OpenAI Responses 思考签名回传新增格式下拉：官方 GPT Responses 模式完整保留 `id/status/summary/encrypted_content`，Codex 反代兼容模式省略 `status`；默认使用官方模式，DeepSeek 的 `reasoning_text` 回传逻辑保持独立。
  - 新增 Gemini Interactions API 渠道类型（`gemini-interactions`）：适配谷歌新版 Interactions API（`POST /v1beta/interactions`，2026-06 GA）。无状态模式（`store:false`）由客户端管理完整 steps 历史；thought 为独立 step（`signature` + `summary`），签名复用 `thoughtSignatures.gemini` 存储键（与 generateContent 渠道互通）；函数调用以 `function_call` / `function_result` 独立 step 关联（call_id）；流式解析 SSE 事件流（`step.start/delta/stop`、`interaction.completed/status_update`，含 `arguments_delta` 增量合并与 `thought_signature` 签名回传）；`generation_config` 使用 snake_case（`thinking_level` 替代旧 `thinkingConfig.thinkingLevel`，预算模式不发送）。原 `gemini`（generateContent）渠道完全保留，第三方中转不受影响。新增 `GeminiInteractionsFormatter` / `GeminiInteractionsConfig`，前端渠道类型选项与 Token 计数面板同步（复用 GeminiOptions 组件），新增 27 个测试用例。
  - 用量统计界面新增缓存命中率展示：总览卡片与设置面板用量摘要显示「总缓存命中率」（缓存命中 token 占输入 token 的比例），按对话/模型/日期列表行同步显示单项命中率。命中率 = `cacheReadTokens / promptTokens`（`promptTokens` 已含缓存部分，数据层无需改动）；无命中或输入为 0 时不展示（与 `cacheReadTokens > 0` 才显示的既有口径一致）。i18n 三语新增 `cacheHitRate` 文案。
  ...
  - openai-responses 渠道新增 `prompt_cache_key` 透传支持（会话缓存命中）：为支持该字段的 OpenAI 兼容网关（如 openai-api-server-via-codex）发送请求体顶层 `prompt_cache_key`，网关会将其转成 `session_id` / `x-client-request-id` 请求头发给 Codex 后端，相同 key 的后续请求可复用后端会话缓存（省 token）。渠道设置新增 `promptCacheKeyEnabled` 开关（默认关闭）：开启后基于主聊天 `conversationId` 的 sha256 生成稳定且不含隐私信息的 key（每对话独立缓存域，续跑沿用旧 runId 时缓存域天然一致，与 DeepSeek user_id 机制同语义；总结、子代理等内部请求不携带 conversationId，不会发送该字段）；另支持 `promptCacheKey` 显式覆盖（跨对话共享同一缓存域，高级用法，留空回退自动生成）。前端 OpenAIResponsesOptions 设置新增开关与自定义 key 输入，i18n 三语同步，新增 7 个 formatter 回归测试。
...
### Fixed
  ...
  - 修复子代理运行后台任务的结果不回投发起者：①嵌套后台子代理（`background=true`）完成结果此前硬编码投递主会话——由子代理启动时发起者收不到，主模型反而收到无关回执；②子代理内部启动的后台命令（`execute_command` 且 `background=true`）结果此前被前端按 `subagentRunId` 过滤后无处投递，任务条展示完即丢失。现 `agentMailbox` 新增通用系统投递 `enqueueSystemMessage`（`toRunId` 缺省投主会话与旧 API 完全一致，指定 run 时校验其必须仍为当前会话已知活跃 run）；`TaskManager` 终态投递按发起者寻址（`background_subagent` 取 `metadata.parentRunId`，后台 terminal 取 `data.subagentRunId`）——发起者仍活跃时结果进入其 inbox、随下一次工具调用注入模型（「插入返回给对应子代理」），已注销/主模型发起时回退主会话保持原语义；`subagents.ts` 后台分支注册任务携带 `parentRunId`，`runController` 活跃记录补 `parentRunId` 字段（嵌套前台子代理 detach 转后台后同样可回投发起者）。新增 6 个投递寻址回归测试（嵌套投递/回退/terminal 归属/前台不投递）。
  - 修复 OpenAI Responses 思考耗时统计：专用 reasoning 合并分支现在以首次收到可见思考文本的时间作为起点，切换到正文时得到正确的思考耗时，并补充回归测试。
  - 修复前台子代理发送消息后主会话不能立即响应：识别仍挂在父回合上的发送方，经 `preserveSubAgents` 转后台并立即开启消息回合，同时隔离 agent 消息重试与普通后台报告，避免消息等待当前回合结束。
  - 修复手动总结在「当前两轮、`keepRecentRounds=2`」时被误判为无需总结：总量超过保留预算时，现把活跃历史按单轮长回合规划，在完整消息序列上寻找安全的 model 切点，不再固定保留内容很少的最后一轮。
  - 修复子代理自动总结与主会话策略分叉、总结期间无反馈及上下文骤降后无法判断是否丢失信息：子代理现在共用主会话的软阈值判定、`keepRecentRounds` / `keepRecentTokens` 安全切点、独立总结模型、提示词、流式解析、低质量校验和最大尝试次数；总结模型装不下全部候选时只消费能完整装入的最大安全前缀，未进入总结请求的尾部继续保留，不再“删除整段但只总结后半段”。总结失败且尚未越过模型明确硬上限时 provider history 保持原样，只有真实硬超限才执行可见的细粒度 fallback；首条任务与已有总结均不可截断。SubAgent Monitor 新增持久化总结状态卡：开始时显示触发估算/阈值，完成后显示估算前后 token 并在下一次普通请求返回时回填 provider 实报 prompt token；完整 transcript 在真实索引处显示总结边界，原文仍可分页查看。`lastSentHistory` 继续保存并在续跑时逐条复用上一次真实 provider history 前缀（总结 + 最近上下文），只追加新续跑任务，避免重装旧大结果并保持 provider 前缀缓存命中。
  - 修复子代理工作 history 丢失普通模型响应 `usageMetadata` 与工具结果回合身份的问题：模型响应现在连同上游 token 用量、思考签名和 transcript 索引完整进入后续请求，functionResponse 明确标记为工具结果而非新用户回合；总结范围、触发诊断和 Monitor 截断位置因此都基于正确的消息语义。provider history 与 Monitor transcript 通过仅供内部使用的索引元数据关联，该字段不进入各渠道请求格式。
  - 修复上下文预算未为模型输出预留 token：新增 `maxInputTokens` / 输出预算解析，自动裁剪、总结和 granular fallback 统一使用扣除输出预留后的输入预算；Gemini / Interactions 的输入上限不重复扣除，OpenAI Responses、OpenAI Compatible 与 Anthropic 的组合窗口避免因输出占用导致请求超限。
  - 修复 DeepSeek Responses reasoning_text 在部分回合缺失或模型完全跳过思考时无法继续回传：对有 assistant 输出的文本回合和工具回合补齐最小 `reasoning_text` 占位，并处理空 reasoning item；保留原有 content-only reasoning 文本与后续请求配对，避免 thinking mode 400。
...
### Fixed
  ...
  - 修复同一用户回合内多次模型请求调用工具时「批次前存档」跨迭代冗余：此前每次工具迭代（一次模型请求）各创建一对「批次前/批次后」存档，相邻迭代间出现「迭代 N 的批次后存档」紧挨「迭代 N+1 的批次前存档」的冗余展示。现批次前存档提升为回合级（`ToolIterationLoopService` 新增 `turnBatchCheckpoints` 回合状态，按「会话 + 回合起始用户消息 id」锚定，与 `turnAutoSummarizeAttempts` 同生命周期策略：新回合或锚点变化自动重置、有界淘汰）：整个真实用户回合（含确认工具跨请求执行、`isNewTurn=false` 续跑）只创建一次批次前存档（挂在回合首个创建迭代的模型消息位置），中间迭代不再创建 before；每次迭代仍创建自己的批次后存档（恢复粒度不损失——迭代 N 的批次后存档即迭代 N+1 的执行前状态）。前端 `addCheckpoint` 按 `cp.id` 去重，重复下发的回合级 before 不会重复展示。非流式路径（`runNonStreamLoop`）同步接入回合级 before + 迭代级 after（此前为工具级 `auto` 存档，现以 `checkpointMode='skip'` 批次化，存档持久化后经 `checkpoint.getCheckpoints` 读取可见）。新增跨迭代回归测试（流式/非流式各一条：两次工具迭代只建一次 before，before 与迭代 1 的 after 同索引、迭代 2 的 after 挂迭代 2 模型消息索引）。
  - 修复工具执行存档部分快照的恢复误删（审查发现）：部分快照的 fileHashes 只含受影响文件，恢复引擎却按「目标缺失 + base 链存在 = 快照时被工具删除」默认删除——恢复到工具批次存档会把工作区其余文件误删（无需用户确认）。现存档 manifest/记录携带 `partial` 标记（`CheckpointBackupExecutor` 写入、`enrichRecord` 回填、`RestoreTargetState` 透传），`computeRestorePlan` 对 partial 目标禁用 deletedInSnapshot 判定：缺失文件一律归 untracked 默认保留，白名单缺省时也不得进入 toDelete（引擎层最强制保证——恢复部分快照绝不删除任何文件）；同时部分快照不再计算 `changes.deleted`（「上一存档有而部分快照没有」只是不在扫描范围内，并非被删除，此前会污染恢复链与后续存档的增量比较）。新增恢复引擎回归测试（partial 目标不误删 / 全量目标旧语义保持 / 恢复执行不删除未记录文件）。
  - 修复部分快照提取与判定的四处缺陷：① `search_in_files` replace 模式此前取 `args.path` 进入部分快照，但 `args.path` 通常是目录（对整个子树替换），部分快照对非空目录不递归——被替换的文件不进入快照，恢复时既无法还原内容又触发误删判定；现一律返回 null 回退全量（影响面 = pattern × 目录子树，静态不可知，正确性优先）。② 部分快照分支在 stat 之前以 `isDirectory=false` 判定忽略规则，目录型规则（尾斜杠）对受影响路径中的目录不生效；现先 stat 再按真实类型判定（与全量分支 collectEntries 顺序一致）。③ `isPathWithin` 新增 platform 参数可注入（与 `isSameStoragePath` 同策略，Linux CI 可测 win32 大小写折叠）；`workspaceUriToFsPath` 剥离未编码 fragment/query 并拒绝非法编码序列（回退全量）。④ execution.ts 批量路径 after 存档排除被参数错误/策略拒绝/中止未执行的工具（与 orchestrator 同一语义），不再多建空批次存档。
  ...
  - 修复 Linux CI 上 `isPathWithin` 平台参数注入不完整（与 `isSameStoragePath` 同源问题，release 1.5.3 发布流水线再次被拦）：此前的 platform 参数只切换大小写折叠策略，`path.resolve`/`path.sep` 仍取宿主平台——Linux CI 上注入 `'win32'` 时 `D:\\GrayCode\\src\\a.ts` 被 POSIX 语义当作普通文件名拼到 cwd 下，与 `d:/graycode` 永不匹配，`affectedPaths.test.ts` 断言失败。现按 platform 选择 `path.win32`/`path.posix`（与 `isSameStoragePath` 修复同一模式），路径归一化/分隔符边界/盘符识别全部随注入平台；生产默认 `process.platform` 行为零变化。
  - 修复子代理（及主链路）在 DeepSeek Responses 兼容端点上运行一段时间后报 `HTTP 400: The reasoning_text in the thinking mode must be passed back to the API`：DeepSeek 无状态 Responses 接口要求带 tools 的后续请求把上一轮 reasoning 作为 `reasoning` 输入项完整回传（plain `content[].reasoning_text`），而此前仅识别 `encrypted_content`/`id` 形态，plain reasoning 会被降级成普通 `output_text` 或直接过滤。现 `openai-responses` formatter 把 reasoning 回传拆成双开关：`sendHistoryThoughts` 控制 plain `reasoning_text`/summary 回传（DeepSeek 形态），`sendHistoryThoughtSignatures` 控制 `encrypted_content`+`summary` 回传（OpenAI 官方形态），无 Responses 元数据的普通裸 thought 仍保持过滤不误包装；流式补齐 `response.reasoning_text.done` / `response.reasoning_summary_text.done` 解析，delta 携带 `openaiResponsesReasoning`（id/status/content），`StreamAccumulator` 按 reasoning id 合并增量与完成事件（无 id 时回退最近思考 part），完成事件覆盖最终全文且不重复下发；主链路 `formatHistoryForAPI` 关闭历史思考时保留带 Responses reasoning 元数据的 part（协议回放数据，非普通 thought 文本）。新增 DeepSeek content-only 流式合并/下一轮回传/开关降级回归测试，OpenAI 官方加密 reasoning 行为保持不变。
  - 测试：`xmlFormatter.test.ts` 的超深嵌套与危险键名安全用例触发 fast-xml-parser 的 [SECURITY]/maxNestedTags 防御并打印 `console.warn`（预期防御行为），全量测试运行把该告警计为失败输出；现两个用例在断言期间静默该告警（`jest.spyOn(console,'warn')` 包裹后恢复），断言语义不变。
  - 修复工具执行存档部分快照的增量链统计失真与恢复误删（CP-PARTIAL-3）：部分快照的 fileHashes 只含受影响文件，批内出现副作用不可知工具（search_in_files / execute_command 等）导致 after 存档回退全量扫描时，`computeChanges(部分快照的几个键, 全工作区)` 会把整个工作区全部判为 added，产生 fileCount=全工作区文件数的「伪全量」存档（数据完整但变化统计失真）；恢复该伪全量存档之前的某个部分快照时，工作区其余文件（快照时存在且未修改、只是不在部分快照扫描范围）全部被归入 untrackedToDelete，用户确认后误删。现部分快照必须建立在完整基线上：仅当上一存档存在且本身不是部分快照时成立（上一存档缺失/本身是 partial 时回退全量扫描，顺带自愈旧版部分快照链），其 fileHashes/fileStats 继承上一存档的完整映射、只覆盖受影响文件（受影响文件 stat 失败 = delete_file 删除/不可读的从继承映射剔除，避免被删文件「复活」），emptyDirs 同样去重合并继承。修复后部分快照映射保持完整：后续全量存档的增量只有真实变化（伪全量消失），恢复部分快照时未受影响文件与工作区哈希一致（skipped，不进删除清单）。新增 5 个集成回归测试（映射继承 / 全量增量不再伪全量 / 恢复计划不误删 / delete_file 剔除 / 上一存档为 partial 时回退全量）。

### Changed
  - 自动总结触发时保留主请求的 system prompt、动态上下文、工具定义、提示词模式、原始历史和 DeepSeek conversation 缓存域，仅在末尾追加详细 user 总结提示词；同渠道/模型的总结请求复用原始前缀以提高 provider 缓存命中，独立总结模型继续使用原有清洗隔离路径。总结预算按原有基础提示词计算，不因追加说明改变压缩切点。
  - 上下文裁剪的自动总结判定与前端 usedTokens 显示口径对齐：`accumulateContextTokens` 新增 `useUsageAnchor` 模式（`getHistoryWithContextTrimInfo` 开启）——以「最后一条带 `usageMetadata` 的模型消息」的 `totalTokenCount`（缺失时回退 `promptTokenCount`）作为估计总量锚点，直接返回该值并跳过逐条本地估算（×1.5 安全系数）累加；无任何 usage 记录（新会话、代理不返回 usage 等）时回退原有估算逻辑，行为不退化。修复「界面显示 26 万却等到 40 万阈值才总结」的脱节。新增 `tokenAccumulatorUsageAnchor.test.ts` 覆盖锚点命中 / totalTokenCount 回退 / usage 后新消息不累加 / 无 usage 回退 / 默认关闭保持旧行为。
  - 修正 write_file 工具契约说明：新建文件时工具声明此前仅描述「执行前展示 Diff 预览并等待确认」，未说明确认前会先落盘空文件（确认等待期间工作区短暂可见）；现中英文描述如实补充「新建文件确认前先创建空文件、拒绝或取消时残留自动删除」，并在预创建空文件处补充设计说明注释，不改运行行为。
  - 设置核心深合并收敛：`core/deepMerge.ts` 的 `deepMerge` 新增可选 `options`（`nullMode`/`undefinedMode`/`conflictMode`），缺省行为与历史完全一致；`SettingsCore.deepMergeToolsConfig` 改为该参数化实现的薄封装（`nullMode:'write'`、`conflictMode:'reuse-source'`、`undefinedMode:'skip'`），删除本地递归实现，对非循环输入行为零变化（额外获得循环引用防护）；`deepMergeConfig` 因「default 为主、stored 补齐」方向相反且含损坏数据兜底语义，刻意保留本地实现，三套 merge 的关系与差异集中文档化到 core/deepMerge.ts 头注释。
  - DependencyManager 单例支持存储路径切换：`getInstance(context, customDepsPath)` 在实例已存在且显式传入的 `customDepsPath` 与当前 `graycodeDir` 不一致时重建实例（需携带 context，否则 fail-fast 抛错），依赖重新安装到新目录，不再静默沿用旧目录；重建会丢弃旧目录的 progressListeners/loadedModules 缓存（切换路径场景可接受）。
  - 工具声明 schema 与行为对齐：`goto_definition` 的 `line`/`column` 由 `type:'number'` 改为 `type:'integer'` + `minimum:1`，与 handler 整数校验及 read_file 对齐；`refreshSubAgentsTool` 由「只打日志的空操作」改为纯 no-op 并注明 subagent 工具声明是 getter 每次访问重算（保留导出仅兼容既有 webview 调用方）。
  - webview 置顶文件配置响应收敛：`getPinnedFilesConfig` 移除前端零消费的硬编码英文 `sectionTitle` 字段，有无工作区两个分支统一返回 `{ files }` 契约（`sectionTitle` 仅后端 PromptManager 直接读 settings 消费）。
  - 测试补齐：新增 `write_file`/`insert_code`/`delete_code` 三个写类工具 handler 的 diff 审阅终态单测（接受/拒绝/取消/autoSave 失败/参数边界，共 23 用例）与 `ToolRegistry` 核心行为单测（重复注册抛错、refreshTool 失败保留旧实例、revision 递增语义，5 用例），覆盖此前无直接测试的终态组装逻辑。

  - 设置读取性能优化：`SettingsCore`/`SettingsManager` 新增 `getScalarSettings(...keys)`（标量直返、对象/数组走深拷贝兜底）与 `getUpdateSettings()`（checkForUpdates/updateChannel/proxy 快照）；bootstrap UpdateChecker 回调改走 `getUpdateSettings`，不再每次对整个设置树（含庞大 toolsConfig）全量深拷贝。`getSettings()` 全量语义保留；WindowsAgentStopNotificationService 因低频读取暂保持。
  - 工具声明 getter memo 化与轮询优化：apply_diff/subagents/history_search 的动态声明 getter 按「语言 + 输入指纹」进程级缓存，输入变化时重建（subagents 复用 toolNameSnapshotCache.key）；子代理 runLoop 超时轮询间隔由 500ms 拉大到 2000ms（暂停时间扣减由 getActiveElapsedMs 保证，与轮询间隔无关）。
  - 流式视图不可达保护补齐「流启动即无视图」场景：`StreamChunkProcessor` 新增 `allowHeadlessConsume`（默认 false），默认下流启动时目标 webview 已销毁/未注册同样判定不可达并立即 abort 后端生成；仅显式允许 headless 的路径保留「视图从可达变为不可达才中止」的旧语义。
  - 设置导出/导入三份平行实现收敛：新建 `webview/utils/settingsTransfer.ts` 的 `exportSettingsToFile`/`importSettingsFromFile`（统一 exporter 构造、保存/打开对话框、覆盖询问与默认文件名 graycode-settings.json），ChatViewProvider/SettingsTransferHandlers/settingsCommands 三处调用方只保留各自入口适配。
  - 路由映射生命周期收敛：`MessageRouter.requestClients` 的增删收敛为 `RequestRouteTable` 的 track/finish/fail/routeResponse/routeError 语义方法，删除时机唯一事实来源（流式 keepUntilFinalize 保留、非流式/错误终态删除、投递失败回退主聊天），消除约 9 处散落 delete 的漂移风险。
  - 视图重建后旧会话任务隔离：ChatViewProvider 引入会话代际号（视图重建/关闭递增），消息到达时捕获代际，旧代际的响应/错误经 `GenerationAwareWebviewClientRegistry` 拦截不再投递到新视图（取消类副作用仍执行）。
  - 错误边界统一：BranchHandlers/CheckpointHandlers/McpHandlers/SubAgentsHandlers 的手写 try/catch 收敛为 `withBoundary` 统一口径（对外错误码语义不变；BranchHandlers 因错误码动态映射保留 `withBranchBoundary` 薄封装）。
  - 传输层契约文档化：`shared/protocol.ts` 固化「流式多响应」「requestId 归一化」「resolveClientId 未注册回退」三条跨端约定为注释文档，不改运行时逻辑。
  - nightly 版本基线修复：nightly 版本改为「当前版本的下一 minor 版本 + `-nightly.<date>`」（如 1.5.3 → 1.6.0-nightly.<date>），恒高于当前稳定线，避免稳定版 bump 后 nightly 被 compareVersions 判为落后；解析失败退回旧形式。
  - 消息列表虚拟窗口上限：`useVirtualMessageWindow` 由「只增不减分页」改为 200 行滑动窗口（顶部滑窗 + 底部裁行 + 锚点法恢复滚动位置），长会话上滚渲染 DOM 不再无界增长；吸底与流式新增贴尾语义保持。
...
### Fixed
  ...
  - 修复流式早启动工具产生多组存档：流式边执行路径对每个工具单独调用工具执行服务，原实现为每个工具各创建一组 before/after 存档（一次模型回复 N 个工具 → N 组物理存档，前端重复展示、每次存档全量扫描工作区、额外消耗 maxCheckpoints 配额）。现检查点提升到「一次模型回复 = 一个工具批次」维度统一管理：before 在第一个已配置存档工具启动前创建（tool_batch，挂模型消息索引）、after 在全部工具完成后创建（幂等），早启动与主循环执行均以 `checkpointMode='skip'` 跳过执行核心内部检查点；纯只读批次仍不创建；取消/中止路径不补 after。修复后每条模型回复仅一对存档（前端显示为模型消息前一条「工具批次前」+ 后一条「工具批次后」），消除多组存档点错位/刷屏；`CheckpointService` 新增 `isToolConfiguredForCheckpoint` 供批次判定；新增 `streamToolBatchCheckpoint.test.ts` 覆盖全早启动/混合/纯只读/取消/纯主循环场景。
  - 修复工具批次存档的 before/after 判定过宽（批次存档仍出现「未勾选执行前」的误存档）：`CheckpointManager` 对 `tool_batch` 只按「beforeTools/afterTools 列表非空」决定创建，只要用户给任意工具勾过 before，批内仅配置了 after 的工具（如只勾「执行后」的 write_file）也会在它执行前错误创建批次 before；且确认路径（diff 审阅后执行）按工具级 `auto` 模式各建一组存档，导致「写入文件后/执行命令前」出现多余存档。现 `createCheckpoint` 新增 `options.batchToolNames`，`tool_batch` 按批内工具与 `beforeTools`/`afterTools` 的交集精确判定（未传时回退旧语义兼容）；`isToolConfiguredForCheckpoint` 支持 phase 参数（before/after 各自精确判定）；流式批次在遇到确认工具时不再提前创建 after（`finalizeStreamBatchCheckpoints` 新增 `createAfter` 参数），由确认路径（`handleToolConfirmation`）在队列全部完成后以 `tool_batch` 补建一次 after，确认工具与 autoSuffix 均以 `checkpointMode='skip'` 执行；批次 before 在确认事件下发前补建（`ensureBatchBeforeForConfirmation`，批内确认工具配置了 before 而自动工具未触发时）。修复后：未勾选「执行前」的工具不再有批次 before 存档，确认/自动混合批次全流程仅一对存档（确认工具执行前 + 全部工具完成后）。新增回归测试：`CheckpointManager` 精确判定 3 用例（未配置 before 不创建/批内任一配置即创建/未传 batchToolNames 回退）、`toolConfirmationAbort.test.ts` 确认路径正常完成（skip 执行 + 队列完成补建 after）。
  - 批次存档精确化交叉审查修复：① `batchIndexPromise`（批次挂载索引惰性计算）reject 未捕获，transcript 读取失败会打穿整个工具批次与流——创建时挂 catch 索引留空，各消费点（ensure/finalize/确认补建）在 `messageIndex` 缺省时走独立兑底读取；② 早启动 `ensureStreamBatchBeforeCheckpoint` 无 try/catch，checkpoint 异常会连带首个早启动工具失败——现与主循环一致 warn 降级不阻断工具执行；③ `isToolConfiguredForCheckpoint` 新增 `config.enabled` 检查，整体关闭存档时不再让每个早启动工具各触发一次含全量 transcript 读取的无效创建尝试；④ 确认路径补建批次 after 排除本回合被拒工具（`rejectedToolIdsThisTurn`）——「批内唯一工具被拒」不再产生一对空批次存档；⑤ 早停（shouldStop）且存在确认工具时不再提前创建批次 after（确认工具未决，与「取消/中止不补 after」语义一致）；⑥ `needsCheckpoint` 收敛为纯「批内工具命中 afterTools」语义，仅配 before 的批次 finalize 只下发 before 不再多发一次 after 创建尝试；⑦ `batchToolNames` 显式空数组＝无工具不建存档（与「未传」回退旧语义明确区分）。
  - 删除子代理 runLoop 工具执行段的 30s 无条件超时兜底（`Promise.race([Promise.all, 30s 定时器])`，恢复裸 `Promise.all`）——abort 收敛已由 `executeToolCall` 的 500ms 宽限（`waitForAbortableOperation` / `SUBAGENT_TOOL_ABORT_GRACE_MS`）覆盖：abort 信号触发 → 立即返回 aborted → 500ms 宽限后把不响应 abort 的工具结算为 cancelled 结果 → `Promise.all` 收敛、run 正常收尾；正常执行时工具层自身保证落定（`execute_command` timeout 参数含 SIGTERM→SIGKILL 升级 / 嵌套 subagents `maxRuntime` 默认 1800s / MCP 默认超时）；与主会话 ToolExecutionService 的裸 `Promise.all` 语义对齐。原兜底把正常执行超过 30s 的工具误杀（子代理内跑测试、大文件扫描、慢 MCP），错误信息 "Parallel tool execution did not finish within 30s; run aborted (M5)"。删除 m5_hang 测试，新增「挂死工具不被超时兜底误杀（推进超过原 30s 窗口 run 仍活跃）」「abort 后 500ms 宽限收敛不卡死」两个回归测试。

### Changed
  - 工具执行存档改为按「模型传入参数涉及的文件」构建部分快照（不再全量扫描工作区）：新增 `extractAffectedPaths` / `workspaceUriToFsPath`（`checkpoint/affectedPaths.ts`），对 write_file / apply_diff / insert_code / delete_code / delete_file / create_directory 白名单工具提取 `args.path`（相对路径 resolve 到工作区根、绝对路径原样、路径穿越防御回退全量；`search_in_files` 无论 search/replace 一律回退全量——replace 影响面不可静态确定，见 Fixed 段）；`buildWorkspaceSnapshot` 新增 `affectedPaths` 分支——只对列出的路径 stat + 哈希（忽略规则/大小上限/previous 复用/空目录/多根归属/ENOENT 不可读语义与全量分支一致），不再递归扫描整个工作区；ToolIterationLoopService（批次 before/after 与确认路径）、execution.ts（单工具/批量路径）、orchestrator（确认路径补 after）在创建检查点时收集受影响路径透传，批内任一工具无法确定（execute_command 等副作用不可知）→ 整批回退全量保证快照完整性；手动存档 / user/model 消息存档保持全量不变。新增 `affectedPaths.test.ts`（提取单测 / 部分快照分支 / 链路透传与回退）。
  - 前端：子代理设置「全局配置」四个参数（最大并发数/默认迭代次数/队列超时/默认运行时长）由一行四列改为 2×2 网格布局——一行四个时 label 与提示文字在窄面板下挤成多行；现「并发数/迭代次数」与「队列超时/运行时长」各占一行，每格宽度翻倍（`.global-config-row`，仅作用于全局配置区，不影响 agent 配置区的 `.form-row` 布局）。
...
### Fixed
  ...
  - 修复 `show_windows_notification` 只在 VS Code 右下角显示而非 Windows 系统通知：曾因 node-notifier 停更/传递依赖安全告警改用 VS Code 原生通知（`VSCodeNotificationAdapter`）；现按需求换回 node-notifier（`NodeNotifierWindowsToastAdapter`，系统 toast），`WindowsAgentStopNotificationService` 同步换回；esbuild 恢复 node-notifier 的 external 与 dist/node_modules 复制逻辑（snoretoast.exe 二进制随包分发）；测试重写为覆盖 `NodeNotifierWindowsToastAdapter`。
  ...
  - 修复通知点击后 VS Code 窗口仍无法置为前台：`focusWindow` 此前裸调 `SetForegroundWindow`，受 Windows 前台锁限制（仅前台进程/刚接收过用户输入的进程才有权抢前台），新建的 powershell.exe 子进程无前台权限被系统静默拒绝——点击 toast 后聊天面板虽打开但窗口不聚焦。现 `Win32Focus` 封装 `FocusWindow` 组合拳：`keybd_event` 模拟一次 Alt 键（让系统认为本进程刚收到用户输入）→ `AttachThreadInput` 附加到前台线程输入队列后 `SetForegroundWindow` → `BringWindowToTop` 兜底（激活被拒也尽量把窗口提到 Z 序顶部）。已验证：Add-Type 编译通过（PowerShell 5.1 / C# 5 兼容）、进程树可定位到 Code.exe 主窗口句柄、调用后 `GetForegroundWindow` 命中目标窗口；`notifications` 目录测试全绿（3 suite / 19 cases）。
  - 性能：`DiffManager` 预热扩展到虚拟原文档——上一轮只预热真实目标文档，首个 `vscode.diff` 仍会为 `gemini-diff-original` 左侧冷启动内容提供器/语言模式/Diff 模型；现 `prewarmDocument` 双侧预热，`consumePrewarmedDocument` 统一消费（防 stale、失败 fallback）。
  - 性能：`DiffManager` 新增单 Diff 写入就绪共享屏障（`writeReadyPromises`）——checkpoint 完成与 deferred 写锁获取只执行一次，`createPendingDiff` / 50ms 自动确认 / 手动接受 / 拒绝 / 保存事件全部汇合到同一 Promise，不再对同一 holder 重复重入加锁；预览出现后立即发布 pending 状态并启动自动确认，写盘/回滚仍等待屏障，审阅期间不再被首个共享 checkpoint 挡在 UI 与倒计时之前。
  - 移除流式早启动工具等待超时兜底：等待循环恢复「与 abort race + 无限等待落定」语义，不再把固定窗口内未落定的工具以失败占位结算。理由：工具执行层自身已保证最终落定（`execute_command` 受 timeout 参数约束且有 SIGTERM→SIGKILL 升级、MCP 请求有默认 30s 超时、`show_windows_notification` 等待用户点击后 resolve），且 `runToolLoop` 全部调用方均传入 `abortSignal`（停止按钮不受影响）；固定窗口（曾 3s、放宽后 10s）会把仍在正常执行的慢工具（如 MCP 工具）误判为超时失败，而真实结果随后到达时已结算的失败占位无法被覆盖，导致副作用重复执行与结果丢失。删除 `STREAM_EARLY_TOOL_SETTLE_TIMEOUT_MS` / `SHOW_WINDOWS_NOTIFICATION_SETTLE_TIMEOUT_MS` / `resolveEarlyToolSettleTimeoutMs` 及对应测试。
  ...
  - 安全加固：webview 文件读取链（FileRead/FilePreview/FileOpen/PinnedFile）引入 realpath 感知的工作区包含性校验（新增 `webview/utils/workspaceRealpath.ts`，与后端 `workspacePaths.resolveRealpathForComparison` 同口径：realpath + 最近存在祖先 + Windows 大小写归一），工作区内 symlink 指向工作区外文件不再被误放行，`read_file` 等工具的 deny/ask 策略不再被静默绕过；`shellConfig` POSIX 分支 `which` 探测改 spawnSync argv 传参（消除 customPath 命令注入面）；`FilePreviewHandlers` 临时文件写入改为 `mode: 0o600`（多用户系统不再以 0644 暴露对话/附件明文）。
  - 数据安全：`storage.ts` 分段历史双 rename 提交路径对齐 `renameWithRetry`（EPERM/EACCES/EBUSY 线性退避重试），「删旧+rename」回退改为「旧目录改名备份 → rename 到位 → 清理备份」，第二次 rename 失败时恢复备份，不再可能留下「在线分段历史已删、新目录未落位」的不可恢复状态；`loadSnapshot` 错误分级（ENOENT 返回 null，EACCES/EIO 等真实 IO 错误向上抛，不再被 catch-all 吞掉）；`MemoryManager.writeConfig` 改 tmp+rename 原子写（并发实例 lost-update 与手工编辑被冲掉问题收敛）；`DiffStorageManager` 保持「缓存=已落盘」不变量（persist 失败时移出缓存并让调用方感知，不再出现重启即丢的假象）；`UsageIndexStore` 分支合并去重键优先使用稳定 `entry.id`（同一毫秒 token 计数完全相同的两条消息不再被误判丢弃）。
  - 流式/挂起：`streamBufferParser` 修复 SSE `data:` 前缀被 chunk 边界切断时事件静默丢失（chunk 末尾非完整行并入 remaining 等待拼接）；`processRunner` 超时杀进程补 SIGKILL 升级（对齐 `killTerminalProcess` 的 KILL_WAIT_CLOSE_TIMEOUT_MS，SIGTERM 免疫进程不再让 execute_command 永久挂起）。
  - 性能：`proxyFetch` 非流式错误体/chunked 响应结束标记探测改为增量扫描（仅扫新增尾部 + 保留 3 字节跨包窗口），消除逐 data 事件 `Buffer.concat` 全量的 O(n²) 退化；`ChatViewProvider` 终端输出事件加 50ms 节流批处理（对齐 StreamChunkProcessor，高频日志流不再逐 chunk postMessage 压满扩展宿主）。
  - 设置：`VSCodeSettingsStorage` 读恢复 VS Code 合并值语义（workspaceFolder > workspace > global，工作区显式配置优先），save 按目标层级写入——某键工作区层已有值时写入 Workspace 层（.vscode/settings.json / .code-workspace）、否则写 Global，设置页修改落在最高优先级层、不再被工作区旧值吞掉，同时保留 workspace 显式配置覆盖 Global 的惯例（曾改读优先 Global 破坏覆盖语义，现以层级跟随替代）；`ui.language` 默认值 `'zh-CN'` → `'auto'`（跟随 VS Code 语言，非中文用户不再被默认强制中文回复，且不再经 Settings Sync 固化扩散）；`StoragePathManager.migrateData` 同路径判定 Windows 大小写归一（仅大小写不同的目标路径不再触发「同目录中转迁移」清空全部存储数据）；`DEFAULT_DIAGNOSTICS_CONFIG.enabled` 改回 `false`（与接口文档一致，默认不再每轮向模型注入工作区诊断）。
...
### Added
  ...
  - fast-tavern 子项目：实现 build 模块——`buildPrompt`/`buildPromptFromSillyTavern` 重构（阶段视图/深度注入/worldbook 静态条目排序/视图过滤），替换插值转义与正则/数值边界加固；TS 与 Python 双实现同步更新。
...
### Fixed
  ...
  - 后端 checkpoint：修复元数据残留精确匹配（分支/代理残留不再误命中）、retention 假 manifest 防护（损坏的 manifest 不再引发异常）。
  - 后端 conversation：修复 `appendHistory` 索引不可读自愈重建、插入消息同步分支图、`rejectToolCalls` 空数组语义（空数组不再误判）。
  ...
  - webview：修复面板关闭未中止活跃流、消息队列重置、Monitor 路由守卫。
...
### Fixed
  ...
  - 修复后台任务链路完全断连（输入框上方任务小气泡不显示 + 后台子代理/后台命令完成回执不回流主模型 + 后台终端输出不刷新 + 依赖安装进度不显示）：后端统一走 sendCommand 队列推送的消息结构为 `{ type: 'command', command: 'xxx' }`（webview 未 ready 时自动入队、ready 后 flush），但前端 backgroundTaskStore/terminalStore/DependencySettings 三处消费点仍按旧格式 `message.type === 'xxx'` 匹配，taskEvent/terminalOutput/dependencyProgress 三类推送全部无法到达前端；现统一改为 command 格式匹配（store 改 onExtensionCommand，组件改判断条件），并新增 backgroundTaskEventRouting 回归测试（4 例消息路由 + 1 例端到端：command 消息 start→complete 后回执经 chat.sendMessage 发进对话，reported=true），锁定消息格式防止再断。
  ...
  - 更新受安全公告影响的前端与 Fast Tavern 锁定依赖（`nanoid`、`picomatch`、`rollup`），并按当前源码重新核对 `KNOWN_ISSUES.md`，移除已解决条目。
  - 修复聊天输入框粘贴后 `Ctrl+Z` 无法撤销（第 4 次修复）：VS Code Webview 中浏览器原生 undo 栈对 `execCommand` 产生的记录支持不可靠，多次修复仍反复失效。现改为自定义撤销/重做历史栈：每次输入、粘贴、删除、换行等操作后自动保存节点快照与光标位置（上限 100 条），`Ctrl+Z` / `Ctrl+Y`（或 `Cmd+Z` / `Cmd+Shift+Z`）直接接管，不再依赖 Webview 原生 undo；恢复时通过 `renderNodesToDOM` 重建编辑区并精确还原光标文本偏移，同时利用 `isInputting` 标志避免受控节点同步触发无意义的 `innerHTML` 重建。
  - 修复聊天输入框在空窗口/无工作区场景下纯文本粘贴与 `Ctrl+Z` 撤销不可靠：不再于粘贴事件中临时切换 `contenteditable` 编辑宿主（切换会重建 Chromium 原生撤销栈，粘贴产生的撤销记录随恢复动作一并销毁），改为粘贴时单次 `insertHTML` 命令写入——HTML 转义后的纯文本 + 带 `data-lim-break` 标记的 `<br>` 换行（与渲染/提取/删除逻辑的内部表示完全一致，多行粘贴不丢换行；`insertText` 方案会把 `\n` 转成无标记裸 `<br>` 被节点提取吞掉），一次调用恰产生一条撤销记录，`Ctrl+Z` 可整体撤销本次粘贴；Windows 剪贴板 `\r\n` 同步归一化为 `\n`；受控节点同步仅在 DOM 与外部状态确实不一致时重建，避免无意义的 `innerHTML` 清空破坏复制、粘贴、选区和撤销历史；光标 Range 初始化同步加固。输入框自动高度测量改为先恢复 `auto` 再读取真实内容高度，删除长文本后可正常收缩。
  - 修复 agent 模式权限漏洞：ask/plan/design/review 等受限模式此前可借 `search_in_files` 的 `mode=replace` 越权修改文件内容（该工具在 allowlist 中仅以搜索名义授权）。现在模式工具策略新增统一判定（`isSearchInFilesReplaceForbidden`）：allowlist 授予 `search_in_files` 但未授予任一通用写工具（write_file/apply_diff/insert_code/delete_code/delete_file/create_directory）时，`replace` 模式在运行时被拒绝（`getToolRejectionReason` 新规则，主会话与子代理共用同一执行门），且工具声明同步收敛为只读（`mode` 枚举仅剩 `search`、移除 `replace`/`maxFiles` 参数，模型在声明层即不可见替换能力）；只读搜索能力完全保留，用户自定义授予写工具的模式不受影响。
  ...
  - 修复「并行工具调用 + 用户中途插话」竞态导致的 `API_ERROR: HTTP 400`（assistant 消息的 `tool_calls` 缺少配对的 tool 消息）与删除消息 `MESSAGE_CHANGED`：① 主路径 `formatHistoryForAPI`新增配对校验——`functionCall` 必须在其所属消息「紧随其后的连续 functionResponse 块」内存在配对响应，否则调用与配对响应一并剔除（覆盖无响应的孤儿调用与「响应被迟到结算追加到用户消息之后」的错位形态），并同步剔除被丢弃调用的 functionResponse，从根上杜绝 `[assistant(tool_calls), user, tool]` 非法交替顺序；② `settleFunctionResponses` 迟到结算不再追加到历史末尾，而是插回所属 functionCall 消息的 FR 块之后（正常路径位置不变，竞态路径下插回用户消息之前，保证 assistant 的 tool_calls 永远紧随其 tool 消息）；③ 工具主循环 `drain` 收尾窗口超时（工具不响应 abort 且永不结束）时不再整段跳过结算，改为为未完成调用写入 cancelled 占位（早启动工具的真实结果仍保留），不再留下无响应的孤儿 tool_calls；④ openai formatter 防御层同步剔除无配对响应的调用（覆盖不经 `formatHistoryForAPI` 的直传历史路径）。新增 `unrespondedToolCallFilter.test.ts`（6 用例）、`settleFunctionResponsesLateInsert.test.ts`（3 用例）与 toolIterationLoopCancel drain 超时占位用例，并补充前端「待确认工具时发送 = 中断当前回合」回归测试（`sendWhilePendingTools.test.ts`）。
  - Bug Hunt Round 3-5 全面修复（侦察 3 轮 30+ 审查代理 + 修复 4 轮 40+ 修复代理 + 复查 3 轮 30+ 代理；round3 报告见 `.graycode/review/bug-hunt-round3/`）：
    - 数据安全：MemoryManager 降级模式（320B）deleteEntries/truncateLog 误用常量 1024 宽度导致**全量记忆丢失**（改 this.logRecMode，repairLog 先于截断执行）；appendHistory M4 自愈路径中间段不可读静默跳过改整体报错；clearHistory/restoreSnapshot 不同步分支图导致已删消息切分支"复活"（新增 forceResetToEmpty 无条件重置空图 + rebaseWithRootChange 头部删除重链，旧根专属子树物理移除）；deleteMessage 中间删除改 rebase 保留后续消息（对齐 summary 分支）；deleteToMessage 锁外快照校验改锁内（并发越界/截错）；settleFunctionResponses 无 id part 按注释承诺追加末尾 + 全跳过不再空写回。
    - 会话一致性：createAndPersistConversation 竞态窗口（创建期间切标签页消息发错会话 + 误清新会话流式状态）——写回加 null 守卫 + streamingMessageId 守卫；loadCheckpoints 会话归属校验、switchConversation 补清 branchGraph 状态；索引 Map 截断后 rebuildMessageIndexById/toolResponseIndex 补全（retry/delete/send 全路径，跨会话无界增长消除）；deleteConversation 清理残留标签页/快照/缓冲；tabActions 快照恢复兜底复位 isLoading/isLoadingMoreMessages（加载中途切走不再卡死）；backgroundTaskStore watch 依赖注册时机修复（ref 化）。
    - 流式/工具链：SSE 末事件不以空行结尾被丢弃（done 分支 decoder flush + 派发残留事件），tools/call 不再挂起超时；abort 收尾窗口空 functionResponse 不再落盘；ToolIterationLoopService 流式主循环泵与 execution 生成器 reject 统一构造带 error 的配对 responsePart/toolResult（消除孤儿 tool_calls 与 Anthropic 400）；cancelStream TOCTOU（取消代次快照 + create 后复查）；earlyCheckpoints 真实下发（不再恒空数组）；toolsExecuting 快照改护栏后 args；handleToolConfirmation 补 RepeatedCallGuard；subagents runLoop 收尾超时 abort 在飞工具 operationSignal（事件不再发往已注销 run）。
    - 工具/终端：终端任务 timeout 写入注册 metadata（>30min 前台任务不再被 cleanup 强杀）；killTerminalProcess 等待真实 close（SIGKILL 兜底后仍等 close 落定 endTime/exitCode）；TaskManager.cleanup 接线到 BackendRuntime.dispose（此前全仓零调用）；write_file content 与 originalContent 同做 LF 归一化（CRLF 不再产生无意义 diff）；GBK 切换跨 chunk 多字节前缀保留；jsonFormatter 无结束标记块改 malformed_tool_call 反馈；shellAvailabilityCache 加 TTL；MCP refreshChains 合并 + 链长上限 + 代际循环重查（list_changed 风暴不再无限增长）；updateServer transport 关键字段比较 + 已连接时自动重连；stdio error 补发 server:error 事件；StdioClient/HttpClient JsonRpc 接口重复拷贝登记后续合并。
    - 前端体验：auto 语言双实现不一致导致界面中英文混杂（统一 actualLanguage 归一化）；PromptSettings 计数转圈锁死/重名模式可连续创建；超短视频缩略图挂起 10s 超时；空白标签页排队内容丢失；InputArea/TokenCountMethodSettings/ToolMessage 三个缺失 i18n key（全仓 2113 个 t() 字面量脚本比对归零）；Markdown 空格保留正则配对栈重写（嵌套同名标签错配消除，代码块空格不再被污染）；图片加载串行改有界并行（4）。
    - 性能：ContextTrimService getHistoryWithGranularFallback O(n²)→O(n)（validSuffix 预计算 + 精确复核）；SummarizeService 收缩循环游标化 + 判存在不再全量读盘；deleteMessagesInRange O(k·n)→O(n)（先克隆再改标记）；BranchGraph 子树收集 O(n²)→O(n)（邻接表）；fileWriteLockManager 惊群→定向唤醒 + 兜底 timer 死代码清理；Markdown 渲染缓存加字节预算 FIFO；SettingsCore 深拷贝缓存登记（改点分散风险大于收益）。
    - 平台/CI：release.yml/nightly-build.yml 前端 `npm ci` 必失败（锁文件与 package.json 不同步）改 `npm install --no-audit --no-fund`；nightly-build action inputs 修复（overwrite→overwrite_files、删除不存在的 update_release_body）；`graycode.runIntegrityCheck` 命令接线（bootstrap initMaintenanceCommands，此前命令面板 not found）；generate-tool-meta.mjs parseArray 未定义变量 ReferenceError 修复；WindowsAgentStopNotification dedupe 时序（渲染成功后再记录，抛错不再滞留 5 分钟）；串行队列豁免（installUpdate/storagePath.reset/checkpoint.restore/deleteBatch/previewRestore/deleteMemoryEntries 移入 NON_BLOCKING + UNBOUNDED，分钟级任务不再冻结 cancelStream/deleteMessage）；UpdateHandlers 硬编码扩展 id 改走 extensionInfo；「覆盖所有」导入选项两端一致（含 overwriteVscodeSettings）。
    - 测试：补 14 项/26 用例（getIncrementalChain 成环、fs.rm 失败、非字符串 refCount、截断丢弃分支、openai-responses 思考回填、getMessagesPaged 删除保护、nodeId 三条写路径、不可序列化参数 5 例、多工作区锁等价 7 例、rebaseWithRootChange/forceResetToEmpty/clearHistory 后 append 等）；settingsFixtures 收敛 4 个 settings 测试；92 处冗余 (result.data as any) 清理；it→test 遗留悬空 import 清理 81 文件；PromptSettingsSplitVerify 重命名 PromptSettingsSections 并去重；前端全仓 t() key 校验归零。
...
### Refactored
  ...
  - 模块化摊平重构·第一批（纯重构，行为零变化；依据 .graycode/research/modularization/ 八份调研报告与 .graycode/plans/modularization-plan.md）：
    ...
    - frontend chatStore.ts 消息队列编排（约 220 行）外移 `chat/queueActions.ts`，store 公共 API（含 return 对象全部属性）不变，行为逐位保留（跨会话投递防护/动作边界投递/回执/重入保护）。
  ...
  - 模块化摊平重构·第五批（解环 + 依赖方向，高风险接口级改动）：
    ...
    - 层反转修复：`OLD_STREAM_EXIT_WAIT_TIMEOUT_MS` 下沉 `backend/core/streamConstants.ts`，ChatFlowService 改从 core 导入，webview StreamAbortManager re-export 兼容（遗留：ChatFlowService 对 StreamAbortManager 类的 getGlobalInstance 既有依赖保留，已文档化）。
  ...
  - 模块化摊平重构·第七批（遗留尾巴清理：壳路径直连 + 依赖方向清零 + 跨端 parity 防漂移）：
    ...
    - 门禁验证：后端 246 suites/2591 用例（1 例 branchRace 全量并行偶发失败，单独重跑 8/8 全绿，与第二批 usageCache 同类 CPU 抢占竞态，非回归）、前端 85 文件/846 用例、双 typecheck 全绿。
  - 模块化摊平重构·第八批（遗留清理收官：shared 单一来源 + 依赖方向清零 + i18n/渲染元数据脚本化 + 测试风格统一 + 模板拆分）：
    - shared/protocol 单一来源包：新建 `shared/protocol.ts`（208 个消息名常量 MESSAGE_NAMES 经脚本从 30 个 handler 注册处机械提取并与提取源 208/208 精确比对；两份慢消息名单 UNBOUNDED_REQUEST_TYPES 20 项 / NON_BLOCKING_MESSAGE_TYPES 17 项语义分开迁入并改为引用常量，⚠️ 注释明确禁止合并；TokenDetailsEntry/UsageMetadata/SummaryTokenStats 三个两端结构一致类型迁入）；webview MessageRouter 改 re-export（保持同一 Set 实例，测试原地增删不破）、前端 vscode.ts 与 types/index.ts 改从 shared 导入、VSCodeRequest 12 个字面量改引用 MESSAGE_NAMES；构建接入 @shared 别名（frontend tsconfig/vite/vitest + 根 tsconfig include + tsconfig.test.json paths 一行），esbuild 相对路径零配置实测 bundle 通过；ContentPart/CheckpointSummary 因两端结构不一致暂缓，TODO 段记录渐进接入路径。
    - modules→tools 运行时依赖清零（hasAvailableSubAgent）：新建 `backend/core/subAgentAvailabilityBridge.ts` 查询函数桥，组合根 bootstrap 注入 tools/subagents 实现；ToolDeclarationResolver 改走桥，第七批遗留的最后一处 modules→tools 反向依赖消除。
    - 兼容壳退役：删除 `tools/file/diffManager.ts`、`tools/subagents/agentMailbox.ts`、`tools/search/regexGuard.ts`、`tools/promptToolParser.ts` 四个 re-export 壳，webview/tools 内部/测试消费方全部改直连 core（backend 全目录壳路径引用清零）。
    - 跨端可重试错误码收尾：前端 RETRYABLE_ERROR_CODES 补录 EMPTY_RESPONSE_ERROR（后端流式错误到达时错误条恢复显示重试按钮）；PARSE_ERROR 前后端注释互相引用，明确「后端自动重试不放行 vs 前端手动重试放行」为有意差异；parity 测试 KNOWN_BACKEND_ONLY 清空、KNOWN_FRONTEND_ONLY 保留 PARSE_ERROR。
    - i18n 双份合一·第一阶段：新建 `scripts/i18n-sync.mjs`（生成/校验/盘点三合一幂等脚本）+ `scripts/i18n-shared-manifest.json`（54 条三语言完全一致词条的 frontendKey→backendKey 映射）+ AUTO-GENERATED 生成物 `frontend/src/i18n/langs/_shared/`（后端语言包为单一源）+ `backend/__tests__/i18n/generatedSharedParity.test.ts`；54 条重复译文从前端语言包移除改为引用，key 集合与 languageParity 硬门禁零改动；盘点快照落盘（backend 482 叶子/语言 vs frontend 2817；13 条 en/ja 措辞差异词条留待对齐后迁移）。
    - 前端工具渲染元数据脚本化：新建 `scripts/generate-tool-meta.mjs`（内置 TS 字面量求值器，静态确定性提取，--check/--report 模式）+ AUTO-GENERATED 生成物 `frontend/src/utils/tools/__generated__/toolMeta.ts`（48 工具全覆盖，description 静态提取 37/48、parameters 39/48，动态字段诚实标记）+ `toolMetaLookup.ts` 查询层 + `backend/__tests__/parity/toolMetaParity.test.ts` 5 例；27 个前端镜像文件兜底文案改从 toolMeta 取，label/icon/contentComponent 保留手写。
    - 代码组织：settings 11 个 XxxTypes.ts 移入 `settings/types/` 子目录（22 处 import 更新，git mv 内容零变化）；conversation/index.ts 历史变更说明注释精简；maintenance/integrityCheck 接线评估落报告（需命令注册 + checkpointsDir getter，建议手动诊断命令，未执行，留待专门评审）。
    - 测试侧整理：backend/__tests__ 88 个文件 640 处 `it(`/`it.each(`/`it.skip.each(` 统一为 `test(`（脚本化替换，词边界正则零误伤，CRLF/LF 与无 BOM 保持）；CheckpointSettings.vue（2346 行）模板拆分：提取 6 个纯展示子组件（`settings/checkpoint/` 子目录，props/emits 接线、v-model 改 :model-value+emit 等价转换、状态全部留在父组件 composable），父组件降为 613 行，DOM class/事件流/i18n key 零变化。
    - 门禁验证：后端 246 suites/2595 用例（C2 全量实测 2594 passed + 1 skipped）、前端 85 文件/846 用例（+新增路由/端到端/parity 测试，数字待全量确认）、双 typecheck 全绿。
  - 模块化摊平重构·第九批（遗留收尾：门面替换 + require 静态化 + 死代码清理 + 命名规范化）与 batch9 交接清单收尾：
    - 测试直连 import 门面替换 5 批（374 处可机械替换中的 194 处 + 复核保留 196 处真内部符号）：conversation 74 + channel 40 + subagents 49 + checkpoint 28 + api 3 处；jest.mock 联动 0 处（mock 工厂形状与门面不匹配时宁保留深路径，避免静默失效）；替换后逐符号核对门面 value/type 导出，剩余深路径与 s7 保留清单逐一比对无遗漏；全仓 261 个测试文件 import 路径脚本化验证全部可解析。
    - 分类 index require→静态 import 37 处（12 个 index 文件：file 8/terminal 1/search 2/subagents 2/lsp 3/media 5/plan 2/activity 1/design 2/review 6/progress 4/history 1），保留真动态 require（DependencyManager 动态路径、SettingsHandler 条件 vscode、StdioClient/processRunner/shellConfig CJS 惰性加载）；describe 代号规范化 75+ 处（36 文件，BCP/TREE/MIG 等代号迁入文件头注释），新建 `.graycode/research/testing-conventions.md` 命名规范文档（测试文件按行为 camelCase、允许"类名.方法名"镜像、describe 禁代号裸奔）。
    - 死代码清理：`backend/index.ts`（全仓零引用）与 `backend/modules/api/types.ts`（19 个类型零消费）删除、tools/index.ts 重复导出删除（SubAgentRunConversationStore）、todo_write 死函数 loadExistingTodos、StreamRequestHandler 死方法 handleEditAndRetryStream、utils/format.ts 死函数 formatRelativeTime/formatFullTime、read_file binary+lineRange 误导性死分支、promptToolParser 混合流判定增强（成对结束标记优先）；VSCodeRequest 窄类型断言与 7 个遗留消息名清理（MESSAGE_NAMES 210→203，registry.set 179 处全用常量已验证）。
    - 组件类型收敛：PromptModule/PromptEntry/ToolInfo/DynamicContextStrategy 四处重复接口抽为 `settings/prompt/types.ts` 单一来源（7 文件统一 import，纯类型重构行为零变化）；MessageMetadata 去 `[key:string]:any` 索引签名（补 ttft 声明）；settings/index.ts 桶导出补全 12 个缺失组件。
    - 品牌统一：LimCode→GrayCode 头注释 107 处替换（16 处功能相关保留：旧格式迁移逻辑/兼容路径/用户可见文案）；i18n 双实现行为统一（composables 版复用 actualLanguage 归一化 auto、缺失 key 对齐 console.warn）；i18n 双份合一·第二阶段：40 个前端 key 措辞差异统一（15 组实质 + 7 组大小写 + 12 对 zh 措辞，以 backend 语言包为单一来源），manifest 54→160 条共享映射，i18n-sync 三语零漂移；新增 key 全部补登记（assemblyMode/dynamicSection 15 条 + common.unknownError/show/hide 3 条）。
    - 门禁验证：后端 252 suites/2693 用例、前端全量 vitest、双 typecheck 全绿（全量测试由主模型串行执行）。
...
### Added
  - 后台任务回执「完成即插入」（排队消息提前投递增强）：后台子代理 / 后台命令完成时，回执不再硬等模型回合完整结束——LLM 执行完当前动作（非终结 toolIteration，工具结果已全部落盘）且无排队消息可投时，动作边界立即投递回执（`backgroundTaskStore.flushReportsAfterAction`，与排队消息提前投递同构：`cancelStream({preserveSubAgents:true})` 替换当前回合 + 新 `chatStream`，写序保证旧流完全退出后才写入新消息）；队列非空时排队消息优先，回执等下一动作边界或回合结束补发。安全护栏复用既有语义：跨会话跳过（只投递归属当前会话的任务）、投递窗口（cancelStream 往返）内会话切换 / 并发发送者抢先开新流时放弃投递并回滚 reported 等待补发、发送失败回滚 reported 不静默丢失、`queueAfterActionDraining` 防动作边界重入。新增 `backgroundReportEarlyEmit.test.ts`（16 用例：忙时替换回合投递回执、无任务/已回流/运行中不投递、跨会话跳过、投递窗口会话切换、并发发送者抢先、发送失败回滚、重入防护、队列空编排转投回执、排队消息优先、动作边界触发编排、flushReports 持锁跳过、sendMessage false 回滚、迟到调度直投、编排窗口会话切换、失败重试恢复链）。
  - 新增「手动创建存档点」：检查点此前完全由 AI 工具执行时自动创建（受 enabled 开关与工具/消息类型配置过滤），用户没有主动存档入口——AI 做了一系列改动后想回档检查点 / 切分支时，无法先保存当前状态（用户自己手动改过的文件更是从未被存档保护）。现输入区底部工具栏新增存档按钮（codicon-save，BranchTreePanel 旁）：后端 `CheckpointManager.createCheckpoint` 支持 `forceCreate`（跳过 enabled 开关与工具/消息类型过滤，无条件创建；仍尊重排除规则——自定义忽略 / 默认类别 / 文件大小上限等安全边界不变），新 webview 消息 `checkpoint.createManual` 把存档绑定到当前最后一条消息（恢复时「回档到该消息前」= 回到现在）并绑定当前分支活跃尾节点（分支切换 chat-and-workspace 切走再切回也能恢复，往返无损）；前端 `createManualCheckpoint` action + chatStore 接线 + 三语 i18n（按钮提示 / 创建成功 / 失败）。新增测试：CheckpointManager forceCreate 无条件创建（enabled=false 且未配置工具时普通调用仍跳过）+ 手动存档可恢复工作区文件（核心价值）；checkpointActions 成功入列 / 后端拒绝 / 无会话短路 / IPC 异常兜底（4 用例）。
  - 思考强度档位扩展：OpenAI / OpenAI Responses 渠道在 xhigh 之上新增 max、ultra 档位，Anthropic 渠道在 max 之上新增 ultra 档位；三渠道统一新增「自定义 (custom)」档位——选中后出现输入框，用户可手动输入任意 effort 值（如 max、ultra），请求发送时由 formatter 将 custom 解析为 `effortCustom` 字段的值原样透传（空值回退为不发送）；配置类型 `effort` 枚举同步扩展并新增 `effortCustom` 字段；三语 i18n 新增档位文案与输入框占位提示。
...
### Security
  - 子代理不再绕过危险工具确认：主链路对 `delete_file` / `execute_command` 等被用户配置为「需确认」的工具的确认门（`toolNeedsConfirmation`，与 `isToolAutoExec` 设置同源）此前只在主会话的两个循环控制器生效，子代理（subagent）独立工具循环直接调用共享执行器、绕过了确认门；现子代理执行工具前同样查询确认门——需要确认的工具直接拒绝并把明确原因（「Ask the main model to perform this action」）以 functionResponse 回给子模型，共享执行服务缺少确认门时 fail-closed 拒绝（不静默放行）；自动执行（autoExec）与 diff 审阅类工具不受影响。新增 `subagentToolConfirmation.test.ts`（拒绝含原因回流、放行、fail-closed 三用例）。
  - 会话写锁挂起超时不再放行并发写：`ConversationManager.withConversationWriteLock`、分段历史写队列、元数据写链（storage.ts）与用量索引写队列（UsageIndexStore）此前在挂起超时后让队列链继续前进，超时后的底层文件操作仍在后台运行——新写入任务并发启动会互相覆盖/删除同一会话的段文件、索引与元数据（历史损坏）；现队列链尾挂在底层任务（underlying）上：超时只让调用方 fail-fast，链不前进，后续写必须等旧任务真正结束；挂起超时同时改为从任务真正启动时开始计时（排队等待不计入，慢而健康的写入不再被误报）。新增 `writeChainHangTimeout.test.ts`（链不前进 + fail-fast + 正常串行）并更新 `UsageIndexStore.test.ts` 挂起语义用例。
  - 流式响应缓冲加入大小上限：流式解析无法识别上游格式（非 SSE/JSON）时 `parseStreamBuffer` 把整段缓冲保留为 remaining，代理与原生 fetch 两条分支的缓冲逐轮累积且无上限，异常上游持续吐垃圾数据会造成 Extension Host 内存耗尽；现仅在「解析无进展」时按 64MB 上限终止（PARSE_ERROR，不可重试），合法巨型单事件（多模态 base64 附件等）不受影响。新增 `streamSecurity.test.ts` 超限终止 + 合法巨型事件不误杀两用例。
  - 检查点双文件提交配对一致性：`files.json` 与 `manifest.json` 两次独立 rename 无法整体原子，崩溃发生在两次提交之间时磁盘会出现「新 files.json + 旧 manifest.json」的混合配对且读取侧无法识别；现每次提交生成随机 `filesRevision` 同时写入两个文件（读取校验配对、混合配对被拒绝），写新 files.json 前旧配对被暂存为 `files.json.prev`（崩溃后读取路径自动恢复 manifest 对应的完整配对，不误报数据丢失）；`files.json.prev` 同步纳入目录遍历/大小统计/恢复路径的元数据跳过清单。新增 CheckpointManifestRepository ATOMIC-PAIR 测试（配对绑定、两崩溃窗口恢复、无备份孤儿拒绝、损坏 files.json 不裸抛、filesRevision 类型守卫）。
  - 子代理无可用工具时不再向模型声明全部工具：空工具集此前被转成 `undefined`，ChannelManager 把 undefined 当作「未指定覆盖」回退成渠道全量工具声明——模型反复调用不可用工具形成失败循环；现空数组原样透传，各 formatter 只在工具列表非空时注入声明。
  - 流式请求补上渠道配置验证：`generateStream` 此前跳过 `formatter.validateConfig`（非流式路径有），无效 API Key / URL / 模型配置不提前拦截而表现为网络/鉴权/解析错误并空耗重试；现与非流式路径一致在发起请求前校验（VALIDATION_ERROR 提前失败）。
  - 纯文本上游错误正文不再丢失：`executeStreamRequest` 原生分支、`proxyFetch.proxyStreamFetch` 无代理分支与非流式 `executeRequest` 此前先 `response.json()`（消费响应体）失败后再读 `response.text()` 只能拿到空串——网关 502 HTML / 纯文本错误体丢失；现统一改为先读 `text()` 再尝试 `JSON.parse`，真实错误正文进入错误信息。新增 `proxyFetchErrorBody.test.ts` 与 `streamSecurity.test.ts` 错误体用例。
  - 多模态流不再被误判为空响应：流式内容检测 `streamChunkHasContent` 只认 text/functionCall，Gemini `inlineData`/`fileData` 附件流在连接中断时被误判为空响应触发整条流重播（附件重复、重复计费）；现与非流式 `isResponseContentEmpty` 同一口径纳入多模态字段；前端 `messageActions` / `toolActions` / `streamChunkHandlers` 的空消息清理判定同步补齐。新增 `streamSecurity.test.ts` 多模态用例。
  - 子代理请求级上下文裁剪：子代理未接主链路 ContextTrimService，history 只增不减，长任务直接撞模型上下文上限失败；现发送前按渠道 `maxContextTokens`（缺省 128000）× 0.8 做请求级裁剪——超预算时从最旧开始整轮丢弃（functionCall/functionResponse 配对整体移除不产生孤儿，首条任务消息与末尾两轮保留），仍超预算时对超大字符串截断并标记（结构与 provider JSON 兼容），估算含 1.5× 安全系数并与多模态 part 折算对齐；裁剪不修改 run 内原 history，`lastSentHistory` 同步记录裁剪结果保证续跑前缀一致。新增 `subagentContextTrim.test.ts`（零开销原引用、整轮丢弃保配对、首条/末尾保留、超大文本/工具结果截断且不改原史、预算折算）。

## [1.4.5] - 2026-08-07

### Added
  - 渠道设置支持更改已存在渠道的渠道类型：编辑表单中「渠道类型」由只读徽章改为可选下拉框（Gemini / OpenAI Compatible / OpenAI Responses / Anthropic 互转），切换前弹三语确认框提示（明示类型特有参数将重置）；后端 `updateConfig` 放开 type 字段——类型变更时以新类型默认配置为基底重建，仅保留跨类型通用字段（名称、API Key、超时、重试、自定义标头/Body、上下文管理、Token 计数等），类型特有字段（API URL、模型列表、当前模型、高级选项、useAuthorizationHeader 等）重置为新类型默认值，避免旧类型字段残留（如 Gemini thinkingConfig 残留到 Anthropic 配置）污染请求；`createConfig`/`updateConfig` 新增运行时渠道类型校验（非法 type 直接拒绝，不再落入 default 分支产生无 url 的死配置）；显式传入的更新字段优先级最高；设置导入覆盖路径同步获得类型变更能力；若变更的是当前激活渠道，前端同时清空会话级模型覆盖（`selectedModelId`），避免残留旧类型模型 ID 被当作显式模型发送。新增 `ConfigManager.updateConfig.test.ts` 回归测试（11 用例：类型可改且 ID/createdAt 不变、类型特有字段重置、通用字段保留、显式覆盖、互转默认值、普通更新不回归、同类型不触发重建、配置不存在报错、非法 type 拒绝（更新/创建）、openai 特有字段重置）。渠道类型运行时校验常量 `CHANNEL_TYPES` 与 `ChannelType` 类型定义同源（`configs/base.ts`），新增渠道类型时类型与守卫同步维护。
  ...
  - 提示词预设（prompt entries）的 assistant 条目新增「伪造思考过程」（fakeThought）：可额外配置一段伪造的 AI 思考内容，组装请求时以 thought part（thought: true）附加在临时 assistant 消息正文之前，模拟一轮带推理过程的回复；是否随请求回传由渠道「发送历史思考内容」（sendHistoryThoughts）开关决定——显式关闭时不回传（剥离 thought part、正文照发），与真实历史思考语义一致；过滤在发送侧执行（`applyPromptContextThoughtPolicy`），不写入 turnDynamicContext 缓存（同一回合缓存可能被不同渠道复用）。
  - 渠道设置「当前轮次回传配置」与「历史回合回传配置」合并为单一「思考回传配置」区块（内部按当前轮次/历史回合分组，控件与默认值不变），三语 i18n 同步（`common.thinkingBackfill`，渠道级覆盖键迁移为 `thinkingBackfill.*`）。

### Changed
  - 排队消息提前投递：排队消息不再等待整个 LLM 回合完整结束才发出——前端在「非终结 toolIteration」边界（后端已把当前动作批次的工具结果 `settleFunctionResponses`/`addContent` 全部落盘，动作彻底结束）自动取出下一条排队消息，按与「立即发送」相同的方式替换当前回合（`cancelStream({preserveSubAgents: true})` + 新 `chatStream`）；后端 H1 写序保证（webview 层 `awaitOldStreamCompletion` + 后端 `waitForOldStreamExit`）确保旧流完全退出后才写入新用户消息，插入点之前的完整历史保持原样、不丢序；每执行完一个动作投递一条，剩余排队消息在回合结束时照常投递；发送失败自动放回队首，不静默丢失；跨会话消息跳过逻辑与 `processQueue` 一致；终结性 toolIteration（审批门闸/工具取消）与 awaitingConfirmation 阶段不触发，保持原等待语义。新增 `queuedMessageEarlyEmit.test.ts`（12 用例：streamHandler hook 接线 5 + 投递行为 7，含重入保护、跨会话跳过、失败回队首、投递窗口会话切换与并发发送者防护）。`processQueue` 与 `processQueueAfterAction` 的「取当前会话下一条消息」逻辑提取为公共函数 `takeNextForConversation`（跨会话跳过语义单一实现）。

### Fixed
  - 修复编辑消息、删除消息或重试截断历史后误报 `VALIDATION_ERROR: Invalid tool history: orphan_function_call`：共享 transcript 删除逻辑现在会识别“functionResponse 被本次删除、对应 functionCall 仍保留”的配对断裂，并在同一次仓储变更中把调用标记为 rejected（不再伪造“用户拒绝执行此工具”响应）；`deleteMessagesInRange`、`deleteToMessage`/`truncateFrom` 与单条逻辑删除统一继承。`settleFunctionResponses` 同步加固：迟到真实结果仅写回当前历史仍存在的调用，落盘后清除 rejected；调用所属旧分支已被截断时丢弃迟到响应，避免反向制造 `orphan_function_response`。新增 `orphanFunctionCallRepair.test.ts` 覆盖编辑分支复现、截断/区间/单条删除、并行调用与结算时序。
  - 修复后台任务回执（`[Background task completed]`）加载旧对话时显示为普通用户消息：`StreamRequestHandler.handleChatStream` 解构前端 `chatStream` 请求时漏透传 `source` 字段，后端落盘时 `request.source` 为 undefined，回执消息的 `source='background_task'` 从未写入历史——当前会话靠前端内存正常显示，重新加载对话后渲染成完整正文的用户消息（占地超大且无折叠卡片）；现补上 `source` 透传（新数据落盘即带标记），并在 `getMessagesPaged` / `getMessages` 返回路径对历史遗留的「缺 source 且以 `[Background task completed]` 前缀开头的用户消息」做展示层归一化补 `source='background_task'`（只补内存不写盘，避免重写历史文件）；新增 `ensureBackgroundTaskSourceForDisplay` 单元测试（缺 source 补标记 / 已有 source 不覆盖 / 普通消息与 functionResponse 不误判 / 无 parts 安全跳过）。
  ...
  - 修复 VSCode 窗口聚焦时仍播放任务完成/错误提示音：音效控制器 `handleSoundEvent` 此前只在文档隐藏（窗口最小化/切走）时聚合不播，窗口聚焦时无论焦点在哪都直接播放；现由扩展侧监听 `vscode.window.onDidChangeWindowState` 把窗口焦点状态（`windowFocusChanged` 命令）推送到前端，音效按窗口焦点分级——VSCode 窗口聚焦（用户看得见界面、事件结果已可见）不播，窗口失焦（切到其他应用）时照常播放提醒；文档隐藏期间聚合的事件在恢复可见时仍补播一次。新增 `soundEventController.test.ts`（7 用例：默认聚焦不播、聚焦不播、失焦播放、失焦后聚焦不播、隐藏聚合后恢复补播、visibilitychange 集成、事件过期丢弃）。
  - 修复流式输出自动滚动吸底的拉回与丢吸底问题：① 正文主滚动容器（CustomScrollbar sticky-bottom）此前 `wasAtBottom` 在 scroll 事件的 rAF 回调里才更新，滞后一帧——流式期间 MutationObserver 驱动的贴底写入读到陈旧状态，把刚向上滚动的用户拉回底部（「使劲滚才能滚上去」）；现 scroll 事件**同步**更新吸底状态，用户滚动意图立即生效。② 贴底写入后立即用实时 scrollHeight 复验，大段输出/md 异步解析会让 scrollHeight 在写入后继续增长，误判「用户滚离」→ 永久丢吸底；现程序贴底写入记录目标 scrollTop（`programmaticScrollTop`），其触发的 scroll 事件按写入值比对识别为程序行为、保持吸底状态，吸底状态只由用户滚动改变。③ 思维链中展开（MessageRenderBlock）`shouldStickBottom` 按 `scrollHeight - scrollTop - clientHeight` 复验，用户滚回底部后内容大段增长（scrollTop 未变、scrollHeight 变大）同样误判丢吸底；现改用 scrollTop 变化检测——scrollTop 未变 = 用户没动，内容增长不改变吸底意图。④ promote 后的 nextTick 贴底校正增加当前位置复验，用户刚滚离（scroll 事件滞后）不再被拉回。新增 `CustomScrollbarSticky.test.ts`（3 用例：底部增长贴底、滚离不打扰、写入后增长不丢吸底）与 MessageRenderBlock 中展开增长不丢吸底回归测试。
  - 修复高 tps/高刷新率下流式输出「使劲滚才能滚上去」与代码块渲染后丢吸底：① 高 tps 下内容增长会抵消滚动距离（滚 100px 内容长 80px），且 scroll 事件异步派发滞后于 rAF 贴底写入——用户滚动后同一帧的贴底写入先把 scrollTop 拉回、随后派发的 scroll 事件又被 `programmaticScrollTop` 比对误判为程序行为，用户每帧都被拉回底部；现监听 `wheel`/触摸板输入事件（输入处理阶段同步派发、早于 scroll 事件与 rAF，程序写入 scrollTop 不会触发）标记 250ms 冷静期，期间不执行贴底跟随，让用户滚动真正生效；滚动条拖动（`handleThumbMouseDown`）同样标记冷静期；中展开（MessageRenderBlock）同步加 `@wheel` 冷静期短路，重新进入中展开时重置。② 代码块 fence 闭合后遇空行触发渐进 markdown promote，程序贴底写入（CharFlow `scrollToEnd` / promote 后 nextTick 校正）改变了 scrollTop 但未同步记录，紧随其后的代码块异步渲染（hljs 高亮）让 scrollHeight 骤增——中展开的 scrollTop 变化检测把程序写入误判为「用户滚动」→ 位置复验按涨量判定「不在底部」→ 丢吸底；现 `shouldStickBottom` 返回贴底前用微任务读回写入后的实际 scrollTop（浏览器对 scrollTop 赋值有钳制，读回最准），promote 校正写入后同步记录，程序写入不再触发变化检测误判。新增测试：CustomScrollbarSticky 冷静期 2 用例（wheel 后不拉回、冷静期过后恢复贴底）、MessageRenderBlock wheel 抵消滚动距离 1 用例 + 贴底写入后代码块异步渲染高度骤增不丢吸底 1 用例。
  - 修复设置页「保存提示词」误报保存失败（实际已保存）：webview 消息处理走串行队列（`messageHandlingQueue`），`countSystemPromptTokens`（token 计数调用渠道 API）、`tokenizer.getResource`（首次下载词表，分钟级）、`exportPromptModes`/`settings.export`/`settings.import`/`storagePath.selectFolder`（showSaveDialog/showOpenDialog 模态对话框，打开期间 handler 一直 await）、`storagePath.getStats`（大目录统计）等慢 handler 会长期占住队列——期间用户点保存，`savePromptMode` 排队超过前端 180s 超时被误报「保存失败」，而队列轮到时后端实际保存成功；现将上述类型移入 `NON_BLOCKING_MESSAGE_TYPES`（fire-and-forget，handler 内部 sendResponse 仍正常路由回发起方），并同步加入前端 `UNBOUNDED_REQUEST_TYPES` 超时豁免（对话框/下载/导入本身不再超时误报）；前端 `saveConfig` 保存成功后的 token 计数改为不阻塞保存成功反馈（`void countTokens()`）。新增 `messageRouterNonBlockingBehavior.test.ts`（慢 handler 不阻塞后续消息 + 普通 handler 阻塞对照组）与 `promptSaveChain.test.ts`（Vue reactive Proxy 数据保存链路回归：序列化、postMessage、响应路由全链路）。
  - 修复设置保存「每次报失败但数据实际已写入」的根因：`VSCodeSettingsStorage` 的 `SYNCABLE_KEYS` 含 `checkForUpdates`，但 package.json 的 `contributes.configuration` 未声明 `graycode.checkForUpdates`——VS Code 对未注册配置键调用 `config.update` 直接 reject；保存时 `Promise.all` 并发写全部变更键，已注册键（如 toolsConfig）先成功落盘、未注册键（checkForUpdates，默认值 true 且从未写入过、快照恒为空）抛错，整体 save 失败——前端报「保存失败」但提示词已写入 settings.json（重启后仍在），且每次保存必然触发。现补上 `graycode.checkForUpdates` 声明（type: boolean, default: true）；新增 `vscodeSettingsStorageRegisteredKeys.test.ts` 防回归——断言 `ALL_CONFIG_KEYS` 每个键都已注册（`ALL_CONFIG_KEYS` 改为导出）。
  - 修复编辑首条（根）用户消息后会话「无论如何都不发消息」：编辑根节点时前端自动降级 keep（原地保存），但 `editAndRetry` 仍进入流式等待状态（设置 `isStreaming`/`isWaitingForResponse`）且依赖后端 complete 事件复位——keep 模式后端不重新生成，一旦 complete 丢失（视图重建/IPC 抖动/后端异常）状态永久残留，`sendMessage` 会把后续所有新消息拦截成 `deliverInterruptMessage`（投递到无人消费的 inbox），表现为停止/重试/重新保存全部失效。现 keep 模式**不进入流式等待状态**（不设 `isStreaming`/`isWaitingForResponse`/`streamingMessageId`，`activeStreamId` 保留用于 error chunk 路由），`replayBranchStreamAfterError` 的 keep 重放同步处理；后端 keep 分支 complete 改为**不带 content**（此前携带主历史最后一条消息，可能是 user 角色——前端残留占位时会把 user 内容替换进 assistant 消息，`ChatStreamCompleteData.content` 改为可选，与前端 `resetTerminalStreamState` 的「无 content 终结 chunk」协议一致）；编辑对话框对首条消息显示提示并禁用「保存」（branch）按钮（仅保留「原地保存」），消除「保存后应重新生成」的误解。新增/更新测试：keep 模式不进入流式等待状态断言、complete 无 content 路径回归。
  - 修复代码块渲染后无法复制（复制按钮被净化移除）：`sanitizeHtml` 的危险元素黑名单包含 `button`，会把 Markdown fence 渲染器生成的代码块工具栏复制/换行按钮一并移除——默认渲染路径（AI 消息正文）下代码块只有标题栏没有复制按钮，用户无法直接复制代码（测试此前只在 artifactSafe 路径断言按钮存在，default 路径被净化后一直缺失）。现 sanitize 对 class 受控的工具栏按钮（`code-tool-btn`，仅 fence 渲染器产出）放行，其余 `button` 照旧移除，on*/style 等危险属性剥离逻辑不受影响；复制执行同时加固——`navigator.clipboard` 不可用（VSCode Webview 非 secure context）或写入被拒时回退 textarea + `document.execCommand('copy')`，失败给按钮短暂红色反馈（不再静默 console.error）。新增测试：default 路径复制按钮保留 + data-code 可解码、copyToClipboard 兜底链（clipboard 缺失/抛错/双失败/选区恢复，5 用例）。
  - 修复首条（根）用户消息无法「保存并重新发送」：此前根节点无父节点可挂编辑候选（BranchGraph 单根模型），编辑对话框「保存」按钮被禁用、store 层 branch 自动降级为 keep（只改文本不重新生成）、后端 `resolveEditTargetNode` 直接拒绝根节点 branch 模式——用户编辑首条消息后无法让 AI 按新问题重新作答。现整条链路支持根节点编辑重生成：后端 branch 模式对根节点放行，执行「原地改写根节点文本 + 截断其后主历史（旧消息已归档进分支图 sidecar）+ 新建模型候选重新生成」，旧回答保留为可切换候选（与 reroll 同语义），`isFirstMessage` 动态判断（根节点编辑后刷新系统提示词）；前端 `editAndRetry` / `restoreAndEdit` 不再对根节点降级 keep，branch 模式原样透传（截断窗口 + 流式占位 + 分支图刷新标记），「原地保存（keep）」仍由编辑对话框显式选择；编辑对话框「保存」按钮对首条消息恢复可用（title 提示保存将重新生成后续回复），三语 i18n 更新。新增/更新测试：resolveEditTargetNode 根节点 branch 放行（图/线性模式）、根节点编辑重生成编排（改写 + 截断 + 候选 + finishReroll 回填 + 旧回答切回）、前端根节点 branch 流式流程。
  - 修复「伪造思考过程」（fakeThought）内容重启后丢失：前端 `PromptSettings.vue` 的 `normalizePromptEntries` 重建条目对象时只保留 id/name/type/enabled/role/content/order，`fakeThought` 字段在加载（`loadModeConfig`）与保存（`saveConfig`）两条路径都被丢弃——保存时发给后端的 promptEntries 已无 fakeThought，落盘为空，重启后输入框自然空白；现归一化时保留 `fakeThought`，`isSamePromptEntries` 同步纳入比较（修改伪造思考内容后「未保存切换模式」提示才准确）。新增 `PromptSettings.test.ts` 组件级回归测试（3 用例：加载回填、保存保留、修改后保存提交最新值）。

### Performance
  - 存档点 manifest 懒加载：从 schema version 2 起，重量级 `files` 映射（全工作区哈希表，大工作区可达 10-20MB）从 manifest.json 拆出，独立存放于 `checkpoints/cp_xxx/files.json`——`manifest.json` 只保留轻量元数据（workspaceRoots/emptyDirs/changes/excluded/ignoreSnapshot）。读取路径分级：存档列表摘要、设置页排除清单详情、恢复排除说明等只消费元数据视图（`loadManifest`），不再为少量字段解析整张文件哈希表；完整文件映射仅在恢复链构建、增量比较、链合并等真正需要的路径按需懒加载（`loadManifestWithFiles` / `enrichRecord`），经 IPC 下发给前端的 `checkpoint.getManifest` 也同步改为轻量视图。旧格式（v1 内联 files）首次读取后 best-effort 自动拆分为新格式落盘，存量存档渐进迁移：轻量读取路径解析一次后 files 进缓存、**零写放大**，拆分落盘只在完整数据读取（恢复/增量比较/合并）时触发，一次 10-20MB 级写入后后续读取全部走轻量路径；拆分失败/未发生时的 v1 存档由内联兜底继续提供数据，不误判丢失；写入顺序保证 manifest.json 为提交点（先写 files.json 再写 manifest.json，中途崩溃不会出现「新布局指向缺失文件」的不一致）。双缓存隔离：轻量元数据缓存维持既有 LRU 上限（32 条），files 映射缓存按需加载并设独立 LRU 上限（8 条），避免长驻 32 份 10-20MB 大对象；`clearCache`/删除路径同步清理双缓存。完整性检查（integrityCheck）同步支持 v2 拆分布局（files.json 缺失/损坏/ID 不符/形状非法分别上报）。新增回归测试：拆分写入、懒加载不触碰 files.json、files.json 缺失时元数据可读而完整数据显式失败、v1 读取后拆分迁移、双 LRU 淘汰、完整性检查 v2 布局（10 用例）。
  - 存档点 manifest 懒加载后续加固（审查优化）：files.json（10-20MB 级）改为紧凑序列化（无缩进）落盘，减小磁盘体积与序列化开销（manifest.json 保持缩进可读）；manifest 版本校验要求整数（非整数如 1.5 一律视为损坏走迁移/回退，不再按未知布局缓存导致数据丢失误判）；`writeManifest` 写盘失败时清空该存档双缓存，避免链合并等路径在写盘前修改过缓存对象后残留「内存与磁盘不一致」状态。新增回归测试：紧凑序列化无换行、version=1.5 迁移落盘、写失败清缓存（3 用例）。
  - 存档点 manifest 懒加载安全加固（交叉审查）：files 映射形状校验拒绝数组（`typeof [] === 'object'` 曾会让损坏的 files.json 被当作「空工作区」进入恢复流程，工作区全部文件被误判为 untracked 可删除）；manifest 元数据字段补形状校验（缺 excluded/ignoreSnapshot 等的损坏 v2 manifest 不再让列表摘要/恢复排除说明 TypeError）；L5 守卫用 `record.fileCount` 区分「真空工作区存档」（合法空快照可恢复）与「数据丢失」（显式报错）；同一存档的磁盘写入经 per-checkpointId 单飞队列串行化（消除并发迁移/合并共享 tmp 文件的 ENOENT 竞态与 files.json/manifest.json 配对错乱）；`writeManifest` 校验 manifest.checkpointId 与参数一致；legacy 恢复扫描跳过 manifest.json/files.json/*.tmp 元数据文件；integrityCheck 与运行期对未知版本（>当前）同口径（报 warning 不深校验）；`clearCache('')` 不再误清全部缓存；`checkpoint.getManifest` 返回浅拷贝防外部污染缓存。新增回归测试：files.json 数组拒绝、v1 内联数组拒绝、缺元数据字段迁移、真空存档可恢复、checkpointId 不一致抛错、clearCache('') 语义、并发写串行化、integrityCheck 未知版本（8 用例）。
...
### Added
  ...
  - 双作用域合并唤醒与检索：`memory_wake` 依次唤醒全局段 + 工作区段（`--- Global memory ---` / `--- Workspace memory (<文件夹名>) ---` 标注），两段独立分页、续读越界自动跳过、两段都读完才算 awake，压缩提示合并输出；`memory_recall` 同时搜索两个作用域并标注来源、命中数相加；`memory_note` / `memory_compress` / `memory_forget` / `memory_config` / `memory_zoom` 按当前对话工作区路由写入/操作，工作区 id 空间独立无需去重。
  - 设置页记忆分区：记忆条目区新增「全局记忆 / 工作区记忆」作用域切换 tab（工作区 tab 下拉选择「当前打开的工作区 + 已有记忆数据的工作区」，未建记忆的工作区惰性创建）；所有记忆增删改查/配置消息支持 `workspaceUri` 参数，新增 `listMemoryScopes` 消息枚举 scope；运行时参数（wakeLines 等）按作用域独立保存；三语 i18n 同步（scopeGlobal/scopeWorkspace 等 6 键）。
  - 记忆作用域数据缓存与切换即时渲染：每个作用域最近一次成功加载的条目/配置缓存在前端，切换作用域瞬间直接渲染目标作用域缓存数据、后台静默刷新（无中间加载占位帧）；配合既有请求序号竞态守卫（过期响应丢弃）、静默配置刷新（不触整页 loading）与三段互斥渲染（加载占位/空态/列表），彻底消除切换瞬间的内容闪烁与列表高度塌陷空白帧。
  - 新增回归测试 `memoryWakeScopes.test.ts`：无工作区只输出全局段（旧行为不回归）、双作用域分段标注与互不混入、工作区无记忆只出全局段、双空输出 No memories yet、memory_recall 合并命中与来源标注（6 用例）；前端新增 `MemorySettings.test.ts` 组件测试：切回已加载作用域条目立即渲染（缓存直出、无加载占位中间帧——防单帧闪烁回归）、未选工作区不发请求、seq 竞态守卫丢弃过期响应（4 用例）。
  - 记忆条目批量删除：设置页条目列表新增多选复选框与全选工具条（每行可勾选，删除所选 N 条），三语确认框确认后经新消息 `deleteMemoryEntries` 一次批量删除——后端 `MemoryManager.deleteEntries` 对乱序/重复 id 去重排序、相邻 id 聚合为闭区间、从大到小逐个 `deleteRange`（删除大 id 不影响小 id 索引），删除后剩余记忆重编号、相关树摘要清空；单条删除按钮保留。
  ...
  - 新增回归测试：`addMemory.test.ts`（note/updateEntry 记录容量校验、`listEntries(limit)` 截断语义、撕裂尾部容错、wake 批量读取一致性）、`deleteEntry.test.ts`（中间/尾部/唯一条删除、非法 id、树摘要清理与重编号连续性）、`deleteRange.test.ts`（闭区间删除/单点区间/首部整段/非法区间/树摘要清理/批量乱序去重/聚合删除/追加连续性，11 用例）、`memoryForgetTool.test.ts`（工具三模式：单 ID 删单条、闭区间删范围、块 ID 丢摘要不触原始记忆、未初始化与越界返回失败结果，5 用例）。

### Changed
  - 分支对话工作区绑定语义修正：`createBranchConversation` 不再用「当前激活工作区」兜底（此前会覆盖后端「继承源对话工作区」逻辑，把分支错误绑定到当前活动项目），未显式传 `workspaceUri` 时一律由后端继承源对话绑定；`ConversationManager.setWorkspaceUri` 签名放宽支持传 `undefined` 解绑对话（元数据持久化时字段自动移除）。
  - `memory_forget` 工具语义修正：单个数字 ID 不再「截断删除 ID ≥ 该值的全部记忆」，改为**只删除这一条**原始记忆（其余保留、id 前移重编号，等价设置页单条删除）；新增**闭区间模式**——逗号分隔 `"1,3"` 删除 ID 1 到 3 的所有原始记忆（含端点）；破折号块 ID（如 `"16-31"`）保持「仅丢弃树摘要」原语义；工具声明与前端展示（descriptionFormatter 区分 Delete memory / Delete memories / Forget block）同步；底层新增 `MemoryManager.deleteRange(lo, hi)`（闭区间原子删除）与 `deleteEntries(ids)`（批量聚合），`deleteEntry` 委托 `deleteRange(id, id)`。⚠️ 破坏性变更提示：旧语义下 `memory_forget "N"` 表示「截断删除 ID ≥ N 的全部记忆」，新语义只删一条；如需清空全部记忆请改用设置页全选删除，或显式闭区间 `"0,<最大id>"`。删除会触发后续 id 重编号，成功消息会提示重新 `memory_wake` 刷新后再继续删除。
  - 记忆容量校验落位优化（审查后）：`note` 的整条记录容量校验从锁外按 `logLen` 估算 id 改为在 `logAppend` 锁内按真实分配 id 执行（消除并发追加时 id 位数增长导致的估算偏差，彻底杜绝 `pad()` 处晦涩的 Too long）；`entryChars` 配置上限的头部开销魔数 23 提炼为 `MAX_HEADER_BYTES` 常量（附推导注释）；设置页条目列表加载上限 5000 抽为 `ENTRIES_LIMIT` 常量（与后端默认 limit 对齐），手动添加成功提示对齐 `saveConfig` 的 3 秒自动清除。
...
### Fixed
  ...
  - 修复编辑用户消息保存后候选切换器（‹ 2/2 ›）不立即显示：编辑分支流结束后分支图虽已刷新，但本地窗口中被编辑消息仍保留旧候选 id，`buildCandidateGroupForNode` 判定其为候选组非活跃成员返回 null，需切换会话再切回（loadHistory 重载后 id 与后端一致）才恢复；现 `loadBranchGraph` 刷新成功后把窗口内「候选组非活跃成员」的用户消息 id 对齐为图活跃候选（原则：窗口 id 与后端主历史 Content.id 一致），保存后立即可见切换器，幂等不误伤其他路径；新增回归测试（editBranchRefresh.test.ts 第 5 用例：complete 后 id 对齐 + 候选组命中）。
  ...
  - 子代理路径 `ToolDeclarationResolver` 监听器泄漏：每次 run 新建实例会向 McpManager 单例注册 3 个永久事件监听器且从不释放，重度多代理下监听器无界累积、MCP 事件派发退化为 O(n)；改为按依赖引用共享实例（容量 4 LRU 淘汰），并新增 `dispose()` 释放监听器，同时让子代理路径真正享受声明缓存收益。
  ...
  - `ContextTrimService.computeValidSuffixMap` 反向扫描与正向 `validateHistoryIntegrity` 在乱序配对时语义不一致：functionResponse 出现在其配对 functionCall 之前（跨消息或同消息内）时，正向判孤儿（invalid）而旧反向实现误判 valid，裁剪后可能把乱序配对发给 API；修复为按「本消息内更早 part / 右侧消息 / 待左侧治愈」三分支精确匹配正向语义。
  - 设置搜索跳转时序与状态残留：跳转定位改为按目标元素相对滚动容器偏移计算一次 `scrollTo`（避免 smooth 动画未推进时同步读 rect 的时序冲突、打断动画）；跳转成功后清空搜索词，侧边栏恢复常态高亮；`search.hint` 词条投入使用（搜索框聚焦且未输入时显示提示）。
  - `pendingWholeBuffer` 注释修正：原注释声称「上限由硬限制保护」，实际代码对未知格式的整段累积没有大小上限（与改动前行为一致），已修正为准确描述。
  ...
  - diff 部分接受/部分拒绝时前端误显示「全部接受」（根因：`apply_diff` 返回的 appliedCount/failedCount 是创建 pending diff 前的初始匹配统计，用户块级拒绝后 `DiffReviewSession` outcome 为 `partial` 但映射为公共 `accepted`，前端仅凭计数判定部分成功）：`PendingDiff` 新增 `partial`/`rejectedBlockIndices`，`finalizeAcceptedDiff` 在终结时统计被拒绝块写回；`apply_diff` 两条路径返回 `status:'partial'` 与修正后的计数（成功块-拒绝块 / 失败块+拒绝块），消息措辞区分 rejected（用户拒绝）与 failed/skipped（初始未匹配）；前端 `ToolMessage`/`apply_diff.vue` 识别 partial → warning 并做块级 rejected 标记，三语 i18n 同步；5 处工具状态推导（streamingToolProgress、ChatFlowService×2、streamChunkHandlers、buildResponseViewerData、SubAgentMonitor、agentRun reducer）统一 partial 判定，消除流式/对话框/子代理与最终卡片状态不一致；`DiffCodeLensProvider.updateBlockStatus` 由数组下标改为按 `block.index` 定位（混合成败稀疏块下标时原实现静默 no-op）；补回归测试（partial/rejectedBlockIndices、全接受反向护栏、userEditedContent、lens session 缺失、mock 泄漏修复）。
...
### Fixed
  ...
  - 修复编辑第一条消息（根节点）报 `INVALID_BRANCH_RELATION: cannot edit the root node`：BranchGraph 为单根模型，根节点没有父节点可挂编辑候选（遗留「根节点编辑暂拒」）。现在编辑根节点时前端自动降级为「原地保存」语义（keep 模式——只改写根消息，后续内容保留，与编辑对话框「保持当前分支」按钮一致）：`resolveEditTargetNode` 新增 `mode` 参数、keep 模式放行根节点（图模式与线性模式都支持），`editAndRetry` / `restoreAndEdit` 检测目标消息 `parentId == null` 时自动以 `mode='keep'` 发起 `chat.editBranchStream`（错误条重放上下文同步携带 keep 模式）；`contentToMessage` / `contentToMessageEnhanced` 透传 `content.parentId`（Message 类型新增 `parentId` 字段），前端据此识别根节点
