    ...
    components: {
        ...
        settings: {
            ...
            subagents: {
                ...
                maxConcurrentAgentsHint: 'Default 3: run up to three subtasks together and queue the rest. Use 1 to reduce simultaneous model requests. Higher values suit independent work but increase concurrent requests and resource use. -1 means unlimited.',
                defaultMaxIterations: 'Default Max Iterations',
                defaultMaxIterationsHint: 'Default 80 model/tool rounds per subtask; a round may include several tools. Increase for longer multi-step tasks, which may also use more calls. -1 removes the round limit; runtime limits still apply.',
                queueTimeoutSeconds: 'Queue Timeout (seconds)',
                queueTimeoutSecondsHint: 'Default 600 seconds (10 minutes) waiting for a concurrency slot. Once execution starts, the runtime limit applies instead. Increase when long tasks occupy limited slots; -1 waits indefinitely.',
                queueTimeoutSecondsInvalid: 'Enter -1 or a positive integer',
                defaultMaxRuntimeSeconds: 'Max Runtime (seconds)',
                defaultMaxRuntimeSecondsHint: 'Used by configured agents without their own limit. Default 1800 seconds (30 minutes) per run, including model and tool waits but excluding the initial queue and actual monitor pauses. Increase for longer tasks; -1 means unlimited.',
                generalWorker: 'Enable General Worker (easy mode)',
                generalWorkerHint: 'Lets the main model dispatch General Worker without prior setup, inheriting its model and available tools. Global concurrency, queue and iteration limits apply. Runtime uses the default below unless the main model overrides it for an invocation.',
                generalWorkerMaxRuntimeSeconds: 'General Worker Default Runtime (seconds)',
                generalWorkerMaxRuntimeSecondsHint: 'Initially 2400 seconds (40 minutes). Use 7200 (2 hours) for longer work, or lower it for short tasks. -1 means unlimited runtime. Queue time and actual monitor pauses do not count. An explicit model override takes priority. Default changes affect subsequent dispatches.',
                basicInfo: 'Basic Info',
                ...
                maxIterationsHint: 'Override the model/tool round limit for this agent. -1 removes the round limit; runtime limits still apply.',
                maxRuntime: 'Max Runtime (seconds)',
                maxRuntimeHint: 'Override the time limit per run, including model and tool waits but excluding queue time and actual pauses. -1 means unlimited.',
                inheritedLimit: 'Global: {value}',
                inheritLimitHint: 'Leave blank to inherit the global value (currently {value}); enter a value to override it.',
                systemPrompt: 'System Prompt',
            ...
            settingsPanel: {
                ...
                memory: {
                    ...
                    defaultPrompt: 'Long-term memory rules\n\nAt the start of a new work session, call memory_wake when prior agreements may affect the task. A simple, history-independent reply that needs no tools does not require it. If the output is paginated, read the parts in order until “You are awake.”\n\nMemory has global and current-workspace scopes; pay attention to the --- Global memory --- and --- Workspace memory --- markers. memory_note writes to the current workspace by default.\n\nRecord only durable information likely to help in future sessions, such as explicit user preferences and agreements, long-lived project decisions, facts that are difficult to reconstruct from the repository, and recurring verified solutions.\n\nDo not record work logs, current progress, next steps, completed checks, content directly recoverable from code or Git history, duplicates, credentials, or secrets. Do not proactively store sensitive personal information unless the user explicitly asks. When uncertain, do not record.\n\nCompression must not interrupt the current user task. pendingCompression from a successful memory_note or memory_wake is deferred maintenance: finish the current deliverable and call memory_compress later when convenient. The same pending state is not reported again, so there is no need to handle it immediately just to clear the notice. Only when memory_wake explicitly fails because a required summary is missing should you complete the requested compression first and retry wake.\n\nWhen compressing, summarize only the supplied content. Preserve durable decisions, preferences, constraints, facts, and necessary context; remove transient progress and repetition; invent nothing. Independent scopes may be handled with calls in the same response.',
                    systemPrompt: {
                        title: 'Custom Prompt',
            ...
            toolsSettings: {
                ...
                toolDisplayNames: {
                    ...
                    browser_files: 'Browser File Transfer',
                    workspace_files: 'Workspace Files',
                    search_files: 'Search File Contents',
                    run_command: 'Run Command',
                    process_session: 'Process Session',
                    context_notes: 'Task Notes',
                    goal_update: 'Update Long-Term Goal',
                    context_history: 'Conversation History',
                    new_context: 'New Context Window',
                    bot_read_attachment: 'Read Bot Attachment',
                    team_tasks: 'Team Tasks',
                    team_wait: 'Wait for Team Updates',
                    ask_user: 'Ask User',
                    read_file: 'Read File',
                ...
                toolDescriptions: {
                    ...
                    browser_files: 'Upload authorized files in the built-in browser or save downloads to a specified workspace path.',
                    workspace_files: 'List, read, write, edit or delete workspace text files. Writes check file hashes and unsaved edits.',
                    search_files: 'Search literal text in workspace files and return matching lines, skipping symlinks, repository metadata, dependencies and binary files.',
                    run_command: 'Start a program with arguments in the selected workspace. Specify a shell explicitly for shell syntax; ongoing tasks return a session ID.',
                    process_session: 'Read output, send input or stop the managed process tree of a session started by this task.',
                    context_notes: 'Save, read and append task notes in this conversation to preserve goals, progress and next steps across context windows.',
                    goal_update: 'Save progress for the current long-term goal, mark it complete, or request required user input.',
                    context_history: 'List, search and read original messages and tool results in this conversation, including records from earlier context windows by message ID.',
                    new_context: 'After saving task notes, open a fresh context window for the same task. History remains available and work continues.',
                    bot_read_attachment: 'Inspect TXT and MD documents received in a Bot conversation and read their content in ranges using an offset.',
                    team_tasks: 'Manage tasks shared by the main conversation and subagents, including dependencies, claims and completion status.',
                    team_wait: 'Wait for and read team task updates after the last event position. Waiting releases the subagent concurrency slot.',
                    ask_user: 'Ask up to three optional questions while continuing independent work, with suggested choices or free-text answers.',
