    ...
    components: {
        ...
        settings: {
            ...
            subagents: {
                ...
                maxConcurrentAgentsHint: '默认 3，最多同时运行 3 个子任务，其余排队。设为 1 可减少同时发出的模型请求；调高适合相互独立的任务，也会增加瞬时请求和资源占用。-1 表示无限制。',
                defaultMaxIterations: '默认迭代次数',
                defaultMaxIterationsHint: '默认 80，限制一个子任务的模型与工具执行轮数，一轮可能调用多个工具。调高适合多步骤任务，但可能增加总调用量；-1 表示不限制轮数，运行时间限制仍生效。',
                queueTimeoutSeconds: '排队超时（秒）',
                queueTimeoutSecondsHint: '默认 600 秒，即最多排队 10 分钟。只计算等待并发席位的时间，开始运行后使用运行时间限制。并发较低、任务较长时可调高；-1 表示一直等待。',
                queueTimeoutSecondsInvalid: '请输入 -1 或不小于 1 的整数',
                defaultMaxRuntimeSeconds: '默认最大运行时间（秒）',
                defaultMaxRuntimeSecondsHint: '自定义子代理未单独设置时使用此值。默认 1800 秒，即每次运行最多 30 分钟，包含模型响应和工具等待，不包含开始前的排队和监视器中实际暂停的时间。长任务可调高；-1 表示无限制。',
                generalWorker: '启用通用 Worker（傻瓜模式）',
                generalWorkerHint: '主模型可派发无需预先配置的 General Worker，继承当前模型及可用工具范围。遵守全局并发、排队和迭代次数设置；运行时长使用下方默认值，主模型可为单次派发另设时长。',
                generalWorkerMaxRuntimeSeconds: '通用 Worker 默认时长（秒）',
                generalWorkerMaxRuntimeSecondsHint: '初始值 2400 秒（40 分钟）。长任务可改为 7200（2 小时），短任务可调低；-1 表示不限制运行时间。排队和监视器中实际暂停的时间不计入。主模型本次指定的时长优先，修改默认值只影响后续派发。',
                basicInfo: '基本信息',
                description: sharedZhCN.components.settings.subagents.description,
                descriptionPlaceholder: '向主 AI 说明何时使用此子代理',
                maxIterations: '最大迭代次数',
                maxIterationsHint: '为此代理单独限制模型与工具执行轮数；-1 表示不限制轮数，运行时间限制仍生效。',
                maxRuntime: '最大运行时间（秒）',
                maxRuntimeHint: '为此代理单独限制每次运行时间，包含模型响应和工具等待，排队和实际暂停除外；-1 表示无限制。',
                inheritedLimit: '继承全局：{value}',
                inheritLimitHint: '留空继承全局值（当前 {value}）；填写后覆盖全局设置。',
                systemPrompt: '系统提示词',
            ...
            settingsPanel: {
                ...
                memory: {
                    ...
                    runtime: {
                        title: '运行时参数',
                        description: '工程记忆日志的唤醒、单条容量与分页设置。新的分层长期记忆在“长期记忆 → 整理方式”中配置；两者用途和预算独立。',
                        wakeLines: {
                            label: '唤醒输出行数',
                            description: '默认 96 行。增大可保留更多原始细节，减少摘要，但会增加唤醒输入量；稳定项目可降低，需要追溯较多约定时可提高。',
                            unit: '行'
                        },
                        entryChars: {
                            label: '单条记忆最大字节',
                            description: '默认 280 字节，按 UTF-8 计量，常见汉字通常占 3 字节。超过上限会拒绝保存，需缩短条目或调大容量。适合每条只记录一个持久事实。',
                            unit: '字节'
                        },
                        partChars: {
                            label: '分页最大字符数',
                            description: '默认每页 20000 字符。较小的分页便于控制单次工具结果，但完整读取需要更多次调用；分页不会减少总内容。',
                            unit: '字符'
                        },
                        partLines: {
                            label: '分页最大行数',
                            description: '默认每页 500 行，与字符上限同时生效，先达到哪个就分页。多行短记录可适当提高；长记录通常先达到字符上限。',
                            unit: '行'
                        }
                    },
                    info: {
                    ...
                    rawEntries: {
                        ...
                        workspaceNone: '当前没有可选的工作区。请在 VS Code 中打开一个文件夹后重试。',
            ...
            toolsSettings: {
                maxIterations: {
                    label: '单回合最大工具调用次数',
                    hint: '填写正整数，或 -1 表示无限制。每次模型生成计一轮，同一轮可以包含多个工具；数值越大，任务允许继续调用模型的轮数越多。清空保留当前值。',
                    invalid: '请输入正整数，或 -1 表示无限制；0 和小数无效。',
                    unit: '次'
                },
                actions: {
                    refresh: '刷新',
                ...
                toolDisplayNames: {
                    ...
                    browser_read: '读取网页',
                    browser_action: '操作网页',
                    browser_files: '浏览器文件传输',
                    workspace_files: '工作区文件',
                    search_files: '搜索文件内容',
                    run_command: '运行命令',
                    process_session: '进程会话',
                    context_notes: '任务笔记',
                    goal_update: '更新长期目标',
                ...
                toolDescriptions: {
                    ...
                    browser_files: '在内置浏览器中上传获准文件，或将下载内容保存到指定工作区路径。',
                    workspace_files: '列出、读取、写入、修改或删除工作区文本文件；写入时检查文件哈希和未保存的编辑。',
                    search_files: '在工作区文本文件中搜索指定文字并返回匹配行，跳过符号链接、版本库、依赖目录和二进制文件。',
                    run_command: '在选定工作区启动程序并传入参数；需要 Shell 语法时显式指定 Shell，持续运行的任务返回会话编号。',
                    process_session: '读取本任务所启动进程的输出、发送输入，或停止该会话管理的进程树。',
                    ...
                    ask_user: '提出最多三个可选问题，并继续处理不依赖回答的工作；支持建议选项和自由填写。',
                    ...
                    get_activity_stats: '查询用户的 IDE 使用时间统计：每日工作时长、具体工作时段与当前连续工作时长。用于了解用户的工作休息节奏、检测长时间连续工作会话，或查看用户当前是否活跃。活动按会话记录：连续 15 分钟无活动即结束当前会话，之后的活动开启新会话，因此时段之间的空档表示用户离开了编辑器（休息、睡觉或外出），不要把首次活跃到末次活跃当成一段连续工作。数据仅含时间戳，不含用户内容；返回时间为本地时间。',
                },
            },
            tokenCountSettings: {
