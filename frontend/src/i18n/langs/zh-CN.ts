    ...
    components: {
        ...
        settings: {
            ...
            subagents: {
                ...
                defaultMaxRuntimeSecondsHint: '自定义子代理未单独设置时使用此值。默认 1800 秒，即每次运行最多 30 分钟，包含模型响应和工具等待，不包含开始前的排队时间。长任务可调高；-1 表示无限制。',
                ...
                generalWorkerMaxRuntimeSecondsHint: '初始值 2400 秒（40 分钟）。长任务可改为 7200（2 小时），短任务可调低；-1 表示不限制运行时间。主模型本次指定的时长优先，修改默认值只影响后续派发。',
                basicInfo: '基本信息',
                ...
                maxRuntime: '最大运行时间（秒）',
                maxRuntimeHint: '为此代理单独限制每次运行时间，包含模型响应和工具等待；-1 表示无限制。',
