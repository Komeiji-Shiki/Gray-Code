    ...
    components: {
        ...
        settings: {
            ...
            subagents: {
                ...
                defaultMaxRuntimeSecondsHint: '個別の上限がない設定済みエージェントに適用。既定値は実行ごとに 1800 秒（30 分）で、モデルとツールの待機を含み、開始前のキュー待機は含みません。長いタスクでは増やせます。-1 は無制限です。',
                ...
                generalWorkerMaxRuntimeSecondsHint: '初期値は 2400 秒（40 分）。長い作業には 7200（2 時間）などを指定し、短い作業では減らせます。-1 は実行時間無制限です。モデルによる今回の指定が優先され、既定値の変更は次回以降の派遣に適用されます。',
                basicInfo: '基本情報',
                ...
                maxRuntime: '最大実行時間（秒）',
                maxRuntimeHint: 'モデルとツールの待機を含む、実行ごとの制限時間を個別設定します。-1 は無制限です。',
