...
const ja: LanguageMessages = {
    ...
    components: {
        ...
        usageTime: {
            ...
            totalInRange: '期間内合計',
            todayBlocks: '本日の作業時間帯',
            moreSessions: '他 {count} 件',
            range7d: '直近 7 日',
            range30d: '直近 30 日',
            range90d: '直近 90 日',
            range1y: '直近 1 年',
        ...
        input: {
            ...
            createCheckpoint: 'チェックポイントを作成（現在のワークスペース状態を保存し、後で復元可能）',
            ...
            notifications: {
                ...
                checkpointCreated: 'チェックポイントを作成しました。いつでもこの状態に復元できます',
                checkpointCreateFailed: 'チェックポイントの作成に失敗しました。もう一度お試しください',
        ...
        settings: {
            ...
            contextSettings: {
                ...
                workspaceFiles: {
                    ...
                    maxDepth: '最大深度',
                    depthHint: "既定値 2 はルート項目に加えてサブディレクトリを 2 階層展開します。0 はルート項目のみ、-1 は最大 100 階層で、項目数の上限も適用されます。大規模プロジェクトでは小さくすると取得が速くなり、コンテキストも減ります。大きくすると深い構造を含み、入力トークンが増えます。",
                ...
                openTabs: {
                    ...
                    limitHint: "既定値は開いているファイルのパス最大 20 件で、本文は含みません。0 は一覧を省略し、-1 は条件に合う全タブを含めます。無関係なファイルが多い場合は減らせます。個別ファイルは引き続きツールで読み取れます。",
                    maxCount: '最大数'
                },
                activeEditor: {
                    title: '現在のアクティブエディター',
                ...
                diagnostics: {
                    ...
                    perFileHint: "既定値はファイルごとに条件に合う診断を最大 10 件。0 は診断を省略し、-1 は無制限です。増やすとエラーの手がかりとコンテキスト量が増えます。",
                    maxPerFile: 'ファイルあたりの最大数',
                    filesHint: "既定値は診断があるファイルを最大 20 件。0 は診断を省略し、-1 は無制限です。「開いているファイルのみ」で現在の編集対象に絞れます。省略がある場合は明示されます。",
                    maxFiles: '最大ファイル数'
                },
                ignorePatterns: {
                    title: '無視パターン',
            ...
            subagents: {
                ...
                maxConcurrentAgentsHint: '既定値は 3。同時に 3 件まで実行し、残りは待機します。1 にすると同時リクエストを減らせます。値を増やすと独立した作業を並行できますが、同時リクエストと資源使用量も増えます。-1 は無制限です。',
                defaultMaxIterations: 'デフォルトのイテレーション数',
                defaultMaxIterationsHint: '既定値はモデルとツールの実行 80 ラウンド。1 ラウンドに複数のツールを含む場合があります。長い多段階タスクでは増やせますが、呼び出し総数も増える可能性があります。-1 は回数無制限で、実行時間の制限は適用されます。',
                queueTimeoutSeconds: 'キュー待機タイムアウト（秒）',
                queueTimeoutSecondsHint: '既定値は 600 秒（10 分）。同時実行枠を待つ時間だけを数え、開始後は実行時間の制限が適用されます。長いタスクで枠が埋まる場合は増やせます。-1 は無期限に待機します。',
                queueTimeoutSecondsInvalid: '-1 または正の整数を入力してください',
                defaultMaxRuntimeSeconds: '最大実行時間（秒）',
                defaultMaxRuntimeSecondsHint: '個別の上限がない設定済みエージェントに適用。既定値は実行ごとに 1800 秒（30 分）で、モデルとツールの待機を含み、開始前のキュー待機とモニターで実際に一時停止している時間は含みません。長いタスクでは増やせます。-1 は無制限です。',
                generalWorker: '汎用ワーカーを有効化（お手軽モード）',
                generalWorkerHint: '事前設定なしで General Worker を派遣し、現在のモデルと利用可能なツールを継承します。同時実行数、キュー、実行回数はグローバル設定に従います。実行時間は下の既定値を使い、メインモデルが派遣ごとに上書きできます。',
                generalWorkerMaxRuntimeSeconds: '汎用 Worker の既定実行時間（秒）',
                generalWorkerMaxRuntimeSecondsHint: '初期値は 2400 秒（40 分）。長い作業には 7200（2 時間）などを指定し、短い作業では減らせます。-1 は実行時間無制限です。キュー待機と実際の一時停止中は時間に含みません。モデルによる今回の指定が優先され、既定値の変更は次回以降の派遣に適用されます。',
                basicInfo: '基本情報',
                ...
                maxIterationsHint: 'このエージェントのモデルとツールの実行回数を個別設定します。-1 は回数無制限で、実行時間の制限は適用されます。',
                maxRuntime: '最大実行時間（秒）',
                maxRuntimeHint: 'モデルとツールの待機を含む、実行ごとの制限時間を個別設定します。キュー待機と実際の一時停止中は除外します。-1 は無制限です。',
                inheritedLimit: '全体設定：{value}',
                inheritLimitHint: '空欄でグローバル値（現在 {value}）を継承します。入力すると個別設定になります。',
                systemPrompt: 'システムプロンプト',
            ...
            settingsPanel: {
                ...
                memory: {
                    ...
                    save: '設定を保存',
                    reset: 'デフォルトに戻す',
                    defaultPrompt: '長期記憶の利用ルール\n\n新しい作業セッションの開始時に、過去の合意が現在のタスクへ影響し得る場合は memory_wake を呼び出します。履歴と無関係でツールも不要な簡単な返答では、形式のためだけに呼び出す必要はありません。出力が分割された場合は “You are awake.” まで順番に読みます。\n\n記憶にはグローバルと現在のワークスペースのスコープがあります。--- Global memory --- と --- Workspace memory --- の印を区別してください。memory_note は既定で現在のワークスペースへ書き込みます。\n\n将来のセッションでも役立つ持続的な情報だけを記録します。たとえば、ユーザーが明示した好みや合意、長期に有効なプロジェクト判断、リポジトリから再構築しにくい事実、繰り返し発生して検証済みの解決策です。\n\n作業ログ、現在の進捗、次の手順、実行済みの検証、コードや Git 履歴から直接復元できる内容、重複、認証情報、秘密は記録しません。ユーザーから明示的な依頼がない限り、機微な個人情報を自発的に保存しません。迷う場合は記録しません。\n\n圧縮のために現在のユーザータスクを中断しません。成功した memory_note または memory_wake の pendingCompression は後回しにできる保守通知です。現在の成果物を完成させ、都合のよい時に memory_compress を実行します。同じ待機状態が繰り返し通知されることはないため、通知を消すためだけにすぐ処理する必要はありません。必要な要約がないため memory_wake が明示的に失敗した場合だけ、要求された圧縮を先に完了して wake を再試行します。\n\n圧縮では提示された内容だけを要約します。持続的な判断、好み、制約、事実、必要な文脈を残し、一時的な進捗と重複を除き、捏造しません。異なるスコープの独立した圧縮は同じ応答内で呼び出せます。',
                    systemPrompt: {
                        title: 'カスタムプロンプト',
                    ...
                    runtime: {
                        title: '実行時パラメータ',
                        description: '作業記憶ログの読み出し、1 件の容量、ページ分割を設定します。階層型の長期記憶は「長期記憶 → 整理方法」で別途設定し、予算も独立しています。',
                        wakeLines: {
                            label: 'Wake 出力行数',
                            description: '既定は 96 行です。増やすと要約より元の詳細を多く残せますが、入力トークンも増えます。安定したプロジェクトでは減らし、過去の取り決めを詳しく参照するときは増やせます。',
                            unit: '行'
                        },
                        entryChars: {
                            label: 'エントリ最大バイト',
                            description: '既定は UTF-8 で 280 バイトです。一般的な日本語文字は 3 バイトです。超過すると保存は拒否されるため、内容を短くするか上限を増やしてください。1 件に 1 つの持続的な事実を記録します。',
                            unit: 'バイト'
                        },
                        partChars: {
                            label: 'ページ最大文字数',
                            description: '既定は 1 ページ 20000 文字です。小さくすると一度のツール応答を抑えられますが、全体の読み出し回数は増えます。総内容量は変わりません。',
                            unit: '文字'
                        },
                        partLines: {
                            label: 'ページ最大行数',
                            description: '既定は 1 ページ 500 行です。文字数と行数のうち、先に上限に達した時点で分割します。短い行が多い場合は増やせますが、長い記録では文字数上限に先に達します。',
                            unit: '行'
                        }
                    },
                    info: {
            ...
            toolsSettings: {
                maxIterations: {
                    ...
                    hint: '正の整数、または無制限を表す -1 を指定します。モデルの生成 1 回を 1 周として数え、同じ周に複数のツールを含められます。大きくすると生成を続けられる回数が増えます。空欄は現在値を保持します。',
                    invalid: '正の整数、または無制限を表す -1 を入力してください。0 と小数は無効です。',
                    unit: '回'
                },
                actions: {
                    refresh: '更新',
                ...
                toolDisplayNames: {
                    ...
                    browser_files: 'ブラウザーのファイル転送',
                    workspace_files: 'ワークスペースのファイル',
                    search_files: 'ファイル内容を検索',
                    run_command: 'コマンドを実行',
                    process_session: 'プロセスセッション',
                    context_notes: 'タスクノート',
                    goal_update: '長期目標を更新',
                    context_history: '会話履歴',
                    new_context: '新しいコンテキストウィンドウ',
                    bot_read_attachment: 'Bot の添付文書を読む',
                    team_tasks: 'チームタスク',
                    team_wait: 'チームの更新を待機',
                    ask_user: 'ユーザーに質問',
                    read_file: 'ファイルを読む',
                ...
                toolDescriptions: {
                    ...
                    browser_files: '内蔵ブラウザーで許可されたファイルをアップロードし、ダウンロードを指定のワークスペースパスに保存します。',
                    workspace_files: 'ワークスペース内のテキストファイルを一覧表示、読み取り、書き込み、編集、削除します。書き込み時にハッシュと未保存の編集を確認します。',
                    search_files: 'ワークスペース内のファイルから指定文字列を検索し、一致する行を返します。シンボリックリンク、リポジトリ情報、依存関係、バイナリは除外します。',
                    run_command: '選択したワークスペースで引数を指定してプログラムを起動します。シェル構文にはシェルを明示し、実行が続く場合はセッション ID を返します。',
                    process_session: 'このタスクが起動したセッションの出力を読み取り、入力を送信し、管理対象のプロセスツリーを停止します。',
                    context_notes: 'この会話のタスクノートを保存、読み取り、追記し、コンテキストを切り替えても目標、進捗、次の作業を保持します。',
                    goal_update: '現在の長期目標の進捗を保存し、完了または必要なユーザー入力を報告します。',
                    context_history: 'この会話の元のメッセージとツール結果を一覧表示、検索、読み取りします。以前のコンテキストの記録もメッセージ ID で取得できます。',
                    new_context: 'タスクノートを保存してから、同じタスクの新しいコンテキストウィンドウを開きます。履歴は引き続き参照でき、作業を継続します。',
                    bot_read_attachment: 'Bot の会話で受信した TXT、MD 文書の情報を確認し、オフセットを指定して本文を分割して読み取ります。',
                    team_tasks: 'メインの会話とサブエージェントで共有するタスク、依存関係、担当の取得、完了状態を管理します。',
                    team_wait: '最後のイベント位置以降のチームタスクの更新を待ち、取得します。待機中はサブエージェントの同時実行枠を解放します。',
                    ask_user: '回答がなくても進められる作業を続けながら、任意の質問を最大三つ提示します。選択肢と自由入力に対応します。',
                    ...
                    get_activity_stats: 'ユーザーの IDE 使用時間統計を取得します：作業した時間の長さ、具体的な作業時間帯、現在の連続作業時間。ユーザーの作業・休息リズムの把握、長時間の連続作業セッションの検出、現在アクティブかどうかの確認に使用します。活動はセッション単位で記録されます：15 分以上活動がないとセッションが終了し、その後の活動は新しいセッションとして始まります。したがって時間帯の間の空きは、ユーザーがエディターを離れていた（休憩・睡眠・外出）ことを意味します。最初の活動から最後の活動までを連続作業とみなさないでください。"sessions"（直近 7 日）は各日の実際の作業時間帯をローカル時刻の "HH:mm-HH:mm (Nm)" で示し、"gaps"（直近 1 日）はその間の空き時間を示します。データはタイムスタンプのみでユーザーコンテンツは含まれません。返り値の時刻はローカルタイムです。',
                },
            },
            tokenCountSettings: {
        ...
        channels: {
            ...
            'openai-responses': {
                ...
                reasoningSignatureMode: {
                    label: '思考署名の再送形式',
                    placeholder: '再送形式を選択',
                    hint: '公式モードは完全な GPT Responses 項目を保持し、Codex 互換モードは status を省略、DeepSeek モードは平文 reasoning_text のみを送信します。',
                    ...
                    deepseek: 'DeepSeek 互換',
                    deepseekDescription: '平文 reasoning_text のみを送信し、summary / encrypted_content / include は送信しません'
                },
                thinkingBackfill: {
