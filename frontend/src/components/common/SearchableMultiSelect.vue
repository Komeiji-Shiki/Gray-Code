...
function keydown(event: KeyboardEvent) {
  ...
  const rows = [...(panel.value?.querySelectorAll<HTMLButtonElement>('[role=option]') ?? [])];
...
<template>
  <div ref="container" class="searchable-multiselect" @change.stop>
    <button ref="trigger" class="multiselect-trigger" type="button" :aria-label="label" aria-haspopup="listbox" :aria-expanded="opened" @click="opened ? close() : show()" @keydown.down.prevent="show">
      <span class="multiselect-label" :class="{ empty: !modelValue.length }">{{ selected.slice(0, 2).join('、') || placeholder }}{{ selected.length > 2 ? '…' : '' }}</span><small v-if="modelValue.length">已选 {{ modelValue.length }} 项</small><span class="multiselect-arrow">{{ opened ? '▴' : '▾' }}</span>
    </button>
  </div>
