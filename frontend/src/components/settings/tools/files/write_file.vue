...
const accessOptions: Array<{ value: OutsideWorkspaceWriteAccess; labelKey: string; descKey: string }> = [
  {
    value: 'deny',
    labelKey: 'components.settings.toolSettings.files.outsideWorkspaceAccess.deny',
    descKey: 'components.settings.toolSettings.files.writeFile.outsideWorkspaceDenyDesc'
  },
  {
    value: 'ask',
    labelKey: 'components.settings.toolSettings.files.outsideWorkspaceAccess.ask',
    descKey: 'components.settings.toolSettings.files.writeFile.outsideWorkspaceAskDesc'
  }
]
...
async function loadConfig() {
  ...
  try {
    ...
    console.error('Failed to load write_file config:', error)
  } finally {
...
async function saveConfig() {
  ...
  try {
    ...
    console.error('Failed to save write_file config:', error)
  } finally {
    isSaving.value = false
  }
}

function updateAccess(value: OutsideWorkspaceWriteAccess) {
    ...
    <template v-else>
      <div class="config-section">
        <div class="section-header">
          ...
          <span>{{ t('components.settings.toolSettings.files.writeFile.outsideWorkspaceAccess') }}</span>
        </div>
        ...
        <div class="section-content">
          ...
          <div class="config-tip warning">
            ...
            <span>{{ t('components.settings.toolSettings.files.writeFile.outsideWorkspaceTip') }}</span>
          </div>
