...
<template>
  <div class="discord-fields">
    ...
    <label><span>每轮工具迭代上限<small>留空继承；-1 表示不限制。</small></span><input type="number" min="-1" step="1" :value="modelValue.maxIterations ?? ''" placeholder="继承默认配置" @change="set('maxIterations', ($event.target as HTMLInputElement).value === '' ? undefined : Number(($event.target as HTMLInputElement).value))" /></label>
    ...
    <div v-if="character" class="discord-inset">
      ...
      <label><span>关键词扫描范围<small>最近几条消息，条目自己的范围优先。</small></span><input type="number" min="0" step="1" :value="character.scanDepth" @change="setCharacter('scanDepth', Number(($event.target as HTMLInputElement).value))" /></label>
      <label><span>世界书最大注入 Token<small>留空不设默认预算；0 表示不注入。</small></span><input type="number" min="0" step="1" :value="character.worldTokenBudget ?? ''" placeholder="由主人填写" @change="setCharacter('worldTokenBudget', ($event.target as HTMLInputElement).value === '' ? undefined : Number(($event.target as HTMLInputElement).value))" /></label>
    </div>
