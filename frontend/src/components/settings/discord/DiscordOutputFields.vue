...
<template>
  <div class="discord-fields">
    ...
    <label v-if="effective.streaming"><span>更新间隔<small>单位为秒，范围 1 至 60 秒。</small></span><input type="number" min="1" max="60" step="0.1" :value="effective.updateIntervalMs / 1000" @change="set('updateIntervalMs', Math.round(Number(($event.target as HTMLInputElement).value) * 1000))" /></label>
