...
async function saveServer() {
  ...
  // 让用户修正原输入，不把空值、小数或越界值悄悄改成另一个设置。
  if (!Number.isSafeInteger(formData.timeout) || formData.timeout < TIMEOUT_MIN_MS || formData.timeout > TIMEOUT_MAX_MS) {
    saveError.value = t('components.settings.mcpSettings.validation.timeoutInvalid')
    return
  }
