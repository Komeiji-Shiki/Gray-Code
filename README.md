...
- 工具名称与描述随界面语言切换（中 / 英 / 日）。
- 模型传错工具参数类型时自动纠正（如字符串 `"true"` → 布尔、字符串化数组 → 数组），未知参数剥离并回传警告，不会让整次调用失败。
- 同一批相邻的只读工具（read_file、list_files、find_files、get_symbols、goto_definition、find_references、history_search、read_skill）自动并行执行，降低多次读取/搜索的累计延迟。
- 相同参数连续失败 2 次后，第 3 次相同调用会短路返回“换个思路”提示，避免模型反复重试同一失败调用浪费迭代次数。
- 文件工具结果中的路径可点击跳转到编辑器；insert_code / delete_code 还会自动定位到插入/删除行并高亮。
...
> **💡 推荐设置：** 如果你习惯连续输入多轮（长对话、持续让 AI 改代码），建议在 **设置 → 提示词** 的模式编辑中，把 **动态上下文策略** 选为 **「保留旧动态上下文原位」**。旧动态上下文会固定插回原回合位置，让请求前缀在轮次间保持稳定，从而大幅提高 LLM 侧的 Prompt Cache / KVCache 命中率（Anthropic Prompt Caching、DeepSeek KVCache 等），显著降低延迟和成本——**输入次数越多，收益越明显**。

### 多模态与附件

- 输入框支持添加文件、图片、音频、视频、文档等附件。
- 文本附件（如 txt）按 MIME 类型解码为文本块发送；PDF 在 OpenAI Responses 下转为 `input_file`、在 Anthropic 下转为 document 块；OpenAI Chat Completions 可在渠道设置中开启 `pdfAttachmentEnabled` 以文件内容块发送。
...
- 可在 设置 → 记忆 中直接查看并原地编辑所有原始记忆条目（Raw Memory Entries），编辑后自动清理相关树摘要。
- 可在 设置 → 记忆 中调整记忆运行时参数：wakeLines（唤醒输出行数）、entryChars（单条记忆最大字节）、partChars（分页最大字符数）、partLines（分页最大行数）。
...
| `memory_forget` | 丢弃错误摘要或物理截断原始记忆：传范围 ID（如 `"16-31"`）只删树摘要；传单个数字 ID（如 `"0"`）截断原始 LOG，删除 ID ≥ 该值的所有原始记忆及关联树摘要 |
...
| `preserve` | 保留每个回合缓存过的动态上下文，并尽量插回原来的历史位置；新回合上下文插入到新回合位置；请求前缀保持稳定，有利于 LLM Prompt Cache 命中 | 多轮连续编辑同一批文件、需要模型记住每轮当时看到的上下文、用户输入次数多的长对话 |

使用建议：

- 日常问答、短任务：用 `single`。
- 长任务、多轮实现、审查过程中需要保持上下文位置稳定：可以用 `preserve`。
- **用户输入次数多的场景（多轮长对话）优先选 `preserve`（即「保留旧动态上下文原位」）**：请求前缀稳定后，Anthropic Prompt Caching、DeepSeek KVCache 等缓存命中率会大幅提高，延迟和成本更低，输入越多收益越明显。
