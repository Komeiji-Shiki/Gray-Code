...
test('浏览器声明不随身份或宿主变化，拒绝与审批在访问私人页面之前执行', async () => {
  ...
  const app = await PlatformApplication.open({ dataDirectory: f.data, browser: () => host, models: { generate: async input => {
    ...
    const mutation = input.messages.some(message => message.parts.some(part => part.text === '提交表单'));
  ...
  try {
    ...
    for (const [actorId, text] of [['owner', '查看页面'], ['guest', '查看页面'], ['owner', '提交表单']]) {
      ...
      expect(response.response.success).toBe(actorId === 'owner' && text === '查看页面');
      if (actorId === 'guest' || text === '提交表单') expect(response.response.code).toBe('PERMISSION_DENIED');
      expect(finished).toContain(run.id);
    }
    expect(executed).toEqual(['owner']); expect(approvals).toHaveLength(1);
