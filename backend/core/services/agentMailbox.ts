...
import { MAIN_SESSION_RUN_ID, MAIN_AGENT_NAME, USER_INTERRUPT_MAX_LENGTH, USER_INTERRUPT_MIN_INTERVAL_MS,
    MAIN_SESSION_CLAIM_MAX_MESSAGES, MAIN_SESSION_CLAIM_MAX_CHARACTERS, prepareAgentMessage, incrementAgentThreadDepth, isUserInterruptMessage, getAgentMessageKind,
    type AgentMessage, type AgentMessageClaim, type AgentSendMessageInput, type AgentSendMessageResult,
    type MainSessionSystemMessageInput, type SystemMessageInput } from './agentMessages';
...
export class AgentMailbox {
    ...
    sendMessage(input: AgentSendMessageInput): AgentSendMessageResult {
        const result = prepareAgentMessage(input, { knownRuns: this.getKnownRuns(input.conversationId),
            pendingCount: id => (this.inboxes.get(input.conversationId)?.get(id)?.length ?? 0)
                + (this.messageClaims.get(input.conversationId)?.get(id)?.messages.length ?? 0),
            nextHopDepth: threadId => this.incrementThreadDepth(input.conversationId, threadId) });
        if (!result.success) return result;
        let inboxes = this.inboxes.get(input.conversationId);
        if (!inboxes) { inboxes = new Map(); this.inboxes.set(input.conversationId, inboxes); }
        const inbox = inboxes.get(result.data.toRunId) ?? [];
        inbox.push(result.message); inboxes.set(result.data.toRunId, inbox);
    ...
    private incrementThreadDepth(conversationId: string, threadId: string): number {
        ...
        return incrementAgentThreadDepth(convDepths, threadId);
    }
