...
import { getAllWorkspaces, resolveUriWithInfo } from '../shared/artifactHost';
import { slugify } from '../shared/slugify';
import { PROGRESS_PATH_SCOPE_LABEL, buildPathRejectedError } from '../shared/artifactHost';
import {
...
import { ensureParentDir, isProgressModePathAllowedWithMultiRoot, normalizeProgressArtifactRef, validateProgressArtifactRefInput } from './pathUtilsRuntime';
...
export function createCreateProgressTool(): Tool {
  return {
    ...
    handler: async (rawArgs: Record<string, unknown>): Promise<ToolResult> => {
      const args = parseArgs<CreateProgressArgs>(rawArgs);
      ...
      return withProgressWriteLock(outPath, async (): Promise<ToolResult> => {
        ...
        try {
          ...
          await documentWriteFile(uri, new TextEncoder().encode(content));
