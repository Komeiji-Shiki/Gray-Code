...
export interface createOutputRuntimeHost { getConfig(): ExecuteCommandToolConfig }
/** 注入宿主服务；进程、解码状态和事件均归属于当前实例。 */
export function createOutputRuntime(host: createOutputRuntimeHost) {
 ...
 */
function flushDecodeState(modeRef: StreamDecodeState, gbkDecoder?: TextDecoder): string {
    if (modeRef.mode === 'gbk' && gbkDecoder) {
