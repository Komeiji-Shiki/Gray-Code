...
export interface createShellRuntimeHost { getConfig(): ExecuteCommandToolConfig }
 ...
 */
function getEnabledShellTypes(): string[] {
    const config = host.getConfig();
 ...
 */
async function checkShellAvailability(shellType: string, customPath?: string): Promise<{
    available: boolean;
    reason?: string;
}> {
 ...
 * 检测所有 Shell 的可用性
 */
async function checkAllShellsAvailability(shells: Array<{ type: string; path?: string }>): Promise<Map<string, { available: boolean; reason?: string }>> {
...
return { getShellConfig, getEnabledShellTypes, checkShellAvailability, checkAllShellsAvailability, getAvailableShellsDescription, getDefaultShellName, getEnabledShellTypesForEnum, getDefaultShellType, getUnavailableShellsDescription };
}
