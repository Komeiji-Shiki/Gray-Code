...
export function createMemoryForgetRuntime(host: MemoryToolHost) {
const { getMemoryManagerForTool } = host;


function createMemoryForgetDeclaration(): ToolDeclaration {
    return {
        name: 'memory_forget',
        description:
            '丢弃错误的树摘要，或删除原始记忆。\n' +
...
function createMemoryForgetTool(): Tool {
    return {
...
function registerMemoryForget(): Tool {
    return createMemoryForgetTool();
}
return { createMemoryForgetDeclaration, createMemoryForgetTool, registerMemoryForget };
}
