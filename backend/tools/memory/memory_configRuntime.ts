...
const { getMemoryManagerForTool, getGlobalMemoryManager, workspaceUriToScopeKey } = host;


function createMemoryConfigDeclaration(): ToolDeclaration {
    return {
        name: 'memory_config',
        description:
            '查看或修改永久记忆系统的配置参数。\n' +
...
function createMemoryConfigTool(): Tool {
    return {
...
function registerMemoryConfig(): Tool {
    return createMemoryConfigTool();
}
return { createMemoryConfigDeclaration, createMemoryConfigTool, registerMemoryConfig };
}
