...
// （JPEG SOF 覆盖、WebP VP8L 位解析不一致），统一复用 imageUtils 实现，删除本地副本。
import { parseImageDimensions as parseImageDimensionsFromImageUtils } from '../shared/imageDimensions';
...
/** read_file 运行时所需的宿主能力，由 VSCode 门面或独立服务注入。 */
export interface ReadFileHost {
    getAllWorkspaces(): Array<{ name: string }>;
    resolveUriWithInfo(filePath: string): { uri?: unknown; workspace?: { name: string }; error?: string };
    stat(uri: unknown): Promise<{ size: number }>;
    readFile(uri: unknown): Promise<Uint8Array>;
    ensureOutsideWorkspaceAccessApproved(args: Record<string, unknown> | undefined, context?: ToolContext): string | null;
}
