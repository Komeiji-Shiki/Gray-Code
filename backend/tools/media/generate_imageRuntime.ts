...
export function createGenerateImageRuntime(host: MediaToolHost) {
 ...
 * @returns 取消订阅函数
 */
function onImageGenOutput(listener: (event: ImageGenOutputEvent) => void): () => void {
 ...
 * @returns 取消结果
 */
function cancelImageGeneration(toolId: string): {
    success: boolean;
    error?: string;
} {
 ...
 */
function getActiveImageTasks(): Array<{
    toolId: string;
    startTime: number;
}> {
...
async function readReferenceImage(imgPath: string, context?: ToolContext): Promise<{ data: string; mimeType: string } | null> {
    const image = await readImageFile(imgPath, context, 'generate_image');
    return image ? { data: image.data.toString('base64'), mimeType: image.mimeType } : null;
}
 ...
 * 注册图像生成工具（默认配置）
 */
function registerGenerateImage(): Tool {
    return createGenerateImageTool();
}
return { onImageGenOutput, generateToolId, cancelImageGeneration, getActiveImageTasks, createGenerateImageTool, registerGenerateImage };
}
