...
export function createCropImageRuntime(host: MediaToolHost) {
 ...
 * 订阅裁切输出
 */
function onCropImageOutput(listener: (event: CropImageOutputEvent) => void): () => void {
 ...
 * 取消裁切任务
 */
function cancelCropImage(toolId: string): { success: boolean; error?: string } {
...
async function executeCropTask(
    ...
    try {
        ...
        // 保存结果
        await saveImage(finalBuffer, output_path, context, 'crop_image');
 ...
 */
function createCropImageTool(maxBatchTasks: number = 10, defaultOptions?: CropImageToolOptions): Tool {
 ...
 * 注册裁切图片工具（默认配置）
 */
function registerCropImage(): Tool {
    return createCropImageTool();
}
return { onCropImageOutput, cancelCropImage, createCropImageTool, registerCropImage };
}
