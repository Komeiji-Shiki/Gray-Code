...
export function createRotateImageRuntime(host: MediaToolHost) {
 ...
 * 订阅旋转输出
 */
function onRotateImageOutput(listener: (event: RotateImageOutputEvent) => void): () => void {
 ...
 * 取消旋转任务
 */
function cancelRotateImage(toolId: string): { success: boolean; error?: string } {
 ...
 */
function createRotateImageTool(maxBatchTasks: number = 10): Tool {
 ...
 * 注册旋转图片工具（默认配置）
 */
function registerRotateImage(): Tool {
    return createRotateImageTool();
}
return { onRotateImageOutput, cancelRotateImage, createRotateImageTool, registerRotateImage };
}
