...
async function executeToolCall(
    ...
    try {
        ...
        // 注意：必须以成员访问形式调用（toolExecutionService.toolNeedsConfirmation(...)），
        // 不能先解构出方法再调用——该方法内部依赖 this（this.getToolRejectionReason），
        // 解构调用会丢失 this 绑定直接抛 TypeError，导致子代理所有工具调用全部失败。
