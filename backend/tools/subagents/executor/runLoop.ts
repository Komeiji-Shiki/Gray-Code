...
export function createDefaultExecutor(
    ...
    return async (request: SubAgentRequest, abortSignal?: AbortSignal): Promise<SubAgentResult> => {
        ...
        try {
            ...
            // 0 按无限制处理（与 limiter 现有「0 视为配置错误」语义一致，limiter 内 timeoutMs<=0 不启动定时器）。
        ...
        //          而是继承全局 defaultMaxRuntimeSeconds（-1 无限制，默认 1800 秒 = 30 分钟）。
        // 修改方式：优先取 per-agent maxRuntime，其次取全局 defaultMaxRuntimeSeconds，最后回退 1800。
        const maxRuntime = config.maxRuntime
            ?? context.settingsManager?.getSubAgentsConfig?.()?.defaultMaxRuntimeSeconds
            ?? 1800; // 默认 30 分钟
