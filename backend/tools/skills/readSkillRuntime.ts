...
import type { SkillsRuntime } from '../../modules/skills/SkillsRuntime';
import { t, getActualLanguage } from '../../i18n';
import { resolveLocalizationLanguage } from '../localization/types';


export type SkillReadHost = Pick<SkillsRuntime, 'getSkillSummaries' | 'getSkillByName' | 'getEnabledSkills'>;
/** 技能读取不依赖进程级管理器，独立任务传入自己的作用域。 */
export function createSkillReadRuntime(getSkillsManager: () => SkillReadHost | null) {
 ...
 */
function generateReadSkillDeclaration(): ToolDeclaration {
 ...
 * 获取 read_skill 工具
 */
function getReadSkillTool(): Tool {
    return {
 ...
 */
function getReadSkillToolRegistration(): ToolRegistration {
 ...
 */
function hasAvailableSkills(): boolean {
    const skillsManager = getSkillsManager();
    return skillsManager !== null && skillsManager.getEnabledSkills().length > 0;
}

return { generateReadSkillDeclaration, getReadSkillTool, getReadSkillToolRegistration, hasAvailableSkills };
}
