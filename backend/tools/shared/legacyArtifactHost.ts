...
const legacyArtifactHost: ArtifactDocumentHost = {
  ...
  read: target => vscode.workspace.fs.readFile(target as vscode.Uri).then(value => value),
  write: (target, bytes) => vscode.workspace.fs.writeFile(target as vscode.Uri, bytes).then(() => {}),
  stat: target => vscode.workspace.fs.stat(target as vscode.Uri).then(value => value),
  prepareParent: target => vscode.workspace.fs.createDirectory(vscode.Uri.file(path.dirname(target))).then(() => {}),
};
