...
function buildRequest(formatter: AnthropicFormatter, history: Content[], toolMode = 'function_call') {
    return formatter.buildRequest({
        ...
        history
    }, createAnthropicConfig({ id: 'anthropic-merge-test', name: 'Anthropic Merge Test', toolMode } as any));
}
...
describe('AnthropicFormatter 相邻同角色消息合并（function_call 模式）', () => {
    ...
    test('tool_result 后紧跟总结(user) 合并为一条 user 消息（tool_result + text）', () => {
    ...
    test('总结(user) 后紧跟真实 user 消息同样合并', () => {
        const request = buildRequest(formatter, [
            userTextMessage('（总结）之前的对话摘要', { isSummary: true }),
    ...
    test('连续 tool_result 消息合并为一条 user 消息', () => {
        const request = buildRequest(formatter, [
    ...
    test('不跨 tool_use 边界合并：assistant(tool_use) 与 user(tool_result) 保持交替', () => {
        const request = buildRequest(formatter, [
            { role: 'model', parts: [fcPart('toolu_1')] },
            { role: 'user', parts: [frPart('toolu_1')], isFunctionResponse: true },
    ...
    test('连续 assistant 消息合并（防御，很少发生）', () => {
        const request = buildRequest(formatter, [
            { role: 'model', parts: [fcPart('toolu_1')] },
            { role: 'model', parts: [fcPart('toolu_2')] }
        ]);
...
describe('AnthropicFormatter 相邻同角色消息合并（XML/JSON 文本模式）', () => {
    ...
    test('xml 模式：tool_result 文本后紧跟总结(user) 合并为一条 user 消息', () => {
        const request = buildRequest(formatter, [
            { role: 'model', parts: [fcPart('toolu_1')] },
            { role: 'user', parts: [frPart('toolu_1')], isFunctionResponse: true },
            userTextMessage('（总结）之前的对话摘要', { isSummary: true })
...
describe('ConversationManager.formatHistoryForAPI 孤儿 functionResponse 过滤（BR-07）', () => {
    ...
    test('无前置 functionCall 的孤儿 functionResponse 被剔除，消息整体跳过', () => {
    ...
    test('id 匹配的 functionResponse 正常保留（tool_use 后跟 tool_result）', () => {
        const forApi = manager.getHistoryForAPIFrom([
    ...
    test('被截断（startIndex 裁剪掉 functionCall）后的孤儿 functionResponse 被剔除', () => {
    ...
    test('无 id 的 functionResponse 保守保留（不做激进过滤）', () => {
        const forApi = manager.getHistoryForAPIFrom([
    ...
    test('混合消息：孤儿 functionResponse part 剔除，同消息文本保留', () => {
        const forApi = manager.getHistoryForAPIFrom([
            {
                role: 'user',
                parts: [
    ...
    test('拒绝的工具调用（rejected functionCall 在历史中）不误伤对应 functionResponse', () => {
        const forApi = manager.getHistoryForAPIFrom([
            { role: 'user', parts: [{ text: 'do it' }] },
