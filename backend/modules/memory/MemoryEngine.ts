...
export class MemoryEngine {
    ...
    constructor(private readonly host: MemoryHost, config?: Partial<MemoryConfig>) {
        ...
        this.store = host.store(() => this.config);
    }

    async init(): Promise<void> {
        await this.store.initStorage();
        await this.host.config.initialize(this.config);
    }
    ...
    async updateConfig(updates: Partial<MemoryConfig>): Promise<MemoryConfig> {
        ...
        const next = { ...this.config, ...validated };
        await this.host.config.save(next);
        this.config = next;
        // B-2: 返回拷贝，调用方修改返回对象不能绕过校验污染内部 config
        return { ...this.config };
    }

    /** 缺失配置保留默认值；具体宿主决定如何报告读取失败。 */
    async loadConfig(): Promise<MemoryConfig> {
        const loaded = await this.host.config.load();
