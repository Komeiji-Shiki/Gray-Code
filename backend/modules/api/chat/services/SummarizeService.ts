import { SummaryAlgorithms, BUILTIN_SUMMARIZE_SYSTEM_PROMPT, MIN_SUMMARY_LENGTH, MANUAL_SUMMARY_MAX_INPUT_RATIO,
    extractSummaryAnchorText, type AutoSummaryRequestContext } from './SummaryAlgorithms';
export { extractSummaryAnchorText } from './SummaryAlgorithms';
/**
 * GrayCode - 上下文总结服务
 *
 * 负责将对话历史压缩为总结消息
 ...
 */
export class SummarizeService extends SummaryAlgorithms {
        ...
        try {
            ...
            while (estimatedTokens > summaryBudget.maxHistoryTokens) {
                ...
                if (cutIndex <= 0) {
                    ...
                    for (let i = intraCut; i < perMessageTokens.length; i++) {
                        estimatedTokens -= perMessageTokens[i];
                    }
                    perMessageTokens.length = intraCut;
