...
export interface SkillsHost {
    ...
    onWorkspaceChange?(listener: () => void): { dispose(): void };
    resolvePath?(file: string): Promise<string>;
}
...
export class SkillsRuntime {
    ...
    private getWorkspacePathsForScan(): string[] {
        const paths = this.options.host?.workspacePaths();
        return paths?.length ? paths : this.explicitWorkspacePaths;
    }
    ...
    private buildScanDirsForPaths(workspacePaths: string[]): Array<{ path: string; source: SkillSource }> {
        ...
        if (this.options.includeUserSkills === false) return dirs;
    ...
    private registerWorkspaceFoldersListener(): void {
        ...
        const onDidChangeWorkspaceFolders = this.options.host?.onWorkspaceChange;
...
`;
        ...
        try {
            ...
            for (const entry of entries) {
                if (entry.isDirectory()) {
                    ...
                    try {
                        const requestedPath = path.join(dirPath, entry.name);
                        const fullPath = this.options.host?.resolvePath ? await this.options.host.resolvePath(requestedPath) : requestedPath;
    ...
    /** 从已捕获的原文解析技能，供只读迁移复用，避免解析期间再次读取源文件。 */
    parseSkillText(id: string, raw: string, filePath: string, source: SkillSource): Skill | null {
        ...
        if (!SkillsRuntime.validateSkillId(frontmatter.name)) {
