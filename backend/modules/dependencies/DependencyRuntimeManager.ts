 ...
 * 依赖管理器
 */
export class DependencyRuntimeManager {
    ...
    constructor(customDepsPath?: string) {
    ...
    /** 旧目录导入与安装、卸载共用队列，导入后重新读取实际安装状态。 */
    async importDirectory(operation: () => Promise<void>): Promise<void> {
        return this.enqueueMutation(async () => {
            await operation(); this.clearAllLoadedModules(); await this.refreshInstalledCache();
        });
    }

    /** 退出时等待已接收的目录切换结束，避免留下半次安装。 */
    async close(): Promise<void> { this.closing = true; await this.mutationQueue; }

    private enqueueMutation<T>(operation: () => Promise<T>): Promise<T> {
        if (this.closing) return Promise.reject(new Error('依赖管理器正在关闭。'));
