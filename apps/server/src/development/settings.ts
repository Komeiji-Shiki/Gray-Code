...
export function validateDevelopmentSettings(value: DevelopmentSettings | undefined): void {
  ...
  for (const group of [value.languageServers, value.debugAdapters]) {
    ...
    for (const item of group) {
      if (!item || !identity(item.id) || ids.has(item.id) || typeof item.name !== 'string' || !item.name.trim() ||
        typeof item.command !== 'string' || !item.command.trim() || !Array.isArray(item.args) || !item.args.every(arg => typeof arg === 'string' && !arg.includes('\0')))
  ...
  for (const adapter of value.debugAdapters ?? []) if (!['stdio', 'tcp'].includes(adapter.transport) || adapter.port !== undefined &&
