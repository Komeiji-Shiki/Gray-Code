...
export class AppearanceImages {
  ...
  async add(input: { name: string; dataUrl: string; thumbnail: string; width: number; height: number }) {
    ...
    const image: BackgroundImage = { id: randomUUID(), name: String(input.name).slice(0, 160), mimeType: match[1], bytes,
  ...
  async remove(id: string) { await this.storage.commitRecords([{ namespace: 'appearance-images', id, delete: true }, { namespace: 'appearance-image-info', id, delete: true }]); }
