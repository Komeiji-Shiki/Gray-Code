...
export function longMemoryTools(service:PlatformLongMemory):RuntimeTool[]{
  return [
    ...
    {declaration:{name:'memory_search',description:'在授权记忆范围中搜索。可限制主题、类别和两个时间。按账号、剧情与时序筛选后合并关键词和语义结果，受统一预算限制。',parameters:schema({...query,kinds:{type:'array',items:kinds}},['text'])},effects:()=>['workspace_read'],
    ...
    {declaration:{name:'memory_read',description:'按 ids 读取记忆；includeSources 附带来源。过长正文用 page 分段，sourceId 指定该记忆的来源；沿 nextOffset 续读，保留相同版本。omitted/requiredTokenBudget 提示预算不足；失效内容不会返回。',parameters:{...schema({...query,ids:{type:'array',items:{type:'string'},minItems:1,maxItems:50},includeSources:{type:'boolean'},
