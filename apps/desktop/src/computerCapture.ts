...
export class DesktopComputerCapture implements ComputerScreenPort {
  async capture(observation: ComputerObservation, size: { width: number; height: number }): Promise<ComputerCapture> {
    if (observation.window.minimized) throw new ComputerError('WINDOW_MINIMIZED', '请恢复窗口后再采集画面。');
    const sources = await desktopCapturer.getSources({ types: ['window'], thumbnailSize: size, fetchWindowIcons: false });
    ...
    return { capturedAt: Date.now(), windowId: observation.window.id, monitorId: observation.window.monitorId, dpi: observation.window.dpi,
      bounds: { ...bounds }, width: actual.width, height: actual.height, mimeType: 'image/png', data: image.toPNG().toString('base64'), method: 'window' };
  }
}
