...
import { BrowserWindow, WebContentsView, session, net, type Session } from 'electron';
import { randomUUID } from 'node:crypto';
import path from 'node:path';
import { pathToFileURL } from 'node:url';
import type { BrowserLayout, BrowserState, BrowserTab, ToolOutcome } from '@graycode/contracts';
...
export class DesktopBrowser implements BrowserHost {
  ...
  private async create(actorId: string, profileId?: string, foreground = false): Promise<OwnedTab> {
    ...
    if (parent && !parent.isDestroyed()) parent.contentView.addChildView(view);
  ...
  layout(input: BrowserLayout): void {
    ...
    for (const tab of this.tabs.values()) {
      ...
      if (active && visible) {
        ...
        tab.view.setBounds({ x, y, width: Math.max(1, Math.min(width - x, Math.round(input.width * zoom))), height: Math.max(1, Math.min(height - y, Math.round(input.height * zoom))) });
      }
      tab.view.setVisible(active && visible);
    }
  }
    ...
    const operation = tab.queue.catch(() => {}).then(async (): Promise<ToolOutcome> => {
      ...
      try {
        ...
        if (name === 'browser_action') {
          ...
          signal.throwIfAborted();
          return tab.blockedDownload ? { success: false, code: 'DOWNLOAD_DESTINATION_REQUIRED', error: '该操作触发文件下载，请通过 browser_files 指定保存路径。', data: tab.blockedDownload }
            : { success: true, data: this.describe(tab) };
        }
  ...
  private closeTab(tab: OwnedTab): void {
    ...
    const parent = this.getWindow(); if (parent && !parent.isDestroyed()) parent.contentView.removeChildView(tab.view);
  ...
  close(): void { this.closing = true; for (const tab of [...this.tabs.values()]) this.closeTab(tab); }
}
