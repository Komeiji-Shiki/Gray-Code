...
const groups = computed(() => {
  ...
  if (navigationScope.value === 'bots') return (['discord', 'onebot'] as const).map(platform => ({ key: platform, name: platform === 'discord' ? 'Discord' : 'QQ / OneBot', uri: '', workspace: undefined,
    items: items.filter(item => item.botPlatform === platform) }));
...
watch([query, navigationScope], () => { ++epoch; menu.value = undefined; initialLoading.value = true; navigation.value = { items: [], pinned: [], workspaces: [], runs: [] }; if (refreshTimer) clearTimeout(refreshTimer); refreshTimer = setTimeout(() => { refreshTimer = undefined; void refresh(true); }, 180); });
  ...
  <aside class="conversation-sidebar" :class="{ collapsed }" aria-label="对话导航">
    ...
    <div v-if="!collapsed" class="navigation-scroll">
      ...
      <section v-for="group in groups" :key="group.key" class="navigation-group">
        <div class="navigation-group-heading" :class="{ selected: group.workspace?.id === state.workspaceId }" @contextmenu="showProjectMenu($event, group)">
