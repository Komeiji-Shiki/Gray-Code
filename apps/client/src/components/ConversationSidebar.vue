...
import type { ConversationNavigationItem, ConversationNavigationResult, ConversationViewInfo } from '@graycode/contracts';
...
const menu = ref<{ item?: ConversationNavigationItem; view?: ConversationViewInfo; x: number; y: number }>();
const dialog = ref<{ kind: 'rename' | 'delete' | 'close'; item?: ConversationNavigationItem; view?: ConversationViewInfo }>();
const title = ref('');
...
const drafts = computed(() => state.conversationViews.filter(view => !view.conversationId && (view.active || view.hasDraft)));
...
const groups = computed(() => {
  ...
  const other = [...unmapped].map(([key, values]) => {
    ...
    return { key: key || 'general', name, uri, workspace: undefined, items: values };
  });
  return [...known, ...other];
});
async function refresh(reset = false, more = false) {
  ...
  try {
    const result = await rpc<ConversationNavigationResult>('conversation.navigation', { query: searching, ...(more ? { cursor: navigation.value.nextCursor } : {}) });
    ...
    const cursor = reset || more || !navigation.value.nextCursor ? result.nextCursor : navigation.value.nextCursor;
    navigation.value = { ...result, items: [...items.values()].sort((a, b) => b.updatedAt - a.updatedAt || a.id.localeCompare(b.id)), nextCursor: cursor };
    error.value = '';
...
function showMenu(event: MouseEvent, item?: ConversationNavigationItem, view?: ConversationViewInfo) {
  ...
  menu.value = { item, view: view ?? (item && views.value.get(item.id)), x: Math.max(8, Math.min(rect.right - 210, window.innerWidth - 220)), y: Math.min(rect.bottom + 3, window.innerHeight - 185) };
}
function showDialog(kind: 'rename' | 'delete' | 'close') {
  if (!menu.value) return;
  error.value = '';
  dialog.value = { ...menu.value, kind }; title.value = menu.value.item?.title ?? menu.value.view?.title ?? ''; menu.value = undefined;
...
async function confirmDialog() {
  ...
  try {
    if (selected.kind === 'rename') await rpc('conversation.rename', { conversationId: selected.item!.id, title: title.value });
    ...
    dialog.value = undefined; await refresh();
...
function dismiss(event: KeyboardEvent) { if (event.key === 'Escape') { menu.value = undefined; if (!dialogBusy.value) dialog.value = undefined; } }
watch(query, () => { ++epoch; navigation.value = { ...navigation.value, items: [], pinned: [], nextCursor: undefined }; if (refreshTimer) clearTimeout(refreshTimer); refreshTimer = setTimeout(() => { refreshTimer = undefined; void refresh(true); }, 180); });
watch(dialog, value => { state.navigationDialogOpen = !!value; });
onMounted(() => {
...
<template>
  <aside class="conversation-sidebar" :class="{ collapsed }" aria-label="对话导航">
    <div class="navigation-top"><button class="new-conversation" title="新建对话" @click="guard(() => newConversation())"><NavigationIcon name="plus" /><span v-if="!collapsed">新对话</span></button><button v-if="collapsed" title="搜索对话" aria-label="搜索对话" @click="focusSearch"><NavigationIcon name="search" /></button></div>
    ...
    <div v-if="!collapsed" class="navigation-scroll">
      ...
      <section v-if="drafts.length && !query" class="navigation-group"><h3>当前输入</h3><div v-for="view in drafts" :key="view.id" class="navigation-draft" :class="{ active: view.active }"><button @click="guard(() => command('platform.switchConversationView', { tabId: view.id }))"><NavigationIcon name="chat" /><span>{{ view.hasDraft ? '未发送的草稿' : view.title }}</span></button><button title="草稿操作" aria-label="草稿操作" @click="showMenu($event, undefined, view)"><NavigationIcon name="more" /></button></div></section>
      <section v-if="navigation.pinned.length" class="navigation-group"><h3><NavigationIcon name="pin" />置顶</h3><ConversationNavigationRow v-for="item in navigation.pinned" :key="item.id" :item="item" :active="state.conversationId === item.id" :status="runs.get(item.id)" :has-draft="views.get(item.id)?.hasDraft" @select="open(item)" @menu="showMenu($event, item)" /></section>
      <p v-if="initialLoading" class="navigation-empty">正在读取对话…</p>
      <section v-for="group in groups" :key="group.key" class="navigation-group">
        <div class="navigation-group-heading" :class="{ selected: group.workspace?.id === state.workspaceId }"><button class="navigation-project-toggle" :aria-label="`${collapsedGroups.has(group.key) ? '展开' : '收起'}${group.name}`" :aria-expanded="!collapsedGroups.has(group.key)" @click="toggleGroup(group.key)"><NavigationIcon name="chevron" :class="{ expanded: !collapsedGroups.has(group.key) }" /></button><button class="navigation-project-select" :title="group.workspace?.directory || group.uri" @click="guard(() => selectProject(group))"><NavigationIcon :name="group.uri ? 'folder' : 'chat'" /><span>{{ group.name }}</span></button><button v-if="group.workspace" class="navigation-project-add" title="在这个项目中新建对话" aria-label="在这个项目中新建对话" @click="guard(() => newConversation(group.workspace!.id))"><NavigationIcon name="plus" /></button></div>
        <template v-if="!collapsedGroups.has(group.key)"><ConversationNavigationRow v-for="item in group.items" :key="item.id" :item="item" :active="state.conversationId === item.id" :status="runs.get(item.id)" :has-draft="views.get(item.id)?.hasDraft" @select="open(item)" @menu="showMenu($event, item)" /><p v-if="!group.items.length" class="navigation-group-empty">{{ query ? '没有匹配的对话' : '暂无最近对话' }}</p></template>
      </section>
      ...
      <button class="navigation-add-project" @click="emit('addWorkspace')"><NavigationIcon name="folder" />添加项目</button>
    </div>
    <div class="navigation-bottom"><button title="全部对话历史" @click="guard(() => command('showHistory'))"><NavigationIcon name="history" /><span v-if="!collapsed">全部历史</span></button><button title="用量统计" @click="guard(() => command('showUsage'))"><NavigationIcon name="chart" /><span v-if="!collapsed">用量统计</span></button><button title="设置" @click="guard(() => command('showSettings'))"><NavigationIcon name="settings" /><span v-if="!collapsed">设置</span></button></div>
  </aside>
  <Teleport v-if="menu || dialog" to=".application"><template v-if="menu"><div class="navigation-menu-dismiss" @pointerdown="menu = undefined"></div><div class="navigation-menu" role="menu" :style="{ left: menu.x + 'px', top: menu.y + 'px' }"><button v-if="menu.item" role="menuitem" @click="showDialog('rename')">重命名</button><button v-if="menu.item" role="menuitem" @click="guard(changePin)">{{ menu.item.pinnedAt ? '取消置顶' : '置顶对话' }}</button><button v-if="menu.view" role="menuitem" @click="guard(closeView)">关闭视图，保留任务</button><button v-if="menu.item" role="menuitem" class="danger" @click="showDialog('delete')">删除对话</button></div></template>
    <div v-if="dialog" class="navigation-dialog-backdrop"><form class="navigation-dialog" role="dialog" aria-modal="true" aria-labelledby="navigation-dialog-title" @submit.prevent="confirmDialog"><h2 id="navigation-dialog-title">{{ dialog.kind === 'rename' ? '重命名对话' : dialog.kind === 'close' ? '关闭输入草稿' : '删除对话' }}</h2><input v-if="dialog.kind === 'rename'" ref="dialogInput" v-model="title" maxlength="300" required aria-label="对话标题" /><p v-else>{{ dialog.kind === 'close' ? '关闭后会丢弃这个视图中尚未发送的文字和附件，已有历史与后台任务会保留。' : `将删除「${dialog.item?.title || '未命名对话'}」及其分支记录，正在执行的任务也会停止。此操作无法撤销。` }}</p><p v-if="error" role="alert" class="navigation-error">{{ error }}</p><div class="navigation-dialog-actions"><button type="button" :disabled="dialogBusy" @click="dialog = undefined">取消</button><button :class="dialog.kind === 'rename' ? 'primary' : 'danger'" :disabled="dialogBusy">{{ dialogBusy ? '正在处理…' : dialog.kind === 'rename' ? '保存标题' : '确认' }}</button></div></form></div>
  </Teleport>
</template>
