...
<template>
  <section class="memory-imports" aria-label="独立导入资料库">
    ...
    <template v-if="selected">
      <div class="import-stats"><span><strong>{{ selected.originalFileCount.toLocaleString() }}</strong>原始文件</span><span><strong>{{ selected.records.toLocaleString() }}</strong>初次导入条目</span><span><strong>{{ memoryFileSize(selected.bytes) }}</strong>归档总大小</span></div>
