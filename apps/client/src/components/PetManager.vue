...
async function preview(command: PetCommandInput) { await player.value?.apply({ ...command, durationMs: duration.value * 1000 }); notice.value = '预览已应用。'; }
function close() { if (busy.value) return; if (JSON.stringify(configuration) !== savedDraft || pendingFiles.value.length) confirmLeave.value = true; else emit('close'); }
...
<template>
  <dialog ref="dialog" class="pet-manager" aria-labelledby="pet-title" @cancel.prevent="close">
    ...
    <div class="pet-columns"><aside>
      <button @click="emit('screenSense')">屏幕感知设置</button><p>独立开启屏幕采集与交流。</p>
    ...
    <footer v-if="confirmLeave" class="confirm" role="alert"><p>还有未保存的显示设置或待导入文件。</p><button @click="confirmLeave = false">继续编辑</button><button @click="emit('close')">放弃并关闭</button></footer>
  </dialog>
</template>
<style scoped>
