...
<template>
  <div class="node-screen">
    ...
    <template v-else>
      <p v-if="peer.capabilities?.visual" class="node-hint">此设备支持按截图像素操作；画面中的点击和拖动使用当前截图的位置。</p>
      ...
      <p class="node-hint">1 帧/秒适合看进度，2～5 帧/秒便于观察连续变化，但会增加采集和网络开销；实际帧率还受请求耗时限制。文字界面优先 PNG，带宽有限时可降低宽度或使用 JPEG。</p>
