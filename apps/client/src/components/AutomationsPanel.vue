...
async function editTask(row: AutomationView) {
  ...
  Object.assign(form, { kind: row.kind, name: row.name, objective: row.objective, target: 'current', agentId: row.agentId,
    ...
    weekDays: source?.type === 'daily' ? [...source.weekDays ?? []] : [], missedRunPolicy: row.missedRunPolicy ?? '' });
  creating.value = true;
}
...
<template>
  <dialog ref="dialog" class="automations-dialog" aria-labelledby="automations-title" @cancel.prevent="emit('close')">
    <header class="automations-header"><div><h2 id="automations-title">自动任务</h2><p>长期目标与定时执行，进度保留在关联对话中。</p></div><button aria-label="关闭自动任务" @click="emit('close')">关闭</button></header>
    ...
    <div class="automations-layout">
      <aside class="automations-list">
        ...
        <select v-model="filter" aria-label="筛选自动任务"><option value="all">全部任务</option><option value="goal">长期目标</option><option value="schedule">定时任务</option></select>
        <button v-for="row in visibleRows" :key="row.id" class="automation-row" :class="{ selected: row.id === selectedId && !creating }" @click="select(row); creating = false">
          <span>{{ row.kind === 'goal' ? '目标' : '定时' }} · {{ status(row) }}</span><strong>{{ row.name }}</strong><small>{{ used(row).toLocaleString() }} Token · {{ row.completedRuns }} 轮</small>
        </button>
        <p v-if="!visibleRows.length" class="muted empty-automations">还没有{{ filter === 'goal' ? '长期目标' : filter === 'schedule' ? '定时任务' : '自动任务' }}。</p>
      </aside>
      <main class="automation-detail">
        <form v-if="creating && options" class="automation-form" @submit.prevent="action(create)">
          ...
          <footer class="automation-actions"><button type="button" @click="creating = false">返回列表</button><button class="primary" :disabled="busy" type="submit">{{ editingId ? '保存更改，保持暂停' : form.kind === 'goal' ? '创建并开始' : '创建定时任务' }}</button></footer>
        </form>
        ...
        <p v-else class="muted empty-automations">选择一个任务查看进度，或创建新的长期目标与定时任务。</p>
      </main>
    </div>
  </dialog>
</template>
