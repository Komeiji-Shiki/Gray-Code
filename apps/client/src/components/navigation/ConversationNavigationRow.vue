...
<template>
  <div class="navigation-row" :class="{ active }" :data-conversation-id="item.id" @contextmenu.prevent="$emit('menu', $event)">
    <button class="navigation-select" :aria-current="active ? 'page' : undefined" :title="item.title || '未命名对话'" @click="$emit('select')">
      ...
      <span class="navigation-title">{{ item.title || '未命名对话' }}</span>
    </button>
...
button{border:0;background:transparent;padding:8px;min-width:0}.navigation-select{display:flex;align-items:center;gap:8px;flex:1;text-align:left;min-height:35px}.navigation-title{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.navigation-more{display:flex;padding:6px;flex-shrink:0;opacity:0}.navigation-row:hover .navigation-more,.navigation-row:focus-within .navigation-more{opacity:1}.navigation-more:hover{background:var(--hover)}
